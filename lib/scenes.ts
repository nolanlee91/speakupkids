// Ngân hàng cảnh cho Picture Detective & Picture Talk (chơi tự do, bốc ngẫu nhiên, chơi lại không lặp).
// Mỗi cảnh có nhiều câu; nội dung viết khớp đúng chi tiết trong ảnh.
// Ảnh do codex tạo; emoji chỉ là fallback nếu thiếu ảnh.

import type { Difficulty } from "./gameplay";

export type SceneQKind = "observe" | "locate" | "compare" | "infer" | "sequence";
export type SceneQ = {
  id: string; kind: SceneQKind;
  q: string; vi: string;
  options: string[]; answer: string; explainVi?: string;
  difficulty?: Difficulty; // tùy chọn — nếu thiếu sẽ suy ra từ kind
};
export type DetectiveScene = {
  id: string; title: string; vi: string; image: string; emojis: string[];
  questions: SceneQ[];
};

const IMG = "/assets/images/gen/";
const L1 = "/assets/images/learn/level-1/";
const L2 = "/assets/images/learn/level-2/";
const L3 = "/assets/images/learn/level-3/";

export const DETECTIVE_SCENES: DetectiveScene[] = [
  {
    id: "park", title: "At the Park", vi: "Ở công viên", image: IMG + "scene-park.webp",
    emojis: ["🌳", "☀️", "🐕", "🐦", "⚽", "🧒", "👧", "🌷", "🪑", "🦋"],
    questions: [
      { id: "pk1", kind: "observe", q: "What is the weather like?", vi: "Thời tiết thế nào?", options: ["It is sunny.", "It is rainy.", "It is snowy."], answer: "It is sunny." },
      { id: "pk2", kind: "observe", q: "How many children are playing?", vi: "Có mấy bạn nhỏ đang chơi?", options: ["One", "Two", "Three"], answer: "Two" },
      { id: "pk3", kind: "compare", q: "Which animal is flying?", vi: "Con vật nào đang bay?", options: ["The dog", "The blue bird", "The cat"], answer: "The blue bird" },
      { id: "pk4", kind: "observe", q: "What is the boy playing with?", vi: "Cậu bé chơi với thứ gì?", options: ["A soccer ball", "A kite", "A book"], answer: "A soccer ball" },
      { id: "pk5", kind: "infer", q: "The dog is running toward the ball. What does it want to do?", vi: "Chú chó chạy về phía quả bóng. Nó muốn làm gì?", options: ["Sleep", "Chase the ball", "Eat lunch"], answer: "Chase the ball", explainVi: "Chó chạy theo bóng nghĩa là muốn đuổi bắt bóng." },
      { id: "pk6", kind: "infer", q: "The leaves are red and orange. What season is it?", vi: "Lá cây đỏ và cam. Đang là mùa nào?", options: ["Summer", "Autumn", "Winter"], answer: "Autumn", explainVi: "Lá phong đỏ là dấu hiệu của mùa thu." },
      { id: "pk7", kind: "locate", q: "What can you see far in the background?", vi: "Phía xa có gì?", options: ["A desert", "Mountains and a city", "A farm"], answer: "Mountains and a city" },
      { id: "pk8", kind: "sequence", q: "The children are smiling and running. What probably happened?", vi: "Các bạn cười và chạy. Có lẽ vừa xảy ra gì?", options: ["They got hurt", "They started a fun game", "They went home"], answer: "They started a fun game", explainVi: "Vẻ mặt vui + đang chạy → các bạn vừa bắt đầu trò chơi." },
      { id: "pk9", kind: "infer", q: "Why are the children wearing light jackets?", vi: "Vì sao các bạn mặc áo khoác mỏng?", options: ["Because it is snowing", "Because it is cool but sunny", "Because it is very hot"], answer: "Because it is cool but sunny", explainVi: "Trời thu nắng nhẹ, hơi mát nên mặc áo khoác mỏng." },
      { id: "pk10", kind: "locate", q: "Where is the empty bench?", vi: "Cái ghế trống ở đâu?", options: ["On the left in the water", "On the right, near the tree", "Up in the sky"], answer: "On the right, near the tree" },
    ],
  },
  {
    id: "kitchen", title: "In the Kitchen", vi: "Trong bếp", image: IMG + "scene-kitchen.webp",
    emojis: ["🍎", "🍌", "🥕", "🥛", "🍞", "🧀", "🍳", "👩‍🍳", "🍽️", "🫖"],
    questions: [
      { id: "kt1", kind: "observe", q: "What is the cook making?", vi: "Người nấu đang làm món gì?", options: ["Soup", "A fried egg", "A cake"], answer: "A fried egg" },
      { id: "kt2", kind: "compare", q: "Which food is red?", vi: "Món nào màu đỏ?", options: ["The apple", "The bread", "The cheese"], answer: "The apple" },
      { id: "kt3", kind: "compare", q: "Which one is a vegetable, not a fruit?", vi: "Thứ nào là rau, không phải trái cây?", options: ["The apple", "The banana", "The carrot"], answer: "The carrot" },
      { id: "kt4", kind: "locate", q: "What is on the small plate?", vi: "Trên cái đĩa nhỏ có gì?", options: ["The bread", "The cheese", "The egg"], answer: "The cheese" },
      { id: "kt5", kind: "locate", q: "Where is the loaf of bread?", vi: "Ổ bánh mì ở đâu?", options: ["In the pan", "On a wooden board", "In the fridge"], answer: "On a wooden board" },
      { id: "kt6", kind: "observe", q: "What colour is the teapot?", vi: "Ấm trà màu gì?", options: ["Red", "Teal (blue-green)", "Yellow"], answer: "Teal (blue-green)" },
      { id: "kt7", kind: "infer", q: "The cook is holding a pan on the stove. What is she doing?", vi: "Người nấu cầm chảo trên bếp. Cô ấy đang làm gì?", options: ["Washing dishes", "Cooking breakfast", "Cleaning the floor"], answer: "Cooking breakfast", explainVi: "Cầm chảo trên bếp nóng nghĩa là đang nấu ăn." },
      { id: "kt8", kind: "infer", q: "There are bread, cheese and an egg. What meal are they making?", vi: "Có bánh mì, phô mai và trứng. Họ chuẩn bị bữa gì?", options: ["A birthday cake", "A breakfast", "A bowl of ice cream"], answer: "A breakfast", explainVi: "Bánh mì + phô mai + trứng là món ăn sáng quen thuộc." },
      { id: "kt9", kind: "locate", q: "Where is the red lamp?", vi: "Đèn đỏ ở đâu?", options: ["On the floor", "Hanging above the table", "Inside the pan"], answer: "Hanging above the table" },
      { id: "kt10", kind: "sequence", q: "The egg is in the pan and the stove is on. What happens next?", vi: "Trứng đang trong chảo, bếp đang bật. Tiếp theo sẽ thế nào?", options: ["The egg will be frozen", "The egg will be cooked", "The egg will be thrown away"], answer: "The egg will be cooked", explainVi: "Chảo nóng trên bếp → trứng sẽ chín." },
    ],
  },
  {
    id: "classroom", title: "In the Classroom", vi: "Trong lớp học", image: IMG + "scene-classroom.webp",
    emojis: ["🧑‍🏫", "🗺️", "📖", "✋", "🌋", "🎒", "✈️", "🕐", "🌍", "🪴"],
    questions: [
      { id: "cl1", kind: "observe", q: "What is the teacher pointing at?", vi: "Cô giáo đang chỉ vào gì?", options: ["The clock", "The world map", "The window"], answer: "The world map" },
      { id: "cl2", kind: "observe", q: "What is the boy in the red hoodie doing?", vi: "Cậu bé áo hoodie đỏ đang làm gì?", options: ["Reading a book", "Sleeping", "Drawing"], answer: "Reading a book" },
      { id: "cl3", kind: "observe", q: "What experiment are two students doing?", vi: "Hai bạn đang làm thí nghiệm gì?", options: ["A volcano", "A rainbow", "A robot"], answer: "A volcano" },
      { id: "cl4", kind: "locate", q: "Where is the paper airplane?", vi: "Chiếc máy bay giấy ở đâu?", options: ["On the shelf", "On the floor", "In a backpack"], answer: "On the floor" },
      { id: "cl5", kind: "locate", q: "What is on the windowsill?", vi: "Trên bệ cửa sổ có gì?", options: ["A red water bottle", "A globe", "A cake"], answer: "A red water bottle" },
      { id: "cl6", kind: "compare", q: "Who has a hand raised?", vi: "Ai đang giơ tay?", options: ["The teacher", "The girl in the teal jacket", "The boy reading"], answer: "The girl in the teal jacket" },
      { id: "cl7", kind: "infer", q: "One boy is looking under a desk. What is he probably doing?", vi: "Một cậu bé nhìn xuống gầm bàn. Có lẽ đang làm gì?", options: ["Taking a nap", "Looking for something he dropped", "Fixing the desk"], answer: "Looking for something he dropped", explainVi: "Cúi nhìn gầm bàn thường là đang tìm đồ bị rơi." },
      { id: "cl8", kind: "infer", q: "The girl is raising her hand. What does she probably want?", vi: "Bạn nữ giơ tay. Bạn ấy có lẽ muốn gì?", options: ["To leave the room", "To answer or ask something", "To sleep"], answer: "To answer or ask something", explainVi: "Giơ tay trong lớp thường để phát biểu hoặc hỏi bài." },
      { id: "cl9", kind: "observe", q: "What can you see through the window?", vi: "Nhìn qua cửa sổ thấy gì?", options: ["A beach", "Mountains and the city", "A forest"], answer: "Mountains and the city" },
      { id: "cl10", kind: "sequence", q: "A backpack is open on the floor. What probably happened before?", vi: "Balô mở trên sàn. Có lẽ trước đó đã xảy ra gì?", options: ["It was always closed", "Someone took something out of it", "It is brand new"], answer: "Someone took something out of it", explainVi: "Balô mở → ai đó vừa lấy đồ ra khỏi nó." },
    ],
  },
  {
    id: "supermarket", title: "At the Supermarket", vi: "Ở siêu thị", image: IMG + "scene-supermarket.webp",
    emojis: ["🛒", "🍎", "🍌", "🍊", "🥖", "🥛", "🧀", "📝", "☂️", "🧓"],
    questions: [
      { id: "sm1", kind: "observe", q: "What is the boy pushing?", vi: "Cậu bé đang đẩy gì?", options: ["A shopping cart", "A bike", "A stroller"], answer: "A shopping cart" },
      { id: "sm2", kind: "observe", q: "What is the boy holding in his hand?", vi: "Trên tay cậu bé cầm gì?", options: ["A phone", "A shopping list", "A ticket"], answer: "A shopping list" },
      { id: "sm3", kind: "compare", q: "Which fruit is the girl reaching for?", vi: "Bạn nữ đang với lấy trái cây nào?", options: ["Red apples", "Green apples", "Oranges"], answer: "Green apples" },
      { id: "sm4", kind: "infer", q: "The woman is holding two cereal boxes. What is she doing?", vi: "Người phụ nữ cầm hai hộp ngũ cốc. Cô ấy đang làm gì?", options: ["Juggling them", "Comparing them to choose one", "Hiding them"], answer: "Comparing them to choose one", explainVi: "Cầm hai hộp cùng lúc thường để so sánh và chọn." },
      { id: "sm5", kind: "locate", q: "What has spilled on the floor?", vi: "Thứ gì đã đổ ra sàn?", options: ["A box of cereal", "The old woman's shopping bag", "A bottle of milk"], answer: "The old woman's shopping bag" },
      { id: "sm6", kind: "sequence", q: "The old woman looks surprised and her bag is on the floor. What just happened?", vi: "Bà cụ ngạc nhiên, túi nằm dưới sàn. Vừa xảy ra chuyện gì?", options: ["She found money", "Her bag tipped over and spilled", "She won a prize"], answer: "Her bag tipped over and spilled", explainVi: "Túi đổ + đồ rơi ra + vẻ mặt bất ngờ → túi vừa bị đổ." },
      { id: "sm7", kind: "locate", q: "What is rolling on the floor?", vi: "Vật gì đang lăn trên sàn?", options: ["An orange", "An apple", "A ball"], answer: "An orange", explainVi: "Quả cam từ túi đổ ra và đang lăn đi." },
      { id: "sm8", kind: "infer", q: "Someone left a blue umbrella by the door. What was the weather probably like?", vi: "Có người để quên chiếc ô xanh cạnh cửa. Thời tiết có lẽ thế nào?", options: ["It was sunny", "It was rainy", "It was snowy"], answer: "It was rainy", explainVi: "Mang ô đi thường vì trời mưa." },
      { id: "sm9", kind: "observe", q: "Who is working at the checkout?", vi: "Ai đang làm ở quầy thu ngân?", options: ["A cashier", "A doctor", "A teacher"], answer: "A cashier" },
      { id: "sm10", kind: "sequence", q: "The boy's cart is full and he is near the checkout. What will he do next?", vi: "Xe đẩy đầy đồ, cậu bé gần quầy tính tiền. Tiếp theo sẽ làm gì?", options: ["Put everything back", "Pay for the food", "Leave with an empty cart"], answer: "Pay for the food", explainVi: "Xe đầy + gần quầy → bước tiếp theo là thanh toán." },
    ],
  },
  {
    id: "busstop", title: "A Rainy Bus Stop", vi: "Trạm xe buýt ngày mưa", image: IMG + "scene-bus-stop-rain.webp",
    emojis: ["🌧️", "🚌", "☂️", "🎒", "🎫", "👟", "🍁", "⌚", "🧥", "🏔️"],
    questions: [
      { id: "bs1", kind: "observe", q: "What is the weather like?", vi: "Thời tiết thế nào?", options: ["It is sunny", "It is rainy", "It is snowy"], answer: "It is rainy" },
      { id: "bs2", kind: "observe", q: "What is the man looking at?", vi: "Người đàn ông đang nhìn gì?", options: ["His watch", "His phone", "A book"], answer: "His watch" },
      { id: "bs3", kind: "infer", q: "The man is checking his watch and a bus is coming. What is he worried about?", vi: "Chú xem đồng hồ, xe buýt đang tới. Chú lo điều gì?", options: ["The colour of the rain", "Being on time", "His shoes"], answer: "Being on time", explainVi: "Xem giờ + xe sắp đến → lo về việc kịp giờ." },
      { id: "bs4", kind: "locate", q: "What was left on the bench?", vi: "Thứ gì bị bỏ quên trên ghế?", options: ["A backpack", "An umbrella", "A dog"], answer: "A backpack" },
      { id: "bs5", kind: "locate", q: "What is dropped on the wet ground?", vi: "Vật gì rơi trên nền đất ướt?", options: ["A bus ticket", "A phone", "A key"], answer: "A bus ticket" },
      { id: "bs6", kind: "infer", q: "The boy is running and his shoelace is untied. What might happen?", vi: "Cậu bé chạy, dây giày tuột. Điều gì có thể xảy ra?", options: ["He could fly", "He could trip and fall", "Nothing at all"], answer: "He could trip and fall", explainVi: "Chạy khi dây giày tuột dễ bị vấp ngã." },
      { id: "bs7", kind: "compare", q: "Who is holding an open umbrella?", vi: "Ai đang cầm ô đã mở?", options: ["The girl", "The man", "The old woman"], answer: "The old woman", explainVi: "Ô của bạn nữ đang gập lại; bà cụ mới là người mở ô." },
      { id: "bs8", kind: "observe", q: "What colour is the old woman's umbrella?", vi: "Ô của bà cụ màu gì?", options: ["Red", "Yellow", "Blue"], answer: "Yellow" },
      { id: "bs9", kind: "sequence", q: "The bus is coming and the backpack is still on the bench. What should someone do?", vi: "Xe tới mà balô còn trên ghế. Nên làm gì?", options: ["Ignore it", "Grab the backpack before the bus leaves", "Throw it away"], answer: "Grab the backpack before the bus leaves", explainVi: "Sắp lên xe → phải lấy balô kẻo bỏ quên." },
      { id: "bs10", kind: "infer", q: "It is raining and the leaves are red. What season is it?", vi: "Trời mưa, lá đỏ. Đang là mùa nào?", options: ["Spring", "Autumn", "Summer"], answer: "Autumn", explainVi: "Lá phong đỏ + mưa lạnh là đặc trưng mùa thu." },
    ],
  },
  {
    id: "library", title: "At the Library", vi: "Ở thư viện", image: IMG + "scene-library.webp",
    emojis: ["📚", "🪜", "🔖", "👓", "🧣", "🎒", "🪑", "🧑‍🏫", "🏙️", "📖"],
    questions: [
      { id: "lb1", kind: "observe", q: "Where are the two children reading?", vi: "Hai bạn đang đọc sách ở đâu?", options: ["On the floor", "At a table", "On the stairs"], answer: "At a table" },
      { id: "lb2", kind: "locate", q: "What is the girl standing on to reach the shelf?", vi: "Bạn nữ đứng lên vật gì để với kệ sách?", options: ["A chair", "A wooden stool", "A box"], answer: "A wooden stool" },
      { id: "lb3", kind: "observe", q: "What is the librarian pushing?", vi: "Cô thủ thư đang đẩy gì?", options: ["A book cart", "A shopping cart", "A stroller"], answer: "A book cart" },
      { id: "lb4", kind: "compare", q: "Which child is reading a pink book?", vi: "Bạn nào đang đọc quyển sách màu hồng?", options: ["The boy in green", "The girl in the yellow sweater", "The girl on the stool"], answer: "The girl in the yellow sweater" },
      { id: "lb5", kind: "locate", q: "What was left on the small side table?", vi: "Trên chiếc bàn nhỏ có gì được để lại?", options: ["A mug and glasses", "A laptop", "A ball"], answer: "A mug and glasses" },
      { id: "lb6", kind: "locate", q: "What is under the table?", vi: "Dưới gầm bàn có gì?", options: ["A backpack", "A dog", "A bike"], answer: "A backpack" },
      { id: "lb7", kind: "infer", q: "A trail of bookmarks lies on the floor. What could the children follow?", vi: "Một chuỗi bookmark rơi trên sàn. Các bạn có thể lần theo cái gì?", options: ["The lights", "The bookmarks, like a clue trail", "The windows"], answer: "The bookmarks, like a clue trail", explainVi: "Dãy bookmark nối nhau như một dấu vết để lần theo." },
      { id: "lb8", kind: "infer", q: "There is an open book on the armchair. What probably happened?", vi: "Có một cuốn sách mở trên ghế bành. Có lẽ đã xảy ra gì?", options: ["It fell from the sky", "Someone was reading and left it", "It is a decoration"], answer: "Someone was reading and left it", explainVi: "Sách mở trên ghế → ai đó vừa đọc rồi rời đi." },
      { id: "lb9", kind: "observe", q: "What can you see through the big window?", vi: "Nhìn qua cửa sổ lớn thấy gì?", options: ["Only the ocean", "Mountains and the city", "A playground"], answer: "Mountains and the city" },
      { id: "lb10", kind: "sequence", q: "The girl is reaching up high on the shelf. What is she trying to do?", vi: "Bạn nữ với lên cao trên kệ. Bạn ấy đang cố làm gì?", options: ["Hide", "Take a book from a high shelf", "Clean the window"], answer: "Take a book from a high shelf", explainVi: "Với tay lên kệ cao là để lấy một cuốn sách." },
    ],
  },
  {
    id: "sciencefair", title: "At the Science Fair", vi: "Ở hội chợ khoa học", image: L1 + "level-1-unit-07-at-the-science-fair.webp",
    emojis: ["🔬", "🌋", "🌬️", "🎗️", "☀️", "🌉", "🧑‍⚖️", "🎒", "🚗", "🏔️"],
    questions: [
      { id: "sf1", kind: "observe", q: "What is the girl in the teal cardigan presenting?", vi: "Bạn nữ áo teal đang giới thiệu gì?", options: ["A wind turbine model", "A birthday cake", "A pet dog"], answer: "A wind turbine model" },
      { id: "sf2", kind: "observe", q: "What colour is the foam coming out of the volcano?", vi: "Bọt trào ra từ núi lửa màu gì?", options: ["Red", "Blue", "Green"], answer: "Red" },
      { id: "sf3", kind: "infer", q: "A boy pours liquid into the clay volcano. What happens next?", vi: "Cậu bé đổ dung dịch vào núi lửa đất sét. Tiếp theo sẽ ra sao?", options: ["It erupts with foam", "It turns to ice", "It flies away"], answer: "It erupts with foam", explainVi: "Đổ dung dịch vào làm phản ứng, núi lửa phun bọt." },
      { id: "sf4", kind: "compare", q: "Which project uses the sun for power?", vi: "Dự án nào dùng năng lượng mặt trời?", options: ["The solar toy car", "The volcano", "The bridge"], answer: "The solar toy car", explainVi: "Chiếc xe đồ chơi có tấm pin mặt trời trên nóc." },
      { id: "sf5", kind: "observe", q: "Who is holding a clipboard?", vi: "Ai đang cầm bảng kẹp giấy?", options: ["The teacher (the judge)", "The solar car", "The turbine"], answer: "The teacher (the judge)" },
      { id: "sf6", kind: "locate", q: "Where are the prize ribbons?", vi: "Những huy hiệu ruy-băng ở đâu?", options: ["On a board on the right", "In the volcano", "Under the floor"], answer: "On a board on the right" },
      { id: "sf7", kind: "compare", q: "Which ribbon usually means first place?", vi: "Ruy-băng nào thường là giải nhất?", options: ["The blue ribbon", "The yellow ribbon", "The green ribbon"], answer: "The blue ribbon" },
      { id: "sf8", kind: "infer", q: "A boy adds heavy weights onto a model bridge. What is he testing?", vi: "Cậu bé đặt vật nặng lên cầu mô hình. Cậu đang thử điều gì?", options: ["How strong the bridge is", "How sweet it tastes", "How fast it can fly"], answer: "How strong the bridge is", explainVi: "Thêm vật nặng để kiểm tra độ chắc của cây cầu." },
      { id: "sf9", kind: "locate", q: "What can you see through the big windows?", vi: "Nhìn qua ô cửa lớn thấy gì?", options: ["Mountains and a city", "A desert", "The deep sea"], answer: "Mountains and a city" },
      { id: "sf10", kind: "sequence", q: "The judge has looked at every project. What will she do next?", vi: "Giám khảo đã xem hết các dự án. Tiếp theo cô sẽ làm gì?", options: ["Give a ribbon to the best one", "Go to sleep", "Erase all the projects"], answer: "Give a ribbon to the best one", explainVi: "Xem xong thì trao giải cho dự án tốt nhất." },
    ],
  },
  {
    id: "sciencemuseum", title: "At the Science Museum", vi: "Ở bảo tàng khoa học", image: L1 + "level-1-unit-08-at-the-science-museum.webp",
    emojis: ["🦖", "🪐", "🔭", "🧲", "🐚", "🤖", "🌈", "🎒", "🏔️", "🏙️"],
    questions: [
      { id: "mu1", kind: "observe", q: "What big skeleton stands in the hall?", vi: "Bộ xương lớn nào đứng trong sảnh?", options: ["A dinosaur", "A whale", "A horse"], answer: "A dinosaur" },
      { id: "mu2", kind: "observe", q: "What is hanging from the ceiling?", vi: "Vật gì treo trên trần?", options: ["Model planets", "Kites", "Lamps only"], answer: "Model planets" },
      { id: "mu3", kind: "observe", q: "What is the girl touching with her hand?", vi: "Bạn nữ đang chạm tay vào gì?", options: ["A plasma ball", "A candle", "A fish tank"], answer: "A plasma ball" },
      { id: "mu4", kind: "compare", q: "Which object splits light into a rainbow?", vi: "Vật nào tách ánh sáng thành cầu vồng?", options: ["The glass prism", "The magnet", "The telescope"], answer: "The glass prism" },
      { id: "mu5", kind: "locate", q: "What is the boy on the right looking through?", vi: "Cậu bé bên phải nhìn qua vật gì?", options: ["A telescope", "A window", "A book"], answer: "A telescope" },
      { id: "mu6", kind: "observe", q: "What are the two children in the middle looking at?", vi: "Hai bạn ở giữa đang xem gì?", options: ["A model Mars rover", "A cake", "A bicycle"], answer: "A model Mars rover" },
      { id: "mu7", kind: "infer", q: "The boy holds a spiral ammonite. What is it?", vi: "Cậu bé cầm một vật xoắn ốc (ammonite). Đó là gì?", options: ["A fossil", "A cookie", "A ball"], answer: "A fossil", explainVi: "Ammonite là hoá thạch con vật biển cổ đại." },
      { id: "mu8", kind: "compare", q: "Which is the shape of the magnet on the table?", vi: "Cục nam châm trên bàn có hình gì?", options: ["A horseshoe (U shape)", "A star", "A heart"], answer: "A horseshoe (U shape)" },
      { id: "mu9", kind: "infer", q: "There is a sign that says 'do not touch'. Why?", vi: "Có biển ghi 'không chạm vào'. Vì sao?", options: ["To protect the exhibits", "Because it is dark", "Because it is lunchtime"], answer: "To protect the exhibits", explainVi: "Không chạm để giữ gìn hiện vật quý." },
      { id: "mu10", kind: "sequence", q: "The children finished the rover exhibit. Where might they go next?", vi: "Các bạn xem xong khu xe tự hành. Tiếp theo có thể đi đâu?", options: ["To the telescope to look outside", "Back home to bed", "Into the fossil case"], answer: "To the telescope to look outside", explainVi: "Trong bảo tàng, các bạn đi từ khu này sang khu khác để khám phá tiếp." },
    ],
  },
  {
    id: "waterfront", title: "At the Vancouver Waterfront", vi: "Bờ nước Vancouver", image: L1 + "level-1-unit-09-at-the-vancouver-waterfront.webp",
    emojis: ["🌊", "⛴️", "🏙️", "🏔️", "🐦", "🔭", "🚲", "🍁", "☂️", "⛵"],
    questions: [
      { id: "wf1", kind: "observe", q: "Where are the children standing?", vi: "Các bạn đang đứng ở đâu?", options: ["On a seawall by the water", "In a classroom", "On a mountain top"], answer: "On a seawall by the water" },
      { id: "wf2", kind: "observe", q: "What is the girl in the teal jacket holding?", vi: "Bạn nữ áo teal đang cầm gì?", options: ["A map", "A cake", "A cat"], answer: "A map" },
      { id: "wf3", kind: "compare", q: "What big boat is sailing on the water?", vi: "Con thuyền lớn nào đang chạy trên nước?", options: ["A blue and white ferry", "A pirate ship", "A canoe"], answer: "A blue and white ferry" },
      { id: "wf4", kind: "observe", q: "What tall bird is standing near the water?", vi: "Con chim cao nào đứng gần mặt nước?", options: ["A grey heron", "A parrot", "A penguin"], answer: "A grey heron" },
      { id: "wf5", kind: "observe", q: "What is the girl in yellow using to look far away?", vi: "Bạn nữ áo vàng dùng gì để nhìn xa?", options: ["Binoculars", "A phone", "A cup"], answer: "Binoculars" },
      { id: "wf6", kind: "infer", q: "A boy is kneeling by a bicycle and touching the chain. What is he doing?", vi: "Cậu bé quỳ bên xe đạp và chạm vào xích. Cậu đang làm gì?", options: ["Fixing the bike chain", "Eating lunch", "Painting the bike"], answer: "Fixing the bike chain", explainVi: "Cúi xuống chỉnh xích nghĩa là đang sửa xe đạp." },
      { id: "wf7", kind: "locate", q: "What can you see far across the water?", vi: "Nhìn xa bên kia mặt nước thấy gì?", options: ["A city and snowy mountains", "A hot desert", "A big farm"], answer: "A city and snowy mountains" },
      { id: "wf8", kind: "infer", q: "The sky is full of grey clouds. What might happen soon?", vi: "Bầu trời đầy mây xám. Điều gì có thể sắp xảy ra?", options: ["It might rain", "It will be very hot", "It will snow candy"], answer: "It might rain", explainVi: "Mây xám dày thường báo hiệu sắp mưa." },
      { id: "wf9", kind: "locate", q: "What season is it? Look at the leaves.", vi: "Đang là mùa nào? Hãy nhìn lá cây.", options: ["Autumn", "Summer", "Spring"], answer: "Autumn", explainVi: "Lá phong đỏ cam rơi đầy đất là dấu hiệu mùa thu." },
      { id: "wf10", kind: "sequence", q: "The girl is pointing at the map. What will the friends probably do?", vi: "Bạn nữ chỉ vào bản đồ. Các bạn có lẽ sẽ làm gì?", options: ["Follow the map to their next stop", "Jump into the cold water", "Go to sleep on the path"], answer: "Follow the map to their next stop", explainVi: "Chỉ bản đồ để dẫn đường tới điểm tiếp theo." },
    ],
  },
  {
    id: "missingbackpack", title: "The Missing Backpack", vi: "Chiếc balô thất lạc", image: L2 + "level-2-unit-01-missing-backpack.webp",
    emojis: ["🎒", "🧭", "📓", "🥤", "🔎", "🚪", "🪑", "🏫"],
    questions: [
      { id: "mb1", kind: "observe", q: "Where are the children?", vi: "Các bạn đang ở đâu?", options: ["In a school hallway", "On a beach", "In a kitchen"], answer: "In a school hallway" },
      { id: "mb2", kind: "observe", q: "What is on the floor near the bench?", vi: "Trên sàn cạnh ghế có gì?", options: ["A green notebook", "A red ball", "A slice of cake"], answer: "A green notebook" },
      { id: "mb3", kind: "observe", q: "What colour is the water bottle on the floor?", vi: "Chai nước trên sàn màu gì?", options: ["Light blue", "Bright red", "Yellow"], answer: "Light blue" },
      { id: "mb4", kind: "locate", q: "What is the boy with glasses holding?", vi: "Cậu bé đeo kính cầm gì?", options: ["A map of the school", "A sandwich", "A cat"], answer: "A map of the school" },
      { id: "mb5", kind: "compare", q: "Which child is looking under the bench?", vi: "Bạn nào đang nhìn dưới gầm ghế?", options: ["The girl in the striped shirt", "The boy with glasses", "The boy in the plaid shirt"], answer: "The girl in the striped shirt" },
      { id: "mb6", kind: "observe", q: "What is the weather like outside the window?", vi: "Ngoài cửa sổ thời tiết thế nào?", options: ["Sunny", "Snowy", "Stormy"], answer: "Sunny" },
      { id: "mb7", kind: "infer", q: "The children are looking down and searching the floor. What are they doing?", vi: "Các bạn cúi xuống tìm trên sàn. Họ đang làm gì?", options: ["Looking for something lost", "Cleaning the floor", "Playing football"], answer: "Looking for something lost", explainVi: "Cúi nhìn và tìm quanh sàn nghĩa là đang tìm đồ bị mất." },
      { id: "mb8", kind: "locate", q: "What can you see through the doorway in the background?", vi: "Nhìn qua ô cửa phía sau thấy gì?", options: ["A library with bookshelves", "A swimming pool", "A farm"], answer: "A library with bookshelves" },
      { id: "mb9", kind: "infer", q: "A notebook, a card and a bottle lie on the floor. What probably happened?", vi: "Quyển vở, tấm thẻ và chai nước nằm trên sàn. Có lẽ đã xảy ra gì?", options: ["A bag tipped over and things fell out", "It started to snow indoors", "Someone built a tower"], answer: "A bag tipped over and things fell out", explainVi: "Nhiều đồ rơi vãi trên sàn → một chiếc túi đã bị đổ." },
      { id: "mb10", kind: "sequence", q: "They found clues on the floor. What should they do next?", vi: "Các bạn tìm thấy manh mối trên sàn. Tiếp theo nên làm gì?", options: ["Follow the clues to find the backpack", "Go to sleep", "Throw the clues away"], answer: "Follow the clues to find the backpack", explainVi: "Có manh mối thì lần theo để tìm chiếc balô thất lạc." },
    ],
  },
  {
    id: "classtrip", title: "Planning the Class Trip", vi: "Lên kế hoạch chuyến đi lớp", image: L3 + "collection-01-making-choices/level-3-c01-unit-01-planning-class-trip.webp",
    emojis: ["🗺️", "🔭", "🌲", "🦌", "⛴️", "🗳️", "🌍", "🧑‍🏫"],
    questions: [
      { id: "ct1", kind: "observe", q: "Where are the students?", vi: "Các bạn học sinh đang ở đâu?", options: ["In a classroom", "On a boat", "In a park"], answer: "In a classroom" },
      { id: "ct2", kind: "observe", q: "How many trip choices are on the table?", vi: "Trên bàn có mấy lựa chọn chuyến đi?", options: ["Two", "Three", "Five"], answer: "Three" },
      { id: "ct3", kind: "observe", q: "What is on the first card?", vi: "Trên tấm thẻ đầu tiên có gì?", options: ["A telescope and a science flask", "A birthday cake", "A racing car"], answer: "A telescope and a science flask" },
      { id: "ct4", kind: "compare", q: "Which card shows a forest with a deer?", vi: "Tấm thẻ nào có rừng cây và một con hươu?", options: ["The middle card", "The first card", "The last card"], answer: "The middle card" },
      { id: "ct5", kind: "observe", q: "What is on the last card?", vi: "Trên tấm thẻ cuối cùng có gì?", options: ["A boat with an anchor", "An airplane", "A school bus"], answer: "A boat with an anchor" },
      { id: "ct6", kind: "locate", q: "Who is holding a small voting board?", vi: "Ai đang cầm bảng bỏ phiếu nhỏ?", options: ["The teacher", "The youngest boy", "Nobody"], answer: "The teacher" },
      { id: "ct7", kind: "locate", q: "What is on the shelf in the background?", vi: "Trên kệ phía sau có gì?", options: ["A globe", "A television", "A fish tank"], answer: "A globe" },
      { id: "ct8", kind: "infer", q: "The students point at the cards and talk. What are they doing?", vi: "Các bạn chỉ vào các thẻ và trò chuyện. Họ đang làm gì?", options: ["Choosing where to go on a trip", "Cooking dinner", "Cleaning the room"], answer: "Choosing where to go on a trip", explainVi: "Chỉ vào các lựa chọn và bàn bạc → đang chọn nơi đi chơi." },
      { id: "ct9", kind: "infer", q: "Each card has small tokens under it. What are the tokens for?", vi: "Dưới mỗi thẻ có những đồng nhỏ. Chúng dùng để làm gì?", options: ["Voting and comparing the choices", "Buying food", "Telling the time"], answer: "Voting and comparing the choices", explainVi: "Các đồng nhỏ dùng để bỏ phiếu, so sánh giữa các lựa chọn." },
      { id: "ct10", kind: "sequence", q: "After everyone votes, what will the class do?", vi: "Sau khi mọi người bỏ phiếu, cả lớp sẽ làm gì?", options: ["Pick the trip with the most votes", "Go home immediately", "Erase the whole board"], answer: "Pick the trip with the most votes", explainVi: "Bỏ phiếu xong thì chọn chuyến đi được nhiều phiếu nhất." },
    ],
  },
  {
    id: "talentshow", title: "The School Talent Show", vi: "Hội diễn tài năng", image: L2 + "level-2-unit-03-school-talent-show.webp",
    emojis: ["🎸", "🎩", "🃏", "🎤", "✨", "🎭", "🧣", "🌟"],
    questions: [
      { id: "sh1", kind: "observe", q: "Where are the children?", vi: "Các bạn đang ở đâu?", options: ["Backstage at a talent show", "In a kitchen", "On a farm"], answer: "Backstage at a talent show" },
      { id: "sh2", kind: "observe", q: "What is the girl on the case playing?", vi: "Bạn nữ ngồi trên thùng đồ đang chơi nhạc cụ gì?", options: ["A guitar", "A drum", "A flute"], answer: "A guitar" },
      { id: "sh3", kind: "observe", q: "What is the boy in the cape holding?", vi: "Cậu bé mặc áo choàng đang cầm gì?", options: ["Playing cards", "A book", "A ball"], answer: "Playing cards" },
      { id: "sh4", kind: "locate", q: "What red hat is on the floor?", vi: "Chiếc mũ đỏ nào nằm trên sàn?", options: ["A top hat", "A cap", "A helmet"], answer: "A top hat" },
      { id: "sh5", kind: "observe", q: "What is the girl in the purple sweater holding?", vi: "Bạn nữ áo len tím đang cầm gì?", options: ["A clipboard", "A guitar", "A cake"], answer: "A clipboard" },
      { id: "sh6", kind: "locate", q: "What is inside the open prop trunk?", vi: "Trong thùng đạo cụ mở có gì?", options: ["A feather boa and a star wand", "Food", "Books"], answer: "A feather boa and a star wand" },
      { id: "sh7", kind: "infer", q: "A boy peeks through the curtain at the crowd. How does he feel?", vi: "Một cậu bé nhìn trộm qua rèm ra đám đông. Cậu thấy thế nào?", options: ["Nervous", "Sleepy", "Angry"], answer: "Nervous", explainVi: "Sắp lên biểu diễn trước đông người nên hồi hộp." },
      { id: "sh8", kind: "infer", q: "The boy wears a cape and holds cards. What act will he do?", vi: "Cậu bé mặc áo choàng và cầm bài. Cậu sẽ biểu diễn tiết mục gì?", options: ["A magic show", "A cooking show", "A running race"], answer: "A magic show", explainVi: "Áo choàng và bộ bài là đạo cụ của ảo thuật." },
      { id: "sh9", kind: "locate", q: "What can you see on the stage through the curtain?", vi: "Nhìn qua rèm ra sân khấu thấy gì?", options: ["A microphone and the audience", "A swimming pool", "Snowy mountains"], answer: "A microphone and the audience" },
      { id: "sh10", kind: "sequence", q: "The performers are getting ready backstage. What happens next?", vi: "Các bạn đang chuẩn bị sau cánh gà. Tiếp theo sẽ thế nào?", options: ["They go on stage to perform", "They go home", "They fall asleep"], answer: "They go on stage to perform", explainVi: "Chuẩn bị xong thì ra sân khấu biểu diễn." },
    ],
  },
  {
    id: "gardenmystery", title: "The Community Garden Mystery", vi: "Bí ẩn khu vườn chung", image: L2 + "level-2-unit-06-community-garden-mystery.webp",
    emojis: ["🥕", "🍅", "🔍", "🦝", "🚿", "👣", "🌿", "🏙️"],
    questions: [
      { id: "gm1", kind: "observe", q: "Where are the children?", vi: "Các bạn đang ở đâu?", options: ["In a community garden", "At a beach", "In a library"], answer: "In a community garden" },
      { id: "gm2", kind: "observe", q: "What is the girl in the striped shirt holding?", vi: "Bạn nữ áo sọc đang cầm gì?", options: ["A magnifying glass", "A camera", "A spoon"], answer: "A magnifying glass" },
      { id: "gm3", kind: "observe", q: "What has tipped over on the muddy ground?", vi: "Vật gì đổ trên nền đất lầy?", options: ["A green watering can", "A flower pot", "A bucket of paint"], answer: "A green watering can" },
      { id: "gm4", kind: "locate", q: "What animal is hiding in the compost bin?", vi: "Con vật nào đang trốn trong thùng ủ phân?", options: ["A raccoon", "A cat", "A rabbit"], answer: "A raccoon" },
      { id: "gm5", kind: "observe", q: "What small red fruit is scattered on the ground?", vi: "Loại quả đỏ nhỏ nào rơi vãi trên đất?", options: ["Cherry tomatoes", "Strawberries", "Apples"], answer: "Cherry tomatoes" },
      { id: "gm6", kind: "compare", q: "Who is holding a notebook with a drawing?", vi: "Ai đang cầm cuốn sổ có hình vẽ?", options: ["The boy in the plaid shirt", "The girl with the magnifying glass", "The boy with glasses"], answer: "The boy in the plaid shirt" },
      { id: "gm7", kind: "infer", q: "There are paw prints and a raccoon in the bin. Who made the mess?", vi: "Có dấu chân thú và một con gấu mèo trong thùng. Ai đã bày bừa?", options: ["The raccoon", "The children", "The wind"], answer: "The raccoon", explainVi: "Dấu chân và con gấu mèo trốn trong thùng → chính nó gây ra." },
      { id: "gm8", kind: "infer", q: "The girl looks at prints with a magnifying glass. What is she doing?", vi: "Bạn nữ soi dấu vết bằng kính lúp. Bạn ấy đang làm gì?", options: ["Looking for clues", "Planting seeds", "Washing her hands"], answer: "Looking for clues" },
      { id: "gm9", kind: "locate", q: "What can you see far behind the garden?", vi: "Phía xa sau khu vườn có gì?", options: ["Tall city buildings", "Snowy mountains", "The ocean"], answer: "Tall city buildings" },
      { id: "gm10", kind: "sequence", q: "They followed the tomatoes and paw prints to the bin. What did they find?", vi: "Lần theo cà chua và dấu chân tới thùng, các bạn phát hiện gì?", options: ["A raccoon caused the mess", "Nothing at all", "A hidden treasure"], answer: "A raccoon caused the mess", explainVi: "Manh mối dẫn tới con gấu mèo trong thùng ủ phân." },
    ],
  },
  {
    id: "aquarium", title: "The Aquarium at Night", vi: "Thủy cung về đêm", image: L2 + "level-2-unit-09-aquarium-night-mystery.webp",
    emojis: ["🐙", "🔦", "⚠️", "🪣", "🌊", "🕐", "🐠", "🌙"],
    questions: [
      { id: "aq1", kind: "observe", q: "Where are the children?", vi: "Các bạn đang ở đâu?", options: ["At an aquarium", "At a park", "In a bakery"], answer: "At an aquarium" },
      { id: "aq2", kind: "observe", q: "What animal is in the big tank?", vi: "Con vật nào trong bể lớn?", options: ["An octopus", "A shark", "A dolphin"], answer: "An octopus" },
      { id: "aq3", kind: "observe", q: "What is the boy with glasses holding?", vi: "Cậu bé đeo kính đang cầm gì?", options: ["A flashlight", "A sandwich", "A ball"], answer: "A flashlight" },
      { id: "aq4", kind: "locate", q: "What on the floor warns people to be careful?", vi: "Vật gì trên sàn cảnh báo mọi người cẩn thận?", options: ["A yellow caution sign", "A flower", "A toy car"], answer: "A yellow caution sign" },
      { id: "aq5", kind: "observe", q: "What is the girl in the striped shirt looking at?", vi: "Bạn nữ áo sọc đang nhìn gì?", options: ["A tablet with a diagram", "A mirror", "A cake"], answer: "A tablet with a diagram" },
      { id: "aq6", kind: "compare", q: "Who is holding a clipboard?", vi: "Ai đang cầm bảng kẹp giấy?", options: ["The girl in the teal jacket", "The boy with the flashlight", "A fish"], answer: "The girl in the teal jacket" },
      { id: "aq7", kind: "infer", q: "The floor is wet and there is a caution sign. What should they do?", vi: "Sàn ướt và có biển cảnh báo. Các bạn nên làm gì?", options: ["Be careful and tell an adult", "Run and jump on it", "Ignore it"], answer: "Be careful and tell an adult", explainVi: "Sàn ướt dễ trơn — hãy cẩn thận và báo người lớn." },
      { id: "aq8", kind: "locate", q: "What is the staff member doing in the background?", vi: "Nhân viên phía sau đang làm gì?", options: ["Putting bins on a shelf", "Swimming in the tank", "Sleeping"], answer: "Putting bins on a shelf" },
      { id: "aq9", kind: "infer", q: "The lights are low and they use a flashlight. What time is it?", vi: "Đèn tối và phải dùng đèn pin. Lúc này là khi nào?", options: ["Night, after closing", "Breakfast time", "Sunny noon"], answer: "Night, after closing", explainVi: "Tối om, phải dùng đèn pin → là buổi tối sau giờ đóng cửa." },
      { id: "aq10", kind: "sequence", q: "They found a wet floor and a control box. What will the adult do next?", vi: "Phát hiện sàn ướt và hộp điều khiển. Người lớn sẽ làm gì tiếp theo?", options: ["Check it safely to fix the problem", "Let the children touch the wires", "Leave and lock everyone in"], answer: "Check it safely to fix the problem", explainVi: "Người lớn sẽ kiểm tra an toàn để khắc phục sự cố." },
    ],
  },
  {
    id: "cookingworkshop", title: "The Cooking Workshop Mix-Up", vi: "Lớp nấu ăn bị nhầm", image: L2 + "level-2-unit-16-cooking-workshop-mix-up.webp",
    emojis: ["🧁", "🫐", "🍫", "⏲️", "🥣", "📋", "🥛", "🍓"],
    questions: [
      { id: "cw1", kind: "observe", q: "Where are the children?", vi: "Các bạn đang ở đâu?", options: ["In a kitchen workshop", "In a library", "In a garden"], answer: "In a kitchen workshop" },
      { id: "cw2", kind: "observe", q: "What are they making?", vi: "Các bạn đang làm món gì?", options: ["Muffins", "Pizza", "Soup"], answer: "Muffins" },
      { id: "cw3", kind: "compare", q: "What is in the pale batter on the left?", vi: "Trong tô bột nhạt màu bên trái có gì?", options: ["Blueberries", "Chocolate", "Carrots"], answer: "Blueberries" },
      { id: "cw4", kind: "compare", q: "What colour is the batter on the right?", vi: "Tô bột bên phải màu gì?", options: ["Brown, like chocolate", "Green", "Pink"], answer: "Brown, like chocolate" },
      { id: "cw5", kind: "observe", q: "What is in the middle of the table?", vi: "Ở giữa bàn có gì?", options: ["A kitchen timer", "A phone", "A candle"], answer: "A kitchen timer" },
      { id: "cw6", kind: "locate", q: "What do the step cards show?", vi: "Các thẻ bước chỉ điều gì?", options: ["How to make each muffin, in order", "A treasure map", "A song"], answer: "How to make each muffin, in order" },
      { id: "cw7", kind: "infer", q: "The boy looks worried and holds a recipe card. What happened?", vi: "Cậu bé lo lắng và cầm thẻ công thức. Đã xảy ra chuyện gì?", options: ["The recipe cards got mixed up", "It started to rain", "The oven flew away"], answer: "The recipe cards got mixed up", explainVi: "Vẻ mặt lo lắng và xem lại thẻ → công thức đã bị lẫn lộn." },
      { id: "cw8", kind: "observe", q: "What is used to measure liquids?", vi: "Dùng gì để đong chất lỏng?", options: ["A glass measuring cup", "A shoe", "A book"], answer: "A glass measuring cup" },
      { id: "cw9", kind: "compare", q: "Which bowl uses cocoa powder?", vi: "Tô nào dùng bột ca cao?", options: ["The chocolate batter", "The blueberry batter", "The flour bag"], answer: "The chocolate batter" },
      { id: "cw10", kind: "sequence", q: "They fixed the mix-up and filled the trays. What happens next?", vi: "Sửa xong và đổ bột vào khuôn. Tiếp theo là gì?", options: ["They bake the muffins", "They throw them away", "They eat the raw batter"], answer: "They bake the muffins", explainVi: "Đổ khuôn xong thì đem nướng bánh." },
    ],
  },
  {
    id: "wildlifecam", title: "The Wildlife Camera Surprise", vi: "Bất ngờ từ máy ảnh rừng", image: L2 + "level-2-unit-14-wildlife-camera-surprise.webp",
    emojis: ["📷", "🦝", "👣", "🏕️", "🔦", "🔭", "🍁", "🌆"],
    questions: [
      { id: "wc1", kind: "observe", q: "Where are the children?", vi: "Các bạn đang ở đâu?", options: ["At a lakeside camp", "At a supermarket", "In a classroom"], answer: "At a lakeside camp" },
      { id: "wc2", kind: "observe", q: "What are they looking at on the table?", vi: "Trên bàn các bạn đang xem gì?", options: ["Photos from a wildlife camera", "Birthday cards", "Menus"], answer: "Photos from a wildlife camera" },
      { id: "wc3", kind: "observe", q: "What is strapped to the tree?", vi: "Vật gì được buộc vào thân cây?", options: ["A trail camera", "A clock", "A birdhouse"], answer: "A trail camera" },
      { id: "wc4", kind: "compare", q: "What do the muddy photos show?", vi: "Những tấm ảnh chụp bùn cho thấy gì?", options: ["Animal paw prints", "Handprints", "Letters"], answer: "Animal paw prints" },
      { id: "wc5", kind: "locate", q: "What is glowing on the table?", vi: "Vật gì đang phát sáng trên bàn?", options: ["A lantern", "A phone", "A campfire"], answer: "A lantern" },
      { id: "wc6", kind: "observe", q: "Whose striped tail is in one photo?", vi: "Cái đuôi có sọc trong một tấm ảnh là của con nào?", options: ["A raccoon's", "A tiger's", "A zebra's"], answer: "A raccoon's" },
      { id: "wc7", kind: "infer", q: "They found paw prints, a striped tail, and a dark shape. What animal came at night?", vi: "Có dấu chân, đuôi sọc và một bóng đen. Con vật nào đã đến vào ban đêm?", options: ["A raccoon", "An elephant", "A whale"], answer: "A raccoon", explainVi: "Các manh mối đều chỉ về con gấu mèo (raccoon)." },
      { id: "wc8", kind: "locate", q: "What can you use to look far across the lake?", vi: "Dùng gì để nhìn xa qua mặt hồ?", options: ["Binoculars", "A spoon", "A pillow"], answer: "Binoculars" },
      { id: "wc9", kind: "infer", q: "The sky is orange and the lantern is on. What time is it?", vi: "Trời cam và đèn lồng đã bật. Lúc này là khi nào?", options: ["Evening", "Midday", "Early school morning"], answer: "Evening" },
      { id: "wc10", kind: "sequence", q: "The camera took photos at night. What did the photos help them do?", vi: "Máy ảnh chụp ban đêm. Những tấm ảnh giúp các bạn làm gì?", options: ["Find out which animal came", "Cook dinner", "Paint the cabin"], answer: "Find out which animal came", explainVi: "Ảnh ban đêm giúp biết con vật nào đã ghé qua." },
    ],
  },
  {
    id: "teamproject", title: "Choosing the Team Project", vi: "Chọn dự án nhóm", image: L3 + "collection-01-making-choices/level-3-c01-unit-03-choosing-team-project.webp",
    emojis: ["🌱", "🔆", "🗺️", "🐝", "💡", "🧩", "🌍", "🧑‍🏫"],
    questions: [
      { id: "tj1", kind: "observe", q: "Where are the children?", vi: "Các bạn đang ở đâu?", options: ["In a classroom", "In a swimming pool", "On a farm"], answer: "In a classroom" },
      { id: "tj2", kind: "observe", q: "What are the three project models about?", vi: "Ba mô hình dự án nói về điều gì?", options: ["A garden, solar power, and a city map", "Cakes", "Racing cars"], answer: "A garden, solar power, and a city map" },
      { id: "tj3", kind: "observe", q: "What charges the phone in one model?", vi: "Trong một mô hình, thứ gì sạc điện thoại?", options: ["A solar panel", "A candle", "A fan"], answer: "A solar panel" },
      { id: "tj4", kind: "locate", q: "What small animals are in the flower model?", vi: "Trong mô hình vườn hoa có con vật nhỏ nào?", options: ["Bees", "Fish", "Cats"], answer: "Bees" },
      { id: "tj5", kind: "observe", q: "What is the teacher holding?", vi: "Cô giáo đang cầm gì?", options: ["A tablet with a team diagram", "A guitar", "An umbrella"], answer: "A tablet with a team diagram" },
      { id: "tj6", kind: "compare", q: "Which model shows streets and a park?", vi: "Mô hình nào có đường phố và công viên?", options: ["The city map model", "The garden", "The solar panel"], answer: "The city map model" },
      { id: "tj7", kind: "infer", q: "They point at models and a chart of icons. What are they doing?", vi: "Các bạn chỉ vào mô hình và bảng biểu tượng. Họ đang làm gì?", options: ["Choosing a team project", "Cooking lunch", "Cleaning up"], answer: "Choosing a team project" },
      { id: "tj8", kind: "observe", q: "What is the girl in the denim jacket doing?", vi: "Bạn nữ áo jean đang làm gì?", options: ["Drawing icons in a notebook", "Sleeping", "Eating a snack"], answer: "Drawing icons in a notebook" },
      { id: "tj9", kind: "locate", q: "What is on the whiteboard behind them?", vi: "Trên bảng trắng phía sau có gì?", options: ["A flowchart plan", "A movie", "An open window"], answer: "A flowchart plan" },
      { id: "tj10", kind: "sequence", q: "After they compare the ideas, what will the team do?", vi: "Sau khi so sánh các ý tưởng, nhóm sẽ làm gì?", options: ["Pick one project to work on", "Go home", "Erase the board"], answer: "Pick one project to work on", explainVi: "So sánh xong thì chọn một dự án để cùng thực hiện." },
    ],
  },
  {
    id: "greenfestival", title: "The Greener School Festival", vi: "Lễ hội trường xanh", image: L3 + "collection-03-solving-problems/level-3-c03-unit-03-greener-school-festival.webp",
    emojis: ["♻️", "🌱", "💧", "🔆", "🏮", "🎪", "🔦", "🍃"],
    questions: [
      { id: "gf1", kind: "observe", q: "Where are the children?", vi: "Các bạn đang ở đâu?", options: ["At an outdoor school festival", "Inside a classroom", "At a beach"], answer: "At an outdoor school festival" },
      { id: "gf2", kind: "observe", q: "Where are the children filling their bottles?", vi: "Các bạn đổ đầy bình nước ở đâu?", options: ["At a water refill station", "In a swimming pool", "From a juice fountain"], answer: "At a water refill station" },
      { id: "gf3", kind: "observe", q: "How many sorting bins are lined up?", vi: "Có mấy thùng phân loại rác xếp thành hàng?", options: ["Two", "Three", "Five"], answer: "Three" },
      { id: "gf4", kind: "compare", q: "Which bin is for bottles and cans?", vi: "Thùng nào để chai và lon?", options: ["The blue bin", "The green bin", "The grey bin"], answer: "The blue bin" },
      { id: "gf5", kind: "locate", q: "What is the girl on the crate doing?", vi: "Bạn nữ đứng trên thùng gỗ đang làm gì?", options: ["Hanging up bunting flags", "Cooking", "Reading a book"], answer: "Hanging up bunting flags" },
      { id: "gf6", kind: "observe", q: "What is on the table with the green cloth?", vi: "Trên bàn phủ khăn xanh có gì?", options: ["A solar panel and a lantern", "A cake", "A television"], answer: "A solar panel and a lantern" },
      { id: "gf7", kind: "infer", q: "The children sort scraps into different bins. What are they trying to do?", vi: "Các bạn phân loại rác vào các thùng khác nhau. Họ đang cố làm gì?", options: ["Reduce waste at the festival", "Make noise", "Buy toys"], answer: "Reduce waste at the festival", explainVi: "Phân loại rác để giảm rác thải, giữ lễ hội xanh sạch." },
      { id: "gf8", kind: "infer", q: "They use a solar panel and a hand crank. What are these for?", vi: "Họ dùng tấm pin mặt trời và tay quay. Những thứ này để làm gì?", options: ["To make clean electricity", "To cook food", "To cool drinks"], answer: "To make clean electricity", explainVi: "Pin mặt trời và tay quay tạo ra điện sạch." },
      { id: "gf9", kind: "locate", q: "What is being set up in the background?", vi: "Phía sau đang dựng gì?", options: ["A stage", "A farm", "A car park"], answer: "A stage" },
      { id: "gf10", kind: "sequence", q: "They set up bins, water and lights. What happens next?", vi: "Đã chuẩn bị thùng rác, nước và đèn. Tiếp theo là gì?", options: ["The festival will begin", "Everyone goes home", "It starts to snow"], answer: "The festival will begin", explainVi: "Chuẩn bị xong thì lễ hội bắt đầu." },
    ],
  },
  {
    id: "recessspace", title: "Designing a Better Recess Space", vi: "Thiết kế sân chơi tốt hơn", image: L3 + "collection-03-solving-problems/level-3-c03-unit-10-designing-a-better-recess-space.webp",
    emojis: ["🏀", "🛝", "🧩", "♿", "🌳", "🎨", "📋", "🟩"],
    questions: [
      { id: "rs1", kind: "observe", q: "Where are the children?", vi: "Các bạn đang ở đâu?", options: ["In a schoolyard at recess", "In a library", "In a kitchen"], answer: "In a schoolyard at recess" },
      { id: "rs2", kind: "observe", q: "What is on the table they are gathered around?", vi: "Trên bàn các bạn vây quanh có gì?", options: ["A model of a play space", "A birthday cake", "A fish tank"], answer: "A model of a play space" },
      { id: "rs3", kind: "observe", q: "What is the girl in the purple hoodie holding?", vi: "Bạn nữ áo hoodie tím đang cầm gì?", options: ["A rating card", "A guitar", "An umbrella"], answer: "A rating card" },
      { id: "rs4", kind: "locate", q: "What is the boy in the yellow hoodie using to walk?", vi: "Cậu bé áo vàng dùng gì để đi lại?", options: ["Crutches", "A scooter", "Roller skates"], answer: "Crutches" },
      { id: "rs5", kind: "observe", q: "What is the boy with glasses placing on the ground?", vi: "Cậu bé đeo kính đang đặt gì xuống đất?", options: ["Coloured paving tiles", "Books", "Plates"], answer: "Coloured paving tiles" },
      { id: "rs6", kind: "compare", q: "What game are children playing in the background?", vi: "Phía sau các bạn đang chơi trò gì?", options: ["Basketball", "Swimming", "Chess"], answer: "Basketball" },
      { id: "rs7", kind: "infer", q: "They look at a model and a map. What are they planning?", vi: "Họ nhìn mô hình và bản đồ. Họ đang lên kế hoạch gì?", options: ["A better recess space", "A menu", "A new car"], answer: "A better recess space" },
      { id: "rs8", kind: "infer", q: "One boy uses crutches and they add smooth paths. Who are they helping?", vi: "Một bạn dùng nạng và họ làm thêm lối đi phẳng. Họ đang giúp ai?", options: ["Everyone, including kids who find it hard to move", "Only the tallest kids", "Only the teachers"], answer: "Everyone, including kids who find it hard to move", explainVi: "Lối đi phẳng giúp mọi người, kể cả bạn đi lại khó khăn." },
      { id: "rs9", kind: "locate", q: "Where are some children resting in the background?", vi: "Phía sau vài bạn đang nghỉ ở đâu?", options: ["Under a shaded pergola", "In the water", "On the roof"], answer: "Under a shaded pergola" },
      { id: "rs10", kind: "sequence", q: "They rated the ideas with the card. What will they do next?", vi: "Họ chấm điểm các ý tưởng bằng thẻ. Tiếp theo sẽ làm gì?", options: ["Choose the best design", "Erase everything", "Go to sleep"], answer: "Choose the best design", explainVi: "Chấm điểm xong thì chọn thiết kế tốt nhất." },
    ],
  },
  {
    id: "screentime", title: "Screen or Outdoor Time?", vi: "Xem màn hình hay ra ngoài?", image: L3 + "collection-01-making-choices/level-3-c01-unit-02-screen-or-outdoor-time.webp",
    emojis: ["📱", "🎧", "🚲", "🧩", "☀️", "🌳", "🏀", "🪴"],
    questions: [
      { id: "sn1", kind: "observe", q: "Where are the children?", vi: "Các bạn đang ở đâu?", options: ["In a living room", "In a cave", "On a bus"], answer: "In a living room" },
      { id: "sn2", kind: "observe", q: "What is the boy in the plaid shirt holding?", vi: "Cậu bé áo kẻ đang cầm gì?", options: ["A tablet with a game", "A frying pan", "A map"], answer: "A tablet with a game" },
      { id: "sn3", kind: "observe", q: "What is on the table next to the tablet?", vi: "Cạnh máy tính bảng trên bàn có gì?", options: ["Blue headphones", "A helmet", "A straw hat"], answer: "Blue headphones" },
      { id: "sn4", kind: "locate", q: "What can you see outside through the glass door?", vi: "Nhìn qua cửa kính ra ngoài thấy gì?", options: ["A garden with bicycles", "The ocean", "A desert"], answer: "A garden with bicycles" },
      { id: "sn5", kind: "observe", q: "What icons are at the top of the decision board?", vi: "Trên cùng bảng quyết định có biểu tượng gì?", options: ["Weather icons", "Animal icons", "Money icons"], answer: "Weather icons" },
      { id: "sn6", kind: "compare", q: "What are the children deciding between?", vi: "Các bạn đang chọn giữa hai điều gì?", options: ["Screen time and going outside", "Two kinds of soup", "Two songs"], answer: "Screen time and going outside" },
      { id: "sn7", kind: "infer", q: "The girl points outside where it is sunny. What is she suggesting?", vi: "Bạn nữ chỉ ra ngoài trời nắng. Bạn ấy gợi ý điều gì?", options: ["Going outside to play", "Watching more videos", "Going to sleep"], answer: "Going outside to play" },
      { id: "sn8", kind: "locate", q: "What are the children playing with on the table?", vi: "Trên bàn các bạn đang chơi gì?", options: ["A jigsaw puzzle", "A football", "A piano"], answer: "A jigsaw puzzle" },
      { id: "sn9", kind: "infer", q: "The weather board shows a sunny icon. What activity fits best?", vi: "Bảng thời tiết hiện biểu tượng nắng. Hoạt động nào hợp nhất?", options: ["Riding bikes outside", "Staying in bed", "Swimming in the rain"], answer: "Riding bikes outside" },
      { id: "sn10", kind: "sequence", q: "They weigh the choices on the board. What will they decide?", vi: "Họ cân nhắc các lựa chọn trên bảng. Họ sẽ quyết định gì?", options: ["To balance screen time and outdoor play", "To never go outside", "To throw the tablet away"], answer: "To balance screen time and outdoor play", explainVi: "Cân bằng giữa xem màn hình và ra ngoài vận động." },
    ],
  },
  // Learn Level 2 · Unit 07 — khám phá hộp thời gian trong vườn trường.
  {
    id: "timecapsule", title: "The Time Capsule Discovery", vi: "Khám phá hộp thời gian", image: L2 + "level-2-unit-07-time-capsule-discovery.webp",
    emojis: ["📦", "🕰️", "🗺️", "🪙", "📷", "🚗", "🌳", "🏫", "🖌️", "📓"],
    questions: [
      { id: "cap1", kind: "observe", q: "What did the children dig up?", vi: "Các bạn đã đào được thứ gì?", options: ["An old metal box", "A treasure ship", "A new computer"], answer: "An old metal box" },
      { id: "cap2", kind: "observe", q: "How many children are around the box?", vi: "Có mấy bạn nhỏ quanh chiếc hộp?", options: ["Four", "Two", "Six"], answer: "Four" },
      { id: "cap3", kind: "observe", q: "What is the girl on the left using to clean the box?", vi: "Bạn nữ bên trái dùng gì để phủi sạch hộp?", options: ["A paintbrush", "A hammer", "A spoon"], answer: "A paintbrush" },
      { id: "cap4", kind: "observe", q: "What is the boy in the plaid shirt holding?", vi: "Cậu bé áo kẻ đang cầm gì?", options: ["An old black-and-white photo", "A mobile phone", "A kite"], answer: "An old black-and-white photo" },
      { id: "cap5", kind: "compare", q: "Which child is holding two objects?", vi: "Bạn nào đang cầm hai món đồ?", options: ["The boy in the green hoodie", "The girl with the brush", "The teacher"], answer: "The boy in the green hoodie", explainVi: "Cậu bé áo hoodie xanh cầm một đồng xu và một chiếc ô tô gỗ." },
      { id: "cap6", kind: "locate", q: "Where can you see a carved tree symbol?", vi: "Có thể thấy hình cái cây khắc ở đâu?", options: ["On the wooden plank", "On the school wall", "On the backpack"], answer: "On the wooden plank" },
      { id: "cap7", kind: "compare", q: "Which object is round and made of metal?", vi: "Món nào tròn và làm bằng kim loại?", options: ["The coin", "The photo", "The map"], answer: "The coin" },
      { id: "cap8", kind: "infer", q: "An adult in glasses watches the children and smiles. Who is she most likely?", vi: "Một người lớn đeo kính quan sát và mỉm cười. Có lẽ là ai?", options: ["Their teacher", "A stranger", "A baby"], answer: "Their teacher", explainVi: "Người lớn đeo thẻ tên, đứng hướng dẫn các bạn → là cô giáo." },
      { id: "cap9", kind: "infer", q: "The photo shows people in front of the school long ago. What does the box tell us?", vi: "Tấm ảnh chụp mọi người trước trường từ lâu. Chiếc hộp cho biết điều gì?", options: ["It is from the past", "It is from the future", "It is empty"], answer: "It is from the past", explainVi: "Hộp và ảnh cũ cho thấy đây là kỷ vật từ quá khứ." },
      { id: "cap10", kind: "sequence", q: "There is a hole in the ground and the box is now open. What happened?", vi: "Trên đất có một cái hố và hộp đang mở. Chuyện gì đã xảy ra?", options: ["The children dug it up and opened it", "They just bought it", "It fell from the sky"], answer: "The children dug it up and opened it", explainVi: "Có hố trên đất → các bạn đã đào hộp lên rồi mở ra." },
    ],
  },
  // Learn Level 3 · Collection 3 (Solving Problems) · Unit 11 — sân trường ngập khi mưa.
  {
    id: "rainycourtyard", title: "The Rainy School Courtyard", vi: "Sân trường ngày mưa", image: L3 + "collection-03-solving-problems/level-3-c03-unit-11-rainy-school-courtyard.webp",
    emojis: ["🌧️", "☔", "📋", "📏", "💧", "🪑", "🧥", "🌱", "🏫", "🌊"],
    questions: [
      { id: "ryc1", kind: "observe", q: "What is the weather like?", vi: "Thời tiết thế nào?", options: ["It is raining", "It is sunny", "It is snowing"], answer: "It is raining" },
      { id: "ryc2", kind: "observe", q: "What is the girl in the purple coat holding?", vi: "Bạn nữ áo mưa tím đang cầm gì?", options: ["A measuring stick", "A frying pan", "A guitar"], answer: "A measuring stick" },
      { id: "ryc3", kind: "observe", q: "What is the boy in the blue jacket holding?", vi: "Cậu bé áo khoác xanh đang cầm gì?", options: ["A clipboard", "A kite", "A cup"], answer: "A clipboard" },
      { id: "ryc4", kind: "locate", q: "Where is the water pouring from?", vi: "Nước đang chảy ra từ đâu?", options: ["From the drainpipe on the wall", "From the bench", "From the door"], answer: "From the drainpipe on the wall" },
      { id: "ryc5", kind: "locate", q: "Where is the girl in purple crouching?", vi: "Bạn nữ áo tím đang ngồi xổm ở đâu?", options: ["By the drain near the wall", "On the bench", "Up in a tree"], answer: "By the drain near the wall" },
      { id: "ryc6", kind: "observe", q: "What are the two children in the background holding over the bench?", vi: "Hai bạn phía sau đang căng gì phía trên ghế?", options: ["A big blue sheet", "A flag", "A blanket of leaves"], answer: "A big blue sheet" },
      { id: "ryc7", kind: "compare", q: "Who is wearing a yellow raincoat?", vi: "Ai đang mặc áo mưa vàng?", options: ["The girl by the bench", "The boy with the clipboard", "The girl measuring the drain"], answer: "The girl by the bench" },
      { id: "ryc8", kind: "infer", q: "The children measure the water at the drain. What problem are they solving?", vi: "Các bạn đo nước ở miệng cống. Họ đang giải quyết vấn đề gì?", options: ["The courtyard floods when it rains", "The garden has no flowers", "The lights are broken"], answer: "The courtyard floods when it rains", explainVi: "Đo nước ở cống → tìm cách xử lý việc sân bị ngập khi mưa." },
      { id: "ryc9", kind: "infer", q: "Why are two children covering the bench with a sheet?", vi: "Vì sao hai bạn che ghế bằng tấm bạt?", options: ["To keep it dry in the rain", "To paint it", "To hide it"], answer: "To keep it dry in the rain", explainVi: "Căng tấm bạt để ghế không bị ướt mưa." },
      { id: "ryc10", kind: "sequence", q: "The drain is full and water keeps pouring. What should they do next?", vi: "Cống đã đầy và nước cứ chảy tràn. Tiếp theo nên làm gì?", options: ["Find a way to drain the water away", "Add more water", "Do nothing"], answer: "Find a way to drain the water away", explainVi: "Cống đầy, nước tràn → cần tìm cách cho nước thoát đi." },
    ],
  },
  {
    id: "camping", title: "A Stormy Camping Trip", vi: "Chuyến cắm trại trong cơn bão", image: "/assets/images/learn/level-2/level-2-unit-02-stormy-camping-trip.webp",
    emojis: ["⛺", "🌧️", "🔦", "🗺️", "🎒", "🌲", "⛰️", "☕", "⚡", "🌙"],
    questions: [
      { id: "cmp1", kind: "observe", q: "What is the weather like in the picture?", vi: "Thời tiết trong bức tranh như thế nào?", options: ["It is raining", "It is sunny", "It is snowing"], answer: "It is raining" },
      { id: "cmp2", kind: "observe", q: "What is the boy in the orange jacket holding?", vi: "Cậu bé mặc áo khoác màu cam đang cầm gì?", options: ["A map", "A book", "An umbrella"], answer: "A map" },
      { id: "cmp3", kind: "observe", q: "What are the children setting up?", vi: "Các bạn nhỏ đang dựng cái gì?", options: ["A tent", "A house", "A car"], answer: "A tent" },
      { id: "cmp4", kind: "observe", q: "What is the boy with glasses holding?", vi: "Cậu bé đeo kính đang cầm gì?", options: ["A flashlight", "A phone", "A ball"], answer: "A flashlight" },
      { id: "cmp5", kind: "locate", q: "Where is the blue mug?", vi: "Cái cốc màu xanh ở đâu?", options: ["On the ground", "On the table", "In the tent"], answer: "On the ground" },
      { id: "cmp6", kind: "locate", q: "Where is the wooden shelter?", vi: "Nhà chòi gỗ nằm ở đâu?", options: ["On the right side", "On the left side", "In the tent"], answer: "On the right side" },
      { id: "cmp7", kind: "compare", q: "Who is standing up, not crouching?", vi: "Ai là người đang đứng, không phải ngồi xổm?", options: ["The boy in the orange jacket", "The girl in the teal jacket", "The boy with glasses"], answer: "The boy in the orange jacket" },
      { id: "cmp8", kind: "compare", q: "How many children are in the picture?", vi: "Có bao nhiêu bạn nhỏ trong tranh?", options: ["Four", "Two", "Six"], answer: "Four" },
      { id: "cmp9", kind: "infer", q: "Why does the wooden shelter look like a good place to go?", vi: "Vì sao nhà chòi gỗ trông là nơi tốt để đi tới?", options: ["It has a roof and a light, so it is dry", "It is full of food", "It is a swimming pool"], answer: "It has a roof and a light, so it is dry", explainVi: "Trời đang mưa, nhà chòi có mái che và có đèn sáng nên đó là chỗ khô ráo để trú mưa." },
      { id: "cmp10", kind: "sequence", q: "It is raining hard. What should the children do next?", vi: "Trời mưa to. Các bạn nên làm gì tiếp theo?", options: ["Finish and go to the dry shelter", "Take a nap on the wet ground", "Jump into the lake"], answer: "Finish and go to the dry shelter", explainVi: "Vì trời mưa to, sau khi dựng xong đồ các bạn nên đến nhà chòi khô ráo để tránh bị ướt và lạnh." },
    ],
  },
  {
    id: "lunchroom", title: "Making a Quieter Lunchroom", vi: "Làm cho phòng ăn yên tĩnh hơn", image: "/assets/images/learn/level-3/collection-03-solving-problems/level-3-c03-unit-01-quieter-lunchroom.webp",
    emojis: ["🍽️", "🪑", "🔇", "🟩", "🟨", "🧑‍🏫", "🎒", "🪟", "🪴", "🔊"],
    questions: [
      { id: "lnr1", kind: "observe", q: "Where does this scene take place?", vi: "Cảnh này diễn ra ở đâu?", options: ["In a school lunchroom", "In a swimming pool", "In a library"], answer: "In a school lunchroom" },
      { id: "lnr2", kind: "observe", q: "What shape are the soft panels the children put on the board?", vi: "Các tấm mềm mà các bạn gắn lên bảng có hình gì?", options: ["Hexagons", "Circles", "Stars"], answer: "Hexagons" },
      { id: "lnr3", kind: "observe", q: "What is the boy in the green hoodie putting on the chair legs?", vi: "Cậu bé mặc áo hoodie xanh lá đang gắn gì vào chân ghế?", options: ["Soft felt pads", "Small wheels", "Paper cups"], answer: "Soft felt pads" },
      { id: "lnr4", kind: "observe", q: "What is the girl in the striped shirt holding?", vi: "Bạn gái mặc áo sọc đang cầm gì?", options: ["A colorful sound meter", "A water bottle", "A book"], answer: "A colorful sound meter" },
      { id: "lnr5", kind: "locate", q: "Where is the woman in the teal cardigan?", vi: "Người phụ nữ mặc áo len xanh ngọc ở đâu?", options: ["On the right side, holding a chair", "On the left, on the board", "Sitting at a table"], answer: "On the right side, holding a chair" },
      { id: "lnr6", kind: "locate", q: "Where are the extra felt pads kept?", vi: "Những miếng đệm dự phòng được để ở đâu?", options: ["In a blue bin on the floor", "In a backpack", "On the ceiling"], answer: "In a blue bin on the floor" },
      { id: "lnr7", kind: "compare", q: "What are the children in the foreground doing that the students at the tables are not?", vi: "Các bạn ở phía trước đang làm gì mà các bạn ngồi bàn thì không?", options: ["Fixing things to make the room quieter", "Eating lunch", "Sleeping"], answer: "Fixing things to make the room quieter" },
      { id: "lnr8", kind: "compare", q: "How many chairs is the woman working with near the plants?", vi: "Người phụ nữ đang xử lý bao nhiêu chiếc ghế gần chậu cây?", options: ["Two blue chairs", "One red chair", "Five chairs"], answer: "Two blue chairs" },
      { id: "lnr9", kind: "infer", q: "Why are the children adding soft panels and felt pads?", vi: "Vì sao các bạn gắn thêm tấm mềm và miếng đệm?", options: ["To make the lunchroom less noisy", "To make the room look messy", "To keep the room warm"], answer: "To make the lunchroom less noisy", explainVi: "Tấm mềm và miếng đệm dưới chân ghế giúp hút và giảm tiếng ồn, làm phòng ăn yên tĩnh hơn." },
      { id: "lnr10", kind: "sequence", q: "After they finish adding the soft materials, what could they do to check their work?", vi: "Sau khi gắn xong vật liệu mềm, các bạn có thể làm gì để kiểm tra?", options: ["Use the sound meter to measure the noise", "Throw away the bin", "Turn off the lights"], answer: "Use the sound meter to measure the noise", explainVi: "Sau khi lắp xong, các bạn có thể dùng máy đo âm thanh để xem phòng đã yên tĩnh hơn chưa." },
    ],
  },
  {
    id: "ferrytrip", title: "The Ferry Trip Mix-Up", vi: "Chuyến phà bị nhầm lẫn", image: "/assets/images/learn/level-2/level-2-unit-04-ferry-trip-mix-up.webp",
    emojis: ["⛴️", "🚢", "🎫", "🕓", "🧳", "🗺️", "🌊", "🏔️", "🏙️", "👦"],
    questions: [
      { id: "fry1", kind: "observe", q: "What are the children waiting to ride?", vi: "Bọn trẻ đang chờ để đi phương tiện gì?", options: ["A ferry", "A train", "A bus"], answer: "A ferry" },
      { id: "fry2", kind: "observe", q: "How many big ferries are on the water?", vi: "Có bao nhiêu chiếc phà lớn trên mặt nước?", options: ["Two", "One", "Four"], answer: "Two" },
      { id: "fry3", kind: "observe", q: "What is loaded on the metal luggage cart?", vi: "Trên xe đẩy hành lý bằng kim loại có gì?", options: ["A big suitcase", "A bicycle", "A dog"], answer: "A big suitcase" },
      { id: "fry4", kind: "observe", q: "What colors are the two tickets the boy on the left is holding?", vi: "Hai chiếc vé mà cậu bé bên trái đang cầm có màu gì?", options: ["One blue and one red", "Both green", "Both yellow"], answer: "One blue and one red" },
      { id: "fry5", kind: "locate", q: "Where is the round clock?", vi: "Chiếc đồng hồ tròn nằm ở đâu?", options: ["At the top, near the sign board", "On the ground", "On the suitcase"], answer: "At the top, near the sign board" },
      { id: "fry6", kind: "locate", q: "Who is holding the paper map?", vi: "Ai là người đang cầm tấm bản đồ giấy?", options: ["The boy with glasses", "The girl in the striped shirt", "The woman in the cardigan"], answer: "The boy with glasses" },
      { id: "fry7", kind: "compare", q: "Which is bigger, the suitcase or a ticket?", vi: "Cái nào lớn hơn, vali hay tấm vé?", options: ["The suitcase is bigger", "The ticket is bigger", "They are the same size"], answer: "The suitcase is bigger" },
      { id: "fry8", kind: "compare", q: "Who is taller, the woman or the children?", vi: "Ai cao hơn, người phụ nữ hay bọn trẻ?", options: ["The woman is taller", "The children are taller", "They are the same height"], answer: "The woman is taller" },
      { id: "fry9", kind: "infer", q: "Why are they checking the tickets, the map, and the sign board so carefully?", vi: "Tại sao họ lại kiểm tra vé, bản đồ và bảng thông tin kỹ đến vậy?", options: ["They are not sure which ferry is theirs", "They want to buy some ice cream", "They are looking for a lost dog"], answer: "They are not sure which ferry is theirs", explainVi: "Cậu bé cầm một vé xanh và một vé đỏ, mà lại có hai chiếc phà (xanh và đỏ), nên cả nhóm đang bối rối không biết đâu là chuyến phà đúng của mình." },
      { id: "fry10", kind: "sequence", q: "What should they do first, before stepping onto a ferry?", vi: "Họ nên làm gì trước tiên, trước khi bước lên phà?", options: ["Check the ticket and the sign board to find the right ferry", "Run onto any ferry quickly", "Jump into the water and swim"], answer: "Check the ticket and the sign board to find the right ferry", explainVi: "Trước khi lên phà, họ cần xem vé và bảng thông tin để chắc chắn chọn đúng chuyến, rồi mới bước lên đúng chiếc phà của mình." },
    ],
  },
  {
    id: "bikeroute", title: "Planning a Safer Bike Route", vi: "Lên kế hoạch cho tuyến đường xe đạp an toàn hơn", image: "/assets/images/learn/level-3/collection-03-solving-problems/level-3-c03-unit-02-safer-bike-route.webp",
    emojis: ["🚲", "⛑️", "📏", "🚦", "🗺️", "📱", "🚧", "🌳", "🏙️", "💧"],
    questions: [
      { id: "bkr1", kind: "observe", q: "What are the children wearing on their heads?", vi: "Bọn trẻ đang đội gì trên đầu?", options: ["Helmets", "Caps", "Crowns"], answer: "Helmets" },
      { id: "bkr2", kind: "observe", q: "What are the two children on the left stretching out on the ground?", vi: "Hai bạn nhỏ bên trái đang kéo căng vật gì trên mặt đất?", options: ["A measuring tape", "A jump rope", "A garden hose"], answer: "A measuring tape" },
      { id: "bkr3", kind: "observe", q: "What is the boy in the green hoodie holding up?", vi: "Cậu bé mặc áo hoodie xanh lá đang giơ lên vật gì?", options: ["A tablet", "A book", "A pizza"], answer: "A tablet" },
      { id: "bkr4", kind: "observe", q: "What orange-and-white safety objects stand on the sidewalk?", vi: "Những vật màu cam trắng dùng để cảnh báo trên vỉa hè là gì?", options: ["Traffic cones", "Flower pots", "Trash cans"], answer: "Traffic cones" },
      { id: "bkr5", kind: "locate", q: "Where are the bicycles?", vi: "Những chiếc xe đạp đang ở đâu?", options: ["On the right side, near the woman", "Floating on a river", "Up in a tree"], answer: "On the right side, near the woman" },
      { id: "bkr6", kind: "locate", q: "What picture is lit on the pedestrian traffic light?", vi: "Đèn tín hiệu cho người đi bộ đang hiện hình gì?", options: ["A white walking person", "A red hand", "A green car"], answer: "A white walking person" },
      { id: "bkr7", kind: "compare", q: "Are there more children or more adults in the picture?", vi: "Trong bức tranh có nhiều trẻ em hơn hay nhiều người lớn hơn?", options: ["More children", "More adults", "The same number"], answer: "More children" },
      { id: "bkr8", kind: "compare", q: "Do all the children wear the same color helmet?", vi: "Tất cả bọn trẻ có đội mũ bảo hiểm cùng một màu không?", options: ["No, the helmets are different colors", "Yes, all of them are red", "Yes, all of them are purple"], answer: "No, the helmets are different colors" },
      { id: "bkr9", kind: "infer", q: "Why are the children measuring the lane and taking photos?", vi: "Tại sao bọn trẻ lại đo làn đường và chụp ảnh?", options: ["They are studying the road to plan a safer bike route", "They are playing a game with a rope", "They are cleaning the street"], answer: "They are studying the road to plan a safer bike route", explainVi: "Các bạn dùng thước đo làn đường, chụp ảnh bằng máy tính bảng và xem bản đồ, cho thấy các bạn đang khảo sát con đường để lập kế hoạch cho một tuyến đường xe đạp an toàn hơn." },
      { id: "bkr10", kind: "sequence", q: "To make their map, what do they do first?", vi: "Để hoàn thành tấm bản đồ, đầu tiên các bạn làm gì?", options: ["First measure and photograph the lane, then mark it on the map", "First ride home, then eat lunch", "First close their eyes, then sleep"], answer: "First measure and photograph the lane, then mark it on the map", explainVi: "Đầu tiên các bạn phải thu thập thông tin bằng cách đo và chụp ảnh làn đường, sau đó mới ghi lại và đánh dấu lên bản đồ để hoàn thành kế hoạch." },
    ],
  },
  {
    id: "robotstart", title: "The Robot That Wouldn't Start", vi: "Chú robot không chịu khởi động", image: "/assets/images/learn/level-2/level-2-unit-05-robot-wouldnt-start.webp",
    emojis: ["🤖", "🔧", "🔩", "🔋", "⚙️", "⏱️", "🛞", "👓", "📋", "🔵"],
    questions: [
      { id: "rob1", kind: "observe", q: "What are the children gathered around on the floor?", vi: "Bọn trẻ đang tụ tập quanh vật gì trên sàn?", options: ["A small robot", "A birthday cake", "A sleeping cat"], answer: "A small robot" },
      { id: "rob2", kind: "observe", q: "What color are the robot's glowing eyes?", vi: "Đôi mắt phát sáng của robot có màu gì?", options: ["Blue", "Red", "Green"], answer: "Blue" },
      { id: "rob3", kind: "observe", q: "What is the woman standing at the back doing with her arms?", vi: "Người phụ nữ đứng phía sau đang làm gì với hai cánh tay?", options: ["Crossing them", "Waving them", "Clapping them"], answer: "Crossing them" },
      { id: "rob4", kind: "observe", q: "What is hanging on the wall behind the children?", vi: "Vật gì đang treo trên tường phía sau bọn trẻ?", options: ["Tools like wrenches", "Photos of animals", "Winter coats"], answer: "Tools like wrenches" },
      { id: "rob5", kind: "locate", q: "Where is the boy who is working alone at another workbench?", vi: "Cậu bé đang làm việc một mình ở bàn khác nằm ở đâu?", options: ["On the left side", "In the middle", "On the bottom right"], answer: "On the left side" },
      { id: "rob6", kind: "locate", q: "Where is the close-up circle showing a single wheel?", vi: "Vòng tròn phóng to cho thấy một bánh xe nằm ở đâu?", options: ["Bottom right corner", "Top left corner", "Top right corner"], answer: "Bottom right corner" },
      { id: "rob7", kind: "compare", q: "Who is holding a paper with a diagram on it?", vi: "Ai đang cầm tờ giấy có bản vẽ sơ đồ?", options: ["The boy in the plaid shirt", "The girl in the teal jacket", "The woman at the back"], answer: "The boy in the plaid shirt" },
      { id: "rob8", kind: "compare", q: "Which child is wearing round glasses?", vi: "Bạn nhỏ nào đang đeo kính tròn?", options: ["The boy in the green jacket", "The girl in the striped shirt", "The boy in the plaid shirt"], answer: "The boy in the green jacket" },
      { id: "rob9", kind: "infer", q: "Why is the girl in the teal jacket holding a stopwatch?", vi: "Vì sao cô bé mặc áo khoác xanh ngọc lại cầm đồng hồ bấm giờ?", options: ["To time how the robot works", "To listen to music", "To take a photo"], answer: "To time how the robot works", explainVi: "Cô bé cầm đồng hồ bấm giờ, cho thấy nhóm đang thử nghiệm và đo thời gian hoạt động của robot." },
      { id: "rob10", kind: "sequence", q: "The girl points a screwdriver at the robot. What are they most likely doing first?", vi: "Cô bé chĩa tua vít vào robot. Nhiều khả năng họ đang làm gì trước tiên?", options: ["Trying to fix the robot", "Cleaning up to go home", "Painting a new picture"], answer: "Trying to fix the robot", explainVi: "Robot không khởi động nên bước đầu tiên là dùng tua vít và sơ đồ để kiểm tra, sửa chữa chú robot." },
    ],
  },
  {
    id: "foodwaste", title: "Reducing Food Waste", vi: "Giảm lãng phí thức ăn", image: "/assets/images/learn/level-3/collection-03-solving-problems/level-3-c03-unit-05-reducing-food-waste.webp",
    emojis: ["🍎", "🍌", "🗑️", "♻️", "🥛", "🧤", "🌱", "🍽️", "🏫", "📋"],
    questions: [
      { id: "fwz1", kind: "observe", q: "Where does this scene take place?", vi: "Cảnh này diễn ra ở đâu?", options: ["In a school cafeteria", "On a soccer field", "At a swimming pool"], answer: "In a school cafeteria" },
      { id: "fwz2", kind: "observe", q: "How many sorting bins are lined up in the front?", vi: "Có bao nhiêu thùng phân loại xếp ở phía trước?", options: ["Three", "Two", "Five"], answer: "Three" },
      { id: "fwz3", kind: "observe", q: "What is the boy in the green hoodie dropping into a bin?", vi: "Cậu bé mặc áo hoodie xanh lá đang thả gì vào thùng?", options: ["A banana peel", "A soccer ball", "A pencil"], answer: "A banana peel" },
      { id: "fwz4", kind: "observe", q: "What color gloves are several of the children wearing?", vi: "Mấy bạn nhỏ đang đeo găng tay màu gì?", options: ["Blue", "Pink", "Yellow"], answer: "Blue" },
      { id: "fwz5", kind: "locate", q: "Where is the boy sitting in a wheelchair?", vi: "Cậu bé ngồi trên xe lăn ở đâu?", options: ["On the left side", "On the right side", "In the middle"], answer: "On the left side" },
      { id: "fwz6", kind: "locate", q: "Where is the adult wearing a green cap and apron?", vi: "Người lớn đội mũ lưỡi trai và mặc tạp dề xanh ở đâu?", options: ["On the right side", "On the left side", "Lying on the floor"], answer: "On the right side" },
      { id: "fwz7", kind: "compare", q: "Which bin has the apple core and banana peel picture on it?", vi: "Thùng nào có hình lõi táo và vỏ chuối trên đó?", options: ["The green bin", "The blue bin", "The black bin"], answer: "The green bin" },
      { id: "fwz8", kind: "compare", q: "What is the boy in the wheelchair holding that the others are not?", vi: "Cậu bé trên xe lăn đang cầm thứ gì mà các bạn khác không có?", options: ["A clipboard", "A milk carton", "A scoop"], answer: "A clipboard" },
      { id: "fwz9", kind: "infer", q: "Why are there different colored bins with pictures on them?", vi: "Vì sao lại có nhiều thùng khác màu với hình vẽ trên đó?", options: ["To help kids sort waste correctly", "To store toys for playtime", "To keep drinks cold"], answer: "To help kids sort waste correctly", explainVi: "Mỗi thùng có màu và hình riêng cho hộp sữa, thức ăn thừa hay rác, giúp các bạn bỏ đúng loại rác." },
      { id: "fwz10", kind: "sequence", q: "A girl is holding a tray of food. What does she most likely do next?", vi: "Một cô bé đang bưng khay thức ăn. Nhiều khả năng cô bé sẽ làm gì tiếp theo?", options: ["Sort the leftovers into a bin", "Throw the tray out the window", "Go to sleep on the floor"], answer: "Sort the leftovers into a bin", explainVi: "Cô bé cầm khay có thức ăn thừa, nên bước tiếp theo là phân loại và bỏ chúng vào đúng thùng như các bạn." },
    ],
  },
  {
    id: "beachcleanup", title: "Beach Cleanup: A Change of Plan", vi: "Dọn dẹp bãi biển: Kế hoạch thay đổi", image: "/assets/images/learn/level-2/level-2-unit-08-beach-cleanup-change-of-plan.webp",
    emojis: ["🏖️", "🌊", "🪣", "🧤", "🚩", "🐦", "🗺️", "🧹", "♻️", "🌲"],
    questions: [
      { id: "bcu1", kind: "observe", q: "Where are the children standing?", vi: "Các bạn nhỏ đang đứng ở đâu?", options: ["On a rocky beach", "In a swimming pool", "Inside a classroom"], answer: "On a rocky beach" },
      { id: "bcu2", kind: "observe", q: "What is the boy on the left holding?", vi: "Bạn trai bên trái đang cầm gì?", options: ["A blue clipboard", "A soccer ball", "An umbrella"], answer: "A blue clipboard" },
      { id: "bcu3", kind: "observe", q: "What is the girl with long hair holding?", vi: "Bạn gái tóc dài đang cầm gì?", options: ["A green mesh bag", "A red kite", "A basketball"], answer: "A green mesh bag" },
      { id: "bcu4", kind: "observe", q: "What is inside the blue bins on the right?", vi: "Bên trong những thùng màu xanh bên phải có gì?", options: ["Gloves and litter pickers", "Books and pencils", "Fruit and drinks"], answer: "Gloves and litter pickers" },
      { id: "bcu5", kind: "locate", q: "Where are the seagulls?", vi: "Những con mòng biển ở đâu?", options: ["Flying in the sky", "Sitting on the buckets", "Swimming in the water"], answer: "Flying in the sky" },
      { id: "bcu6", kind: "locate", q: "Where is the map?", vi: "Tấm bản đồ ở đâu?", options: ["On a wooden sign at the bottom left", "On the picnic table", "In the girl's hand"], answer: "On a wooden sign at the bottom left" },
      { id: "bcu7", kind: "compare", q: "How many flags are on the tall pole?", vi: "Có bao nhiêu lá cờ trên cây cột cao?", options: ["Two flags", "Five flags", "No flags"], answer: "Two flags" },
      { id: "bcu8", kind: "compare", q: "What color are the stacked buckets?", vi: "Những chiếc xô xếp chồng lên nhau có màu gì?", options: ["Blue", "Pink", "Orange"], answer: "Blue" },
      { id: "bcu9", kind: "infer", q: "Why do the children have gloves and litter pickers?", vi: "Vì sao các bạn nhỏ có găng tay và kẹp gắp rác?", options: ["To pick up trash safely", "To go fishing", "To build a sandcastle"], answer: "To pick up trash safely", explainVi: "Găng tay và kẹp gắp giúp các bạn nhặt rác trên bãi biển mà không bị bẩn hay bị thương tay." },
      { id: "bcu10", kind: "sequence", q: "What should they do first before picking up litter?", vi: "Các bạn nên làm gì đầu tiên trước khi nhặt rác?", options: ["Put on the gloves", "Throw the buckets away", "Go swimming in the sea"], answer: "Put on the gloves", explainVi: "Trước tiên phải đeo găng tay để bảo vệ đôi tay, rồi mới dùng kẹp gắp để nhặt rác cho vào xô." },
    ],
  },
  {
    id: "hotclassroom", title: "Cooling a Hot Classroom", vi: "Làm mát lớp học nóng bức", image: "/assets/images/learn/level-3/collection-03-solving-problems/level-3-c03-unit-06-cooling-a-hot-classroom.webp",
    emojis: ["🏫", "🌀", "☀️", "🌳", "📋", "✏️", "📄", "🪴", "🌍", "📊"],
    questions: [
      { id: "hcl1", kind: "observe", q: "What are the children gathered around on the table?", vi: "Các bạn nhỏ đang tụ tập quanh vật gì trên bàn?", options: ["A cardboard model of a building", "A birthday cake", "A television"], answer: "A cardboard model of a building" },
      { id: "hcl2", kind: "observe", q: "What machine is on the table blowing air?", vi: "Chiếc máy nào trên bàn đang thổi gió?", options: ["An electric fan", "A microwave", "A washing machine"], answer: "An electric fan" },
      { id: "hcl3", kind: "observe", q: "What is the teacher holding up on the right?", vi: "Cô giáo bên phải đang giơ vật gì lên?", options: ["A board with symbols", "A big pizza", "A soccer trophy"], answer: "A board with symbols" },
      { id: "hcl4", kind: "observe", q: "What is the girl in the purple hoodie doing?", vi: "Bạn gái mặc áo hoodie tím đang làm gì?", options: ["Writing with a pencil", "Eating an apple", "Sleeping on the desk"], answer: "Writing with a pencil" },
      { id: "hcl5", kind: "locate", q: "Where is the fan?", vi: "Chiếc quạt ở đâu?", options: ["On the table near the model", "On the ceiling", "Outside the window"], answer: "On the table near the model" },
      { id: "hcl6", kind: "locate", q: "Where are the small potted plants?", vi: "Những chậu cây nhỏ ở đâu?", options: ["On the windowsill", "Inside the fan", "Under the table"], answer: "On the windowsill" },
      { id: "hcl7", kind: "compare", q: "How many children are around the table?", vi: "Có bao nhiêu bạn nhỏ đứng quanh bàn?", options: ["Four children", "One child", "Ten children"], answer: "Four children" },
      { id: "hcl8", kind: "infer", q: "Why are there a fan and an open window?", vi: "Vì sao lại có chiếc quạt và cửa sổ mở?", options: ["To cool down the hot classroom", "To make the room warmer", "To water the plants"], answer: "To cool down the hot classroom", explainVi: "Tên bài là làm mát lớp học nóng bức; quạt và cửa sổ mở giúp không khí mát lưu thông vào phòng." },
      { id: "hcl9", kind: "infer", q: "Why are the papers floating in the air?", vi: "Vì sao những tờ giấy lại bay lơ lửng trong không khí?", options: ["The fan is blowing them", "They are made of birds", "Someone is throwing water"], answer: "The fan is blowing them", explainVi: "Chiếc quạt và gió từ cửa sổ thổi mạnh làm những tờ giấy nhẹ bay lên trong phòng." },
      { id: "hcl10", kind: "sequence", q: "What did the children likely do first for their project?", vi: "Các bạn nhỏ có lẽ đã làm gì đầu tiên cho dự án của mình?", options: ["Plan their ideas on paper", "Turn off all the lights", "Leave the classroom"], answer: "Plan their ideas on paper", explainVi: "Trước tiên các bạn ghi và vẽ ý tưởng ra giấy và bảng, rồi mới làm mô hình và thử với quạt." },
    ],
  },
  {
    id: "sciencefairmix", title: "The Science Fair Mix-Up", vi: "Hội chợ khoa học rối tung", image: "/assets/images/learn/level-2/level-2-unit-11-science-fair-mix-up.webp",
    emojis: ["🔬", "🌬️", "💧", "🔋", "🌱", "💡", "📷", "📊", "🔌", "🧪"],
    questions: [
      { id: "sfm1", kind: "observe", q: "How many children are around the table?", vi: "Có bao nhiêu bạn nhỏ đứng quanh bàn?", options: ["Four", "Two", "Six"], answer: "Four" },
      { id: "sfm2", kind: "observe", q: "What color is the wind turbine model?", vi: "Mô hình tua-bin gió có màu gì?", options: ["White", "Red", "Green"], answer: "White" },
      { id: "sfm3", kind: "observe", q: "What is the girl in the middle holding up?", vi: "Bạn gái ở giữa đang giơ lên thứ gì?", options: ["A photo of a waterfall", "A big book", "A gold trophy"], answer: "A photo of a waterfall" },
      { id: "sfm4", kind: "observe", q: "What is the woman on the right wearing around her neck?", vi: "Người phụ nữ bên phải đang đeo gì quanh cổ?", options: ["An ID badge", "A scarf", "Headphones"], answer: "An ID badge" },
      { id: "sfm5", kind: "locate", q: "Where is the wind turbine model?", vi: "Mô hình tua-bin gió ở đâu?", options: ["On the left of the table", "On the right of the table", "Hanging on the wall"], answer: "On the left of the table" },
      { id: "sfm6", kind: "locate", q: "Where is the tall water filter with glowing blue water?", vi: "Máy lọc nước cao với nước xanh phát sáng ở đâu?", options: ["On the right side of the table", "On the floor", "Outside the window"], answer: "On the right side of the table" },
      { id: "sfm7", kind: "compare", q: "Who is the tallest person at the table?", vi: "Ai là người cao nhất ở bàn?", options: ["The woman", "The boy with glasses", "The girl in the yellow shirt"], answer: "The woman" },
      { id: "sfm8", kind: "compare", q: "Are there more children or more adults at the table?", vi: "Ở bàn có nhiều trẻ em hơn hay nhiều người lớn hơn?", options: ["More children", "More adults", "The same number"], answer: "More children" },
      { id: "sfm9", kind: "infer", q: "What kind of event is happening here?", vi: "Đây là sự kiện gì?", options: ["A science fair", "A birthday party", "A soccer match"], answer: "A science fair", explainVi: "Có nhiều bảng trưng bày và các dự án khoa học như tua-bin gió, máy lọc nước, nên đây là hội chợ khoa học." },
      { id: "sfm10", kind: "sequence", q: "In the water filter, where does the water go first?", vi: "Trong máy lọc nước, nước đi vào đâu trước tiên?", options: ["Into the top with sand and gravel", "Out the bottom", "Into the plants"], answer: "Into the top with sand and gravel", explainVi: "Nước bẩn được đổ vào phần trên có cát và sỏi trước, rồi mới chảy xuống dưới và trở nên sạch hơn." },
    ],
  },
  {
    id: "pickupzone", title: "Improving the School Pickup Zone", vi: "Cải thiện khu vực đón học sinh", image: "/assets/images/learn/level-3/collection-03-solving-problems/level-3-c03-unit-07-improving-school-pickup-zone.webp",
    emojis: ["🚸", "🚌", "🚗", "🚦", "🚲", "🗺️", "📋", "🦺", "🚧", "🏫"],
    questions: [
      { id: "pzn1", kind: "observe", q: "What are the children gathered around?", vi: "Các bạn nhỏ đang tụ tập quanh cái gì?", options: ["A wooden table", "A swimming pool", "A car"], answer: "A wooden table" },
      { id: "pzn2", kind: "observe", q: "What color is the vest the adult is wearing?", vi: "Người lớn đang mặc áo phản quang màu gì?", options: ["Yellow", "Red", "Blue"], answer: "Yellow" },
      { id: "pzn3", kind: "observe", q: "What is on the adult's head?", vi: "Trên đầu người lớn có gì?", options: ["A green cap", "A helmet", "A crown"], answer: "A green cap" },
      { id: "pzn4", kind: "observe", q: "What yellow toy vehicle is on the model street?", vi: "Trên mô hình đường phố có xe đồ chơi màu vàng nào?", options: ["A school bus", "A fire truck", "A boat"], answer: "A school bus" },
      { id: "pzn5", kind: "observe", q: "What is the girl with the purple headband holding?", vi: "Bạn gái đội băng đô tím đang cầm gì?", options: ["A clipboard", "A camera", "A ball"], answer: "A clipboard" },
      { id: "pzn6", kind: "locate", q: "Where is the real yellow school bus?", vi: "Chiếc xe buýt trường học màu vàng thật ở đâu?", options: ["In the background on the street", "On the table", "Inside the building"], answer: "In the background on the street" },
      { id: "pzn7", kind: "locate", q: "Where are the bikes parked?", vi: "Những chiếc xe đạp được đậu ở đâu?", options: ["At the bike racks near the building", "On the table", "On the roof"], answer: "At the bike racks near the building" },
      { id: "pzn8", kind: "compare", q: "On the model, are there more toy cars or more toy buses?", vi: "Trên mô hình có nhiều xe hơi đồ chơi hơn hay nhiều xe buýt hơn?", options: ["More cars", "More buses", "The same number"], answer: "More cars" },
      { id: "pzn9", kind: "infer", q: "What job does the adult in the vest most likely do?", vi: "Người lớn mặc áo phản quang nhiều khả năng làm nghề gì?", options: ["A crossing guard", "A chef", "A pilot"], answer: "A crossing guard", explainVi: "Người này mặc áo phản quang màu vàng và cầm bảng kẹp giấy gần đường, giống chú bảo vệ giúp học sinh sang đường." },
      { id: "pzn10", kind: "sequence", q: "At the crosswalk, what should happen first so people can cross safely?", vi: "Ở vạch sang đường, điều gì nên xảy ra trước để mọi người qua đường an toàn?", options: ["The cars stop, then people cross", "People cross, then cars stop", "Cars and people go at the same time"], answer: "The cars stop, then people cross", explainVi: "Xe phải dừng lại trước, sau đó người đi bộ mới bước qua vạch sang đường để được an toàn." },
    ],
  },
  {
    id: "fieldtripdetour", title: "Field Trip Detour", vi: "Chuyến đi thực tế đổi lộ trình", image: "/assets/images/learn/level-2/level-2-unit-12-field-trip-detour.webp",
    emojis: ["🚌", "🗺️", "🕓", "🌧️", "🎒", "🚢", "🏙️", "⛰️", "🚧", "🧭"],
    questions: [
      { id: "ftd1", kind: "observe", q: "What are the children looking at on the table?", vi: "Bọn trẻ đang nhìn cái gì trên bàn?", options: ["A map", "A book", "A laptop"], answer: "A map" },
      { id: "ftd2", kind: "observe", q: "What is the weather like in the picture?", vi: "Thời tiết trong bức tranh như thế nào?", options: ["It is rainy", "It is sunny", "It is snowy"], answer: "It is rainy" },
      { id: "ftd3", kind: "observe", q: "What vehicle is behind the glass wall?", vi: "Phương tiện gì ở phía sau bức tường kính?", options: ["A bus", "A train", "A truck"], answer: "A bus" },
      { id: "ftd4", kind: "observe", q: "What is floating on the water in the background?", vi: "Cái gì đang nổi trên mặt nước ở phía xa?", options: ["A boat", "A duck", "A surfboard"], answer: "A boat" },
      { id: "ftd5", kind: "locate", q: "Where is the round clock?", vi: "Chiếc đồng hồ tròn ở đâu?", options: ["On the wall on the left", "On the table", "On the bus"], answer: "On the wall on the left" },
      { id: "ftd6", kind: "locate", q: "Where is the blue water bottle?", vi: "Chai nước màu xanh ở đâu?", options: ["On the right end of the table", "Under the bench", "Inside a backpack"], answer: "On the right end of the table" },
      { id: "ftd7", kind: "compare", q: "Who is taller, the woman or the children?", vi: "Ai cao hơn, người phụ nữ hay bọn trẻ?", options: ["The woman", "The children", "They are the same"], answer: "The woman" },
      { id: "ftd8", kind: "compare", q: "Which is bigger, the bus or an orange cone?", vi: "Cái nào to hơn, chiếc xe buýt hay cái nón cam?", options: ["The bus", "The orange cone", "They are the same"], answer: "The bus" },
      { id: "ftd9", kind: "infer", q: "Why are the children looking at the map together?", vi: "Vì sao bọn trẻ cùng nhau nhìn vào bản đồ?", options: ["To find the right way to go", "To play a card game", "To draw a picture"], answer: "To find the right way to go", explainVi: "Có nón cam và rào chắn trên đường, nên bọn trẻ xem bản đồ để tìm đường đi mới." },
      { id: "ftd10", kind: "sequence", q: "Which most likely happened first?", vi: "Điều gì có lẽ đã xảy ra trước nhất?", options: ["They gathered at the bus shelter", "They pointed at a route on the map", "They got on the boat"], answer: "They gathered at the bus shelter", explainVi: "Bọn trẻ tới trú ở nhà chờ xe buýt trước, rồi mới trải bản đồ ra và chỉ vào lộ trình." },
    ],
  },
  {
    id: "watergarden", title: "Saving Water in the School Garden", vi: "Tiết kiệm nước trong vườn trường", image: "/assets/images/learn/level-3/collection-03-solving-problems/level-3-c03-unit-08-saving-water-in-school-garden.webp",
    emojis: ["💧", "🌱", "🪴", "🚿", "🧤", "🌸", "🦋", "🪣", "🏫", "🌿"],
    questions: [
      { id: "wtg1", kind: "observe", q: "What is the boy in the blue hoodie doing?", vi: "Cậu bé mặc áo hoodie xanh đang làm gì?", options: ["Filling cups with water from a tap", "Kicking a ball", "Reading a book"], answer: "Filling cups with water from a tap" },
      { id: "wtg2", kind: "observe", q: "What is the girl in the purple jacket holding in the soil?", vi: "Cô bé mặc áo tím đang cầm cái gì trong đất?", options: ["A tool that checks the soil", "A phone", "A pencil"], answer: "A tool that checks the soil" },
      { id: "wtg3", kind: "observe", q: "What is the boy in the green hoodie wearing on his hands?", vi: "Cậu bé mặc áo hoodie xanh lá đeo gì trên tay?", options: ["Green gloves", "Red mittens", "Nothing"], answer: "Green gloves" },
      { id: "wtg4", kind: "observe", q: "What is lying on the ground near the flowers?", vi: "Cái gì đang nằm trên mặt đất gần những bông hoa?", options: ["Watering cans", "Soccer balls", "Umbrellas"], answer: "Watering cans" },
      { id: "wtg5", kind: "locate", q: "Where is the picture sign board?", vi: "Tấm bảng có hình vẽ ở đâu?", options: ["On the right side", "On the left side", "On the ground"], answer: "On the right side" },
      { id: "wtg6", kind: "locate", q: "Where is the big water barrel with a tap?", vi: "Thùng chứa nước lớn có vòi ở đâu?", options: ["On the left side", "On the right side", "In the sky"], answer: "On the left side" },
      { id: "wtg7", kind: "compare", q: "Which is bigger, the water barrel or a watering can?", vi: "Cái nào to hơn, thùng nước hay bình tưới?", options: ["The water barrel", "The watering can", "They are the same"], answer: "The water barrel" },
      { id: "wtg8", kind: "compare", q: "Who has curly hair in a bun, the girl on the left or the girl on the right?", vi: "Ai có mái tóc xoăn búi cao, cô bé bên trái hay cô bé bên phải?", options: ["The girl on the right", "The girl on the left", "Neither of them"], answer: "The girl on the right" },
      { id: "wtg9", kind: "infer", q: "Why are the children collecting water in a barrel and checking the soil?", vi: "Vì sao bọn trẻ hứng nước vào thùng và kiểm tra đất?", options: ["To save water for the plants", "To wash their shoes", "To make lemonade"], answer: "To save water for the plants", explainVi: "Bọn trẻ trữ nước và đo độ ẩm của đất để dùng nước vừa đủ, giúp tiết kiệm nước cho cây." },
      { id: "wtg10", kind: "sequence", q: "Which should the children do first?", vi: "Bọn trẻ nên làm việc nào trước?", options: ["Fill the cup with water", "Pour the water on the plant", "Watch the plant grow"], answer: "Fill the cup with water", explainVi: "Phải hứng đầy nước vào cốc trước, rồi mới tưới nước lên cây và chờ cây lớn lên." },
    ],
  },
  {
    id: "communitynews", title: "Community News Mix-Up", vi: "Sự nhầm lẫn của bản tin cộng đồng", image: "/assets/images/learn/level-2/level-2-unit-13-community-news-mix-up.webp",
    emojis: ["🎙️", "📷", "🌧️", "☀️", "☁️", "🖥️", "🕐", "🪴", "📊", "👩‍🏫"],
    questions: [
      { id: "cnm1", kind: "observe", q: "What is the boy in the green hoodie using?", vi: "Cậu bé mặc áo hoodie xanh lá đang dùng gì?", options: ["A sound recorder with a green waveform", "A big camera", "A microphone stand"], answer: "A sound recorder with a green waveform" },
      { id: "cnm2", kind: "observe", q: "What is shown on the tablet screen?", vi: "Trên màn hình máy tính bảng hiện gì?", options: ["Weather icons like sun, clouds, and rain", "A map of the city", "A football game"], answer: "Weather icons like sun, clouds, and rain" },
      { id: "cnm3", kind: "observe", q: "What is the girl in the denim jacket holding up?", vi: "Cô bé mặc áo khoác jean đang giơ cái gì lên?", options: ["A photo of a park with people", "A newspaper", "A drawing of a dog"], answer: "A photo of a park with people" },
      { id: "cnm4", kind: "observe", q: "What is the weather like outside the window?", vi: "Thời tiết bên ngoài cửa sổ như thế nào?", options: ["Rainy", "Snowy", "Bright and sunny"], answer: "Rainy" },
      { id: "cnm5", kind: "locate", q: "Where is the blue mug full of pens?", vi: "Chiếc cốc xanh đầy bút nằm ở đâu?", options: ["On the table near the tablet", "On the bookshelf", "On the window sill"], answer: "On the table near the tablet" },
      { id: "cnm6", kind: "locate", q: "Where is the woman with her arms crossed standing?", vi: "Người phụ nữ khoanh tay đang đứng ở đâu?", options: ["Behind the children", "In front of the window", "Next to the plant on the table"], answer: "Behind the children" },
      { id: "cnm7", kind: "compare", q: "Are there more children or more adults around the table?", vi: "Quanh bàn có nhiều trẻ em hơn hay nhiều người lớn hơn?", options: ["More children", "More adults", "The same number"], answer: "More children" },
      { id: "cnm8", kind: "compare", q: "How does the weather in the photo compare with the weather outside?", vi: "Thời tiết trong tấm ảnh so với thời tiết bên ngoài thì thế nào?", options: ["The photo is sunny but outside it is rainy", "Both are rainy", "Both are sunny"], answer: "The photo is sunny but outside it is rainy" },
      { id: "cnm9", kind: "infer", q: "Why might the news report be a 'mix-up'?", vi: "Vì sao bản tin có thể bị 'nhầm lẫn'?", options: ["The sunny photo does not match the rainy weather now", "The children forgot their homework", "The tablet has no battery"], answer: "The sunny photo does not match the rainy weather now", explainVi: "Tấm ảnh chụp trời nắng ở công viên, nhưng máy tính bảng và cửa sổ cho thấy trời đang mưa. Thông tin không khớp nhau nên bản tin bị nhầm lẫn." },
      { id: "cnm10", kind: "sequence", q: "To make their news report, what do the kids do first?", vi: "Để làm bản tin, các bạn nhỏ làm gì đầu tiên?", options: ["Gather the photo and recorder to collect the news", "Turn off all the lights", "Water the plant on the table"], answer: "Gather the photo and recorder to collect the news", explainVi: "Trước tiên các bạn thu thập tư liệu như tấm ảnh và máy ghi âm để có tin tức, sau đó mới kiểm tra và kể lại bản tin." },
    ],
  },
  {
    id: "lostandfound", title: "Improving Lost and Found", vi: "Cải tiến góc đồ thất lạc", image: "/assets/images/learn/level-3/collection-03-solving-problems/level-3-c03-unit-09-improving-lost-and-found.webp",
    emojis: ["🎒", "🧢", "⚽", "🧤", "🧣", "🥤", "📋", "✅", "❓", "📷"],
    questions: [
      { id: "laf1", kind: "observe", q: "What is the girl in the purple hoodie holding up?", vi: "Cô bé mặc áo hoodie tím đang giơ cái gì lên?", options: ["A green water bottle", "A soccer ball", "A backpack"], answer: "A green water bottle" },
      { id: "laf2", kind: "observe", q: "What is stored on the shelves on the left?", vi: "Trên các kệ bên trái để những gì?", options: ["Lost items like backpacks, bottles, and bags", "Books and notebooks", "Toys and games"], answer: "Lost items like backpacks, bottles, and bags" },
      { id: "laf3", kind: "observe", q: "What is the girl in the yellow hoodie holding?", vi: "Cô bé mặc áo hoodie vàng đang cầm gì?", options: ["A purple beanie hat", "A pair of gloves", "A tablet"], answer: "A purple beanie hat" },
      { id: "laf4", kind: "observe", q: "What is the boy with glasses in the blue hoodie holding?", vi: "Cậu bé đeo kính mặc áo hoodie xanh dương đang cầm gì?", options: ["A tablet", "A soccer ball", "A scarf"], answer: "A tablet" },
      { id: "laf5", kind: "locate", q: "Where is the whiteboard with the checklist of shapes?", vi: "Tấm bảng trắng có danh sách các hình nằm ở đâu?", options: ["On an easel on the right side", "On the floor", "Inside a bin"], answer: "On an easel on the right side" },
      { id: "laf6", kind: "locate", q: "Where are the colorful sorting bins?", vi: "Những chiếc thùng phân loại nhiều màu nằm ở đâu?", options: ["On the table in the middle", "Hanging on the wall", "Under the floor"], answer: "On the table in the middle" },
      { id: "laf7", kind: "compare", q: "Are there more children or more adults working at the station?", vi: "Ở khu vực này có nhiều trẻ em hơn hay nhiều người lớn hơn?", options: ["More children", "More adults", "The same number"], answer: "More children" },
      { id: "laf8", kind: "compare", q: "How does the number of items on the shelves compare with the bins on the table?", vi: "Số đồ vật trên kệ so với trong các thùng trên bàn thì thế nào?", options: ["The shelves hold many more items", "The bins hold many more items", "They hold exactly the same amount"], answer: "The shelves hold many more items" },
      { id: "laf9", kind: "infer", q: "Why are the children sorting items into bins with different icons?", vi: "Vì sao các bạn nhỏ phân loại đồ vào các thùng có biểu tượng khác nhau?", options: ["To organize the lost items so they are easy to find", "To throw all the items away", "To sell the items to visitors"], answer: "To organize the lost items so they are easy to find", explainVi: "Các bạn sắp xếp đồ thất lạc vào từng thùng theo loại (chai nước, mũ, khăn...) để sau này ai mất đồ có thể tìm lại dễ dàng hơn." },
      { id: "laf10", kind: "sequence", q: "When a lost item is found, what happens first?", vi: "Khi tìm thấy một món đồ thất lạc, việc gì xảy ra trước tiên?", options: ["A child picks it up and checks its tag", "It is put on the whiteboard", "It is given straight to the teacher"], answer: "A child picks it up and checks its tag", explainVi: "Đầu tiên bạn nhỏ nhặt món đồ lên và kiểm tra thẻ nhãn của nó, sau đó mới bỏ vào đúng thùng phân loại." },
    ],
  },
  {
    id: "festivalschedule", title: "The Festival Schedule Problem", vi: "Bài toán lịch trình lễ hội", image: "/assets/images/learn/level-2/level-2-unit-15-festival-schedule-problem.webp",
    emojis: ["🎪", "🎸", "🕐", "🍂", "💡", "🎨", "🎉", "🎹", "🏮", "🎵"],
    questions: [
      { id: "fsp1", kind: "observe", q: "How many kids are sitting around the wooden table?", vi: "Có bao nhiêu bạn nhỏ đang ngồi quanh chiếc bàn gỗ?", options: ["Five", "Two", "Eight"], answer: "Five" },
      { id: "fsp2", kind: "observe", q: "What are lying flat on the table next to the colored cards?", vi: "Những vật gì đang nằm trên bàn cạnh các tấm thẻ màu?", options: ["Clocks", "Books", "Plates"], answer: "Clocks" },
      { id: "fsp3", kind: "observe", q: "What are the people on the stage in the background doing?", vi: "Những người trên sân khấu phía sau đang làm gì?", options: ["Playing music", "Cooking food", "Sleeping"], answer: "Playing music" },
      { id: "fsp4", kind: "observe", q: "What season does the scene show, from the leaves and colors?", vi: "Cảnh này diễn ra vào mùa nào, nhìn từ lá cây và màu sắc?", options: ["Autumn", "Summer", "Winter"], answer: "Autumn" },
      { id: "fsp5", kind: "locate", q: "Where is the lit lantern?", vi: "Chiếc đèn lồng đang sáng nằm ở đâu?", options: ["On the right end of the table", "On the stage", "Up in a tree"], answer: "On the right end of the table" },
      { id: "fsp6", kind: "locate", q: "Where is the person climbing the ladder?", vi: "Người đang leo thang ở vị trí nào?", options: ["Behind the kids, setting up a tent", "At the food stall", "On the music stage"], answer: "Behind the kids, setting up a tent" },
      { id: "fsp7", kind: "compare", q: "Who is holding a card up in her hand?", vi: "Ai đang cầm một tấm thẻ giơ lên trên tay?", options: ["The woman in the blue cardigan", "The girl in the striped shirt", "The boy with glasses"], answer: "The woman in the blue cardigan" },
      { id: "fsp8", kind: "compare", q: "Which is brighter, the sky or the string lights?", vi: "Cái nào sáng hơn, bầu trời hay dây đèn?", options: ["The string lights", "The sky", "They are the same"], answer: "The string lights" },
      { id: "fsp9", kind: "infer", q: "Why are the kids looking at clocks and colored cards together?", vi: "Vì sao các bạn nhỏ cùng nhìn vào đồng hồ và các tấm thẻ màu?", options: ["They are planning the festival schedule", "They are eating dinner", "They are taking a nap"], answer: "They are planning the festival schedule", explainVi: "Các bạn sắp xếp thẻ màu theo các khung giờ trên đồng hồ, cho thấy các bạn đang lên lịch trình cho lễ hội." },
      { id: "fsp10", kind: "sequence", q: "The tent is still being put up, but the band is already playing. What is happening?", vi: "Lều vẫn đang được dựng, nhưng ban nhạc đã chơi rồi. Điều gì đang xảy ra?", options: ["The festival is still being set up", "The festival has just ended", "Everyone is going home"], answer: "The festival is still being set up", explainVi: "Có người còn đang dựng lều trong khi ban nhạc đã chơi nhạc, nghĩa là lễ hội vẫn đang trong lúc chuẩn bị." },
    ],
  },
  {
    id: "beachpath", title: "The Accessible Beach Path", vi: "Lối đi ra biển cho mọi người", image: "/assets/images/learn/level-3/collection-03-solving-problems/level-3-c03-unit-12-accessible-beach-path.webp",
    emojis: ["🏖️", "♿", "📏", "📝", "🌊", "🪵", "🛒", "⛰️", "🌲", "📐"],
    questions: [
      { id: "bpt1", kind: "observe", q: "What is the girl on the right sitting in?", vi: "Bạn gái ở bên phải đang ngồi trên cái gì?", options: ["A wheelchair", "A beach chair", "A wooden bench"], answer: "A wheelchair" },
      { id: "bpt2", kind: "observe", q: "What is the path across the sand made of?", vi: "Lối đi trên cát được làm bằng gì?", options: ["Wooden boards", "Bricks", "Glass"], answer: "Wooden boards" },
      { id: "bpt3", kind: "observe", q: "What is the kneeling boy in the green hoodie using?", vi: "Bạn nam mặc áo hoodie xanh lá đang quỳ dùng dụng cụ gì?", options: ["A measuring tape", "A paintbrush", "A camera"], answer: "A measuring tape" },
      { id: "bpt4", kind: "observe", q: "What can you see far away in the background?", vi: "Bạn có thể thấy gì ở phía xa trong nền cảnh?", options: ["Mountains and the sea", "A tall castle", "A snowy village"], answer: "Mountains and the sea" },
      { id: "bpt5", kind: "locate", q: "Where is the boy in the blue hoodie pushing the cart?", vi: "Bạn nam mặc áo hoodie xanh dương đang đẩy xe kéo ở đâu?", options: ["On the wooden path", "In the ocean", "Up a tree"], answer: "On the wooden path" },
      { id: "bpt6", kind: "locate", q: "Where is the open notebook with a pencil?", vi: "Cuốn sổ mở cùng cây bút chì nằm ở đâu?", options: ["On the ground in the front left", "On the wheelchair", "In the sea"], answer: "On the ground in the front left" },
      { id: "bpt7", kind: "compare", q: "Who is standing closer to the wheelchair girl, the cart boy or the measuring boy?", vi: "Ai đứng gần bạn gái ngồi xe lăn hơn, bạn đẩy xe hay bạn cầm thước?", options: ["The cart boy", "The measuring boy", "They are the same distance"], answer: "The cart boy" },
      { id: "bpt8", kind: "compare", q: "Which kid is holding a clipboard and writing?", vi: "Bạn nào đang cầm bảng kẹp và viết?", options: ["The girl in the purple jacket", "The boy in the blue hoodie", "The girl in the yellow hoodie"], answer: "The girl in the purple jacket" },
      { id: "bpt9", kind: "infer", q: "Why are the kids measuring the path and testing tile samples?", vi: "Vì sao các bạn đo lối đi và thử các mẫu gạch lát?", options: ["To build a path everyone can use", "To play a beach game", "To find shells"], answer: "To build a path everyone can use", explainVi: "Các bạn đo lối đi, ghi chép và thử mẫu vật liệu để làm một lối đi phẳng mà cả xe lăn cũng đi được ra biển." },
      { id: "bpt10", kind: "sequence", q: "The wheelchair sits on the mat, but the path is still being measured. What comes next?", vi: "Xe lăn đang ở trên tấm thảm, nhưng lối đi vẫn đang được đo. Bước tiếp theo là gì?", options: ["They finish building the path to the beach", "They pack up and leave", "They go swimming right away"], answer: "They finish building the path to the beach", explainVi: "Lối đi vẫn đang được đo và thử vật liệu, nên bước tiếp theo là hoàn thành lối đi để xe lăn ra tới biển." },
    ],
  },
  {
    id: "museumaudio", title: "The Museum Audio Guide Mix-Up", vi: "Sự cố lẫn lộn máy thuyết minh bảo tàng", image: "/assets/images/learn/level-2/level-2-unit-18-museum-audio-guide-mix-up.webp",
    emojis: ["🦖", "🦴", "🏛️", "📱", "🔮", "🐢", "🍃", "🧭", "🗺️", "🔦"],
    questions: [
      { id: "mag1", kind: "observe", q: "What is the big skeleton on the left of the picture?", vi: "Bộ xương lớn ở bên trái bức tranh là gì?", options: ["A dinosaur", "An elephant", "A whale"], answer: "A dinosaur" },
      { id: "mag2", kind: "observe", q: "What is the boy on the left doing with his device?", vi: "Cậu bé bên trái đang làm gì với chiếc máy của mình?", options: ["Holding it to his ear", "Eating it", "Throwing it away"], answer: "Holding it to his ear" },
      { id: "mag3", kind: "observe", q: "How many devices are sitting in the charging dock?", vi: "Có bao nhiêu chiếc máy đang nằm trong khay sạc?", options: ["Four", "Two", "Seven"], answer: "Four" },
      { id: "mag4", kind: "observe", q: "What is the boy with glasses doing?", vi: "Cậu bé đeo kính đang làm gì?", options: ["Writing on the paper with a pen", "Sleeping on the table", "Running away"], answer: "Writing on the paper with a pen" },
      { id: "mag5", kind: "locate", q: "Where is the charging dock with the four devices?", vi: "Khay sạc với bốn chiếc máy nằm ở đâu?", options: ["On the table at the left", "On the ceiling", "Outside the window"], answer: "On the table at the left" },
      { id: "mag6", kind: "locate", q: "What animal can you see in the blue display in the background?", vi: "Con vật nào xuất hiện trong ô kính màu xanh ở phía sau?", options: ["A sea turtle", "A lion", "A horse"], answer: "A sea turtle" },
      { id: "mag7", kind: "compare", q: "Which device in the dock shows a green leaf?", vi: "Chiếc máy nào trong khay có hình chiếc lá xanh?", options: ["The green one", "The red one", "The purple one"], answer: "The green one" },
      { id: "mag8", kind: "compare", q: "Who is holding two devices at the same time?", vi: "Ai đang cầm hai chiếc máy cùng một lúc?", options: ["The girl in the middle", "The boy on the left", "The museum guide"], answer: "The girl in the middle" },
      { id: "mag9", kind: "infer", q: "Why does the boy on the left look worried?", vi: "Vì sao cậu bé bên trái trông có vẻ lo lắng?", options: ["He might have the wrong audio guide", "He is very hungry", "It is raining inside"], answer: "He might have the wrong audio guide", explainVi: "Cậu bé áp máy vào tai và trông bối rối, vì các máy thuyết minh có thể đã bị lẫn lộn nên cậu lo mình cầm nhầm máy." },
      { id: "mag10", kind: "sequence", q: "On the paper trail, which icon comes right after the purple crystals?", vi: "Trên đường icon vẽ trên giấy, hình nào đứng ngay sau cụm tinh thể tím?", options: ["The blue turtle", "The red compass", "The green leaf"], answer: "The blue turtle", explainVi: "Các icon nối bằng mũi tên theo thứ tự: hóa thạch, tinh thể tím, rùa xanh, lá xanh, la bàn đỏ, nên ngay sau tinh thể tím là con rùa xanh." },
    ],
  },
  {
    id: "pollinator", title: "The Pollinator School Garden", vi: "Khu vườn thụ phấn ở trường", image: "/assets/images/learn/level-3/collection-03-solving-problems/level-3-c03-unit-14-pollinator-school-garden.webp",
    emojis: ["🦋", "🐝", "🌻", "🌸", "🌿", "📓", "✏️", "💧", "🪵", "🧢"],
    questions: [
      { id: "pol1", kind: "observe", q: "What kind of insect is flying near the flowers on the left?", vi: "Loại côn trùng nào đang bay gần những bông hoa bên trái?", options: ["A butterfly", "A bird", "A bat"], answer: "A butterfly" },
      { id: "pol2", kind: "observe", q: "What is the girl on the left doing?", vi: "Bạn gái bên trái đang làm gì?", options: ["Writing in a notebook", "Cooking dinner", "Riding a bike"], answer: "Writing in a notebook" },
      { id: "pol3", kind: "observe", q: "What color hoodie is the girl on the left wearing?", vi: "Bạn gái bên trái đang mặc áo hoodie màu gì?", options: ["Purple", "Orange", "Black"], answer: "Purple" },
      { id: "pol4", kind: "observe", q: "What insects are resting on the flowers?", vi: "Những con côn trùng nào đang đậu trên các bông hoa?", options: ["Bees", "Spiders", "Ants"], answer: "Bees" },
      { id: "pol5", kind: "locate", q: "What is the boy in the green hoodie pointing at?", vi: "Bạn trai mặc áo xanh lá đang chỉ vào cái gì?", options: ["A dish of water with stones", "A television", "A car"], answer: "A dish of water with stones" },
      { id: "pol6", kind: "locate", q: "What is the girl on the right holding?", vi: "Bạn gái bên phải đang cầm cái gì?", options: ["A bundle of bamboo tubes", "A soccer ball", "An umbrella"], answer: "A bundle of bamboo tubes" },
      { id: "pol7", kind: "observe", q: "What is the boy in the middle wearing on his face?", vi: "Bạn trai ở giữa đang đeo gì trên mặt?", options: ["Glasses", "A mask", "Face paint"], answer: "Glasses" },
      { id: "pol8", kind: "compare", q: "Which child is wearing a green cap?", vi: "Bạn nào đang đội chiếc mũ lưỡi trai màu xanh lá?", options: ["The child at the back", "The boy in blue", "The girl on the left"], answer: "The child at the back" },
      { id: "pol9", kind: "infer", q: "Why did the children put stones in the shallow dish of water?", vi: "Vì sao các bạn nhỏ đặt những viên đá vào chiếc đĩa nước nông?", options: ["So bees can land and drink safely", "To wash their hands", "To catch fish"], answer: "So bees can land and drink safely", explainVi: "Ong nhỏ và dễ bị chìm, nên các bạn xếp đá vào đĩa nước để ong có chỗ đậu và uống nước mà không bị đuối." },
      { id: "pol10", kind: "sequence", q: "How can a bee safely drink from the shallow dish? Choose the correct order.", vi: "Con ong uống nước từ chiếc đĩa nông một cách an toàn như thế nào? Chọn thứ tự đúng.", options: ["First it lands on a stone, then it drinks the water", "First it dives under the water, then it swims", "First it eats a stone, then it flies away"], answer: "First it lands on a stone, then it drinks the water", explainVi: "Đầu tiên ong đậu lên viên đá nhô khỏi mặt nước, sau đó nó cúi xuống uống nước, nhờ vậy chân không bị ướt và không bị chìm." },
    ],
  },
];

export const detectiveSceneById = (id: string) => DETECTIVE_SCENES.find((s) => s.id === id);
// Chọn 1 cảnh ngẫu nhiên (gọi lúc runtime phía client)
export function randomDetectiveScene(exceptId?: string): DetectiveScene {
  const pool = exceptId ? DETECTIVE_SCENES.filter((s) => s.id !== exceptId) : DETECTIVE_SCENES;
  const list = pool.length ? pool : DETECTIVE_SCENES;
  return list[Math.floor(Math.random() * list.length)];
}

/* ============ Picture Talk — thử thách mô tả tranh CÓ ĐÁP ÁN (máy chấm được) ============ */
// Không thu âm. Bé chọn/xếp câu mô tả đúng bức tranh → chấm điểm thật.
// "say" = câu mô tả hoàn chỉnh để nói theo Maple (bước phụ, KHÔNG tính điểm).
export type TalkTaskKind = "choose" | "spot" | "fill" | "position" | "arrange";
export type TalkTask = {
  id: string;
  kind: TalkTaskKind;
  vi: string;                 // hướng dẫn/nghĩa tiếng Việt
  q?: string;                 // choose/spot/position: câu hỏi · fill: câu có chỗ "___"
  options?: string[];         // choose/spot/fill/position
  answer?: string;            // đáp án đúng (mọi kind trừ arrange)
  solution?: string[];        // arrange: các từ theo đúng thứ tự
  say: string;                // câu mô tả hoàn chỉnh để nói theo (không tính điểm)
  difficulty?: Difficulty;    // tùy chọn — nếu thiếu sẽ suy ra từ kind
};
export type TalkScene = {
  id: string; title: string; vi: string; image: string; emojis: string[];
  intro: string; tasks: TalkTask[];
};

export const TALK_SCENES: TalkScene[] = [
  {
    id: "park", title: "Describe the Park", vi: "Mô tả công viên", image: IMG + "scene-park.webp",
    emojis: ["🌳", "🐕", "⚽", "🧒", "👧", "🌷", "🪑", "🍁"],
    intro: "Nhìn công viên. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "tp1", kind: "choose", vi: "Chọn câu mô tả ĐÚNG bức tranh.", q: "Which sentence is TRUE about the picture?", options: ["The dog is chasing a ball.", "The dog is sleeping.", "The dog is eating dinner."], answer: "The dog is chasing a ball.", say: "The dog is chasing a ball." },
      { id: "tp2", kind: "spot", vi: "Câu nào KHÔNG đúng với tranh?", q: "Which sentence is FALSE about the picture?", options: ["Two children are running.", "It is snowing in the park.", "The trees have red leaves."], answer: "It is snowing in the park.", say: "It is not snowing. It is a sunny autumn day." },
      { id: "tp3", kind: "fill", vi: "Điền từ đúng: Các bạn đang … trong công viên.", q: "The children are ___ in the park.", options: ["running", "sleeping", "swimming"], answer: "running", say: "The children are running in the park." },
      { id: "tp4", kind: "position", vi: "Cái ghế trống ở đâu?", q: "Where is the empty bench?", options: ["next to the tree", "under the water", "up in the sky"], answer: "next to the tree", say: "The empty bench is next to the tree." },
      { id: "tp5", kind: "arrange", vi: "Xếp câu: Chú chó đang đuổi theo quả bóng.", solution: ["The", "dog", "is", "chasing", "a", "ball"], say: "The dog is chasing a ball." },
      { id: "tp6", kind: "arrange", vi: "Xếp câu: Hai bạn nhỏ đang chạy.", solution: ["Two", "children", "are", "running"], say: "Two children are running." },
      { id: "tp7", kind: "fill", vi: "Điền màu: Lá cây màu đỏ và …", q: "The leaves are red and ___.", options: ["orange", "blue", "white"], answer: "orange", say: "The leaves are red and orange." },
    ],
  },
  {
    id: "kitchen", title: "Describe the Kitchen", vi: "Mô tả nhà bếp", image: IMG + "scene-kitchen.webp",
    emojis: ["🍳", "🍎", "🍌", "🥛", "🍞", "🧀", "🥕", "🫖"],
    intro: "Nhìn nhà bếp. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "tk1", kind: "choose", vi: "Chọn câu mô tả ĐÚNG bức tranh.", q: "Which sentence is TRUE about the picture?", options: ["The cook is making a fried egg.", "The cook is washing a car.", "The cook is reading a book."], answer: "The cook is making a fried egg.", say: "The cook is making a fried egg." },
      { id: "tk2", kind: "spot", vi: "Câu nào KHÔNG đúng với tranh?", q: "Which sentence is FALSE about the picture?", options: ["There are apples on the table.", "There is bread on the board.", "The cook is riding a bike."], answer: "The cook is riding a bike.", say: "The cook is cooking, not riding a bike." },
      { id: "tk3", kind: "fill", vi: "Điền từ: Người nấu đang … một quả trứng.", q: "The cook is ___ a fried egg.", options: ["making", "driving", "singing"], answer: "making", say: "The cook is making a fried egg." },
      { id: "tk4", kind: "position", vi: "Miếng phô mai ở đâu?", q: "Where is the cheese?", options: ["on the small plate", "under the floor", "inside the teapot"], answer: "on the small plate", say: "The cheese is on the small plate." },
      { id: "tk5", kind: "arrange", vi: "Xếp câu: Người nấu đang chiên một quả trứng.", solution: ["The", "cook", "is", "frying", "an", "egg"], say: "The cook is frying an egg." },
      { id: "tk6", kind: "arrange", vi: "Xếp câu: Mình thấy bánh mì tươi.", solution: ["I", "can", "see", "fresh", "bread"], say: "I can see fresh bread." },
      { id: "tk7", kind: "fill", vi: "Điền từ: Trên bàn có táo và …", q: "There are apples and ___ on the table.", options: ["bananas", "cars", "shoes"], answer: "bananas", say: "There are apples and bananas on the table." },
    ],
  },
  {
    id: "classroom", title: "Describe the Classroom", vi: "Mô tả lớp học", image: IMG + "scene-classroom.webp",
    emojis: ["🧑‍🏫", "🗺️", "📖", "✋", "🌋", "🎒", "🕐", "🪴"],
    intro: "Nhìn lớp học. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "tc1", kind: "choose", vi: "Chọn câu mô tả ĐÚNG bức tranh.", q: "Which sentence is TRUE about the picture?", options: ["The teacher is pointing at the world map.", "The teacher is cooking dinner.", "The teacher is sleeping."], answer: "The teacher is pointing at the world map.", say: "The teacher is pointing at the world map." },
      { id: "tc2", kind: "spot", vi: "Câu nào KHÔNG đúng với tranh?", q: "Which sentence is FALSE about the picture?", options: ["A boy is reading a book.", "Two students do an experiment.", "A dog is teaching the class."], answer: "A dog is teaching the class.", say: "A teacher is teaching the class, not a dog." },
      { id: "tc3", kind: "fill", vi: "Điền từ: Bạn nữ đang … tay.", q: "The girl is ___ her hand.", options: ["raising", "eating", "washing"], answer: "raising", say: "The girl is raising her hand." },
      { id: "tc4", kind: "position", vi: "Chiếc máy bay giấy ở đâu?", q: "Where is the paper airplane?", options: ["on the floor", "on the moon", "in the water"], answer: "on the floor", say: "The paper airplane is on the floor." },
      { id: "tc5", kind: "arrange", vi: "Xếp câu: Cô giáo chỉ vào bản đồ.", solution: ["The", "teacher", "points", "at", "the", "map"], say: "The teacher points at the map." },
      { id: "tc6", kind: "arrange", vi: "Xếp câu: Hai bạn làm một thí nghiệm.", solution: ["Two", "students", "do", "an", "experiment"], say: "Two students do an experiment." },
      { id: "tc7", kind: "fill", vi: "Điền từ: Lớp học nhộn nhịp và …", q: "The classroom is busy and ___.", options: ["fun", "dark", "cold"], answer: "fun", say: "The classroom is busy and fun." },
    ],
  },
  {
    id: "supermarket", title: "Describe the Supermarket", vi: "Mô tả siêu thị", image: IMG + "scene-supermarket.webp",
    emojis: ["🛒", "🍎", "🍌", "🍊", "🥖", "🥛", "📝", "☂️"],
    intro: "Nhìn siêu thị. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "ts1", kind: "choose", vi: "Chọn câu mô tả ĐÚNG bức tranh.", q: "Which sentence is TRUE about the picture?", options: ["A boy is pushing a shopping cart.", "A boy is flying a kite.", "A boy is swimming."], answer: "A boy is pushing a shopping cart.", say: "A boy is pushing a shopping cart." },
      { id: "ts2", kind: "spot", vi: "Câu nào KHÔNG đúng với tranh?", q: "Which sentence is FALSE about the picture?", options: ["A cashier is at the checkout.", "The old woman's bag spilled.", "An elephant is buying milk."], answer: "An elephant is buying milk.", say: "There is no elephant. People are shopping." },
      { id: "ts3", kind: "fill", vi: "Điền từ: Cậu bé cầm một … mua sắm.", q: "The boy is holding a shopping ___.", options: ["list", "cat", "car"], answer: "list", say: "The boy is holding a shopping list." },
      { id: "ts4", kind: "position", vi: "Chiếc ô xanh ở đâu?", q: "Where is the blue umbrella?", options: ["by the door", "in the sky", "under the sea"], answer: "by the door", say: "The blue umbrella is by the door." },
      { id: "ts5", kind: "arrange", vi: "Xếp câu: Một cậu bé đẩy xe hàng.", solution: ["A", "boy", "pushes", "a", "shopping", "cart"], say: "A boy pushes a shopping cart." },
      { id: "ts6", kind: "arrange", vi: "Xếp câu: Có táo và cam trong thùng.", solution: ["There", "are", "apples", "and", "oranges"], say: "There are apples and oranges." },
      { id: "ts7", kind: "fill", vi: "Điền từ: Bạn nữ với lấy táo màu …", q: "The girl is reaching for green ___.", options: ["apples", "cars", "books"], answer: "apples", say: "The girl is reaching for green apples." },
    ],
  },
  {
    id: "busstop", title: "Describe the Bus Stop", vi: "Mô tả trạm xe buýt", image: IMG + "scene-bus-stop-rain.webp",
    emojis: ["🌧️", "🚌", "☂️", "🎒", "🎫", "👟", "🍁", "⌚"],
    intro: "Nhìn trạm xe buýt ngày mưa. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "tb1", kind: "choose", vi: "Chọn câu mô tả ĐÚNG bức tranh.", q: "Which sentence is TRUE about the picture?", options: ["It is a rainy day.", "It is a hot sunny day.", "It is a snowy night."], answer: "It is a rainy day.", say: "It is a rainy day at the bus stop." },
      { id: "tb2", kind: "spot", vi: "Câu nào KHÔNG đúng với tranh?", q: "Which sentence is FALSE about the picture?", options: ["A man is checking his watch.", "A blue bus is coming.", "A lion is driving the bus."], answer: "A lion is driving the bus.", say: "A driver is on the bus, not a lion." },
      { id: "tb3", kind: "fill", vi: "Điền từ: Cậu bé đang … để kịp xe buýt.", q: "The boy is ___ to catch the bus.", options: ["running", "sleeping", "cooking"], answer: "running", say: "The boy is running to catch the bus." },
      { id: "tb4", kind: "position", vi: "Chiếc balô ở đâu?", q: "Where is the backpack?", options: ["on the bench", "in the bus", "on the roof"], answer: "on the bench", say: "The backpack is on the bench." },
      { id: "tb5", kind: "arrange", vi: "Xếp câu: Một chú đang xem đồng hồ.", solution: ["A", "man", "checks", "his", "watch"], say: "A man checks his watch." },
      { id: "tb6", kind: "arrange", vi: "Xếp câu: Xe buýt xanh đang tới.", solution: ["The", "blue", "bus", "is", "coming"], say: "The blue bus is coming." },
      { id: "tb7", kind: "fill", vi: "Điền màu: Ô của bà cụ màu …", q: "The old woman's umbrella is ___.", options: ["yellow", "green", "black"], answer: "yellow", say: "The old woman's umbrella is yellow." },
    ],
  },
  {
    id: "library", title: "Describe the Library", vi: "Mô tả thư viện", image: IMG + "scene-library.webp",
    emojis: ["📚", "🪜", "🔖", "👓", "🧣", "🎒", "🪑", "🏙️"],
    intro: "Nhìn thư viện. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "tl1", kind: "choose", vi: "Chọn câu mô tả ĐÚNG bức tranh.", q: "Which sentence is TRUE about the picture?", options: ["The librarian is pushing a book cart.", "The librarian is cooking soup.", "The librarian is swimming."], answer: "The librarian is pushing a book cart.", say: "The librarian is pushing a book cart." },
      { id: "tl2", kind: "spot", vi: "Câu nào KHÔNG đúng với tranh?", q: "Which sentence is FALSE about the picture?", options: ["Two children are reading.", "A girl stands on a stool.", "A shark is reading a book."], answer: "A shark is reading a book.", say: "Children are reading, there is no shark." },
      { id: "tl3", kind: "fill", vi: "Điền từ: Bạn nữ đứng trên một … để với kệ.", q: "A girl is standing on a ___ to reach the shelf.", options: ["stool", "bus", "boat"], answer: "stool", say: "A girl is standing on a stool to reach the shelf." },
      { id: "tl4", kind: "position", vi: "Chiếc balô ở đâu?", q: "Where is the backpack?", options: ["under the table", "on the ceiling", "in the river"], answer: "under the table", say: "The backpack is under the table." },
      { id: "tl5", kind: "arrange", vi: "Xếp câu: Thư viện yên tĩnh và êm đềm.", solution: ["The", "library", "is", "calm", "and", "quiet"], say: "The library is calm and quiet." },
      { id: "tl6", kind: "arrange", vi: "Xếp câu: Hai bạn đọc sách ở bàn.", solution: ["Two", "children", "read", "at", "a", "table"], say: "Two children read at a table." },
      { id: "tl7", kind: "fill", vi: "Điền màu: Bạn nữ áo vàng đọc cuốn sách màu …", q: "The girl in yellow is reading a ___ book.", options: ["pink", "green", "black"], answer: "pink", say: "The girl in yellow is reading a pink book." },
    ],
  },
  {
    id: "sciencefair", title: "Describe the Science Fair", vi: "Mô tả hội chợ khoa học", image: L1 + "level-1-unit-07-at-the-science-fair.webp",
    emojis: ["🔬", "🌋", "🌬️", "🎗️", "🌉", "🧑‍⚖️", "🚗", "🎒"],
    intro: "Nhìn hội chợ khoa học. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "df1", kind: "choose", vi: "Chọn câu mô tả ĐÚNG bức tranh.", q: "Which sentence is TRUE about the picture?", options: ["A girl is presenting a wind turbine model.", "A girl is riding a horse.", "A girl is baking a cake."], answer: "A girl is presenting a wind turbine model.", say: "A girl is presenting a wind turbine model." },
      { id: "df2", kind: "spot", vi: "Câu nào KHÔNG đúng với tranh?", q: "Which sentence is FALSE about the picture?", options: ["A volcano is erupting with red foam.", "A teacher is judging the projects.", "A whale is swimming in the gym."], answer: "A whale is swimming in the gym.", say: "There is no whale. It is a science fair in a gym." },
      { id: "df3", kind: "fill", vi: "Điền từ: Núi lửa đang … bọt đỏ.", q: "The volcano is ___ with red foam.", options: ["erupting", "sleeping", "singing"], answer: "erupting", say: "The volcano is erupting with red foam." },
      { id: "df4", kind: "position", vi: "Những huy hiệu ruy-băng ở đâu?", q: "Where are the prize ribbons?", options: ["on a board on the right", "under the water", "in the sky"], answer: "on a board on the right", say: "The prize ribbons are on a board on the right." },
      { id: "df5", kind: "arrange", vi: "Xếp câu: Giám khảo đang xem từng dự án.", solution: ["The", "judge", "is", "looking", "at", "the", "projects"], say: "The judge is looking at the projects." },
      { id: "df6", kind: "arrange", vi: "Xếp câu: Chiếc xe chạy bằng năng lượng mặt trời.", solution: ["The", "car", "runs", "on", "solar", "power"], say: "The car runs on solar power." },
      { id: "df7", kind: "fill", vi: "Điền màu: Dự án tốt nhất được huy hiệu màu …", q: "The best project wins a ___ ribbon.", options: ["blue", "purple", "brown"], answer: "blue", say: "The best project wins a blue ribbon." },
    ],
  },
  {
    id: "sciencemuseum", title: "Describe the Science Museum", vi: "Mô tả bảo tàng khoa học", image: L1 + "level-1-unit-08-at-the-science-museum.webp",
    emojis: ["🦖", "🪐", "🔭", "🧲", "🐚", "🤖", "🌈", "🎒"],
    intro: "Nhìn bảo tàng khoa học. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "dm1", kind: "choose", vi: "Chọn câu mô tả ĐÚNG bức tranh.", q: "Which sentence is TRUE about the picture?", options: ["A huge dinosaur skeleton stands in the hall.", "A dinosaur is eating lunch.", "A dinosaur is driving a car."], answer: "A huge dinosaur skeleton stands in the hall.", say: "A huge dinosaur skeleton stands in the hall." },
      { id: "dm2", kind: "spot", vi: "Câu nào KHÔNG đúng với tranh?", q: "Which sentence is FALSE about the picture?", options: ["Model planets hang from the ceiling.", "A boy looks through a telescope.", "A shark is flying in the room."], answer: "A shark is flying in the room.", say: "There is no flying shark in the museum." },
      { id: "dm3", kind: "fill", vi: "Điền từ: Các hành tinh … trên trần.", q: "The planets ___ from the ceiling.", options: ["hang", "cook", "run"], answer: "hang", say: "The planets hang from the ceiling." },
      { id: "dm4", kind: "position", vi: "Bộ xương khủng long ở đâu?", q: "Where is the dinosaur skeleton?", options: ["on the left side of the hall", "inside a teapot", "under the sea"], answer: "on the left side of the hall", say: "The dinosaur skeleton is on the left side of the hall." },
      { id: "dm5", kind: "arrange", vi: "Xếp câu: Hai bạn nhỏ xem xe tự hành.", solution: ["Two", "children", "look", "at", "the", "rover"], say: "Two children look at the rover." },
      { id: "dm6", kind: "arrange", vi: "Xếp câu: Mình được nhìn nhưng không được chạm.", solution: ["I", "can", "look", "but", "not", "touch"], say: "I can look but not touch." },
      { id: "dm7", kind: "fill", vi: "Điền từ: Lăng kính tách ánh sáng thành một …", q: "The prism splits light into a ___.", options: ["rainbow", "sandwich", "song"], answer: "rainbow", say: "The prism splits light into a rainbow." },
    ],
  },
  {
    id: "waterfront", title: "Describe the Waterfront", vi: "Mô tả bờ nước Vancouver", image: L1 + "level-1-unit-09-at-the-vancouver-waterfront.webp",
    emojis: ["🌊", "⛴️", "🏙️", "🏔️", "🐦", "🔭", "🚲", "🍁"],
    intro: "Nhìn bờ nước Vancouver. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "dw1", kind: "choose", vi: "Chọn câu mô tả ĐÚNG bức tranh.", q: "Which sentence is TRUE about the picture?", options: ["A ferry is sailing on the water.", "A ferry is flying in the sky.", "A ferry is in the classroom."], answer: "A ferry is sailing on the water.", say: "A ferry is sailing on the water." },
      { id: "dw2", kind: "spot", vi: "Câu nào KHÔNG đúng với tranh?", q: "Which sentence is FALSE about the picture?", options: ["A heron is standing near the water.", "A girl is holding a map.", "A polar bear is riding the ferry."], answer: "A polar bear is riding the ferry.", say: "There is no polar bear on the ferry." },
      { id: "dw3", kind: "fill", vi: "Điền từ: Bạn nữ áo vàng nhìn qua chiếc …", q: "The girl in yellow looks through her ___.", options: ["binoculars", "sandwich", "pillow"], answer: "binoculars", say: "The girl in yellow looks through her binoculars." },
      { id: "dw4", kind: "position", vi: "Con diệc xám đứng ở đâu?", q: "Where is the grey heron?", options: ["near the water on the right", "on the moon", "inside a locker"], answer: "near the water on the right", say: "The grey heron is near the water on the right." },
      { id: "dw5", kind: "arrange", vi: "Xếp câu: Bên kia mặt nước có núi tuyết.", solution: ["Across", "the", "water", "are", "snowy", "mountains"], say: "Across the water are snowy mountains." },
      { id: "dw6", kind: "arrange", vi: "Xếp câu: Một cậu bé đang sửa xe đạp.", solution: ["A", "boy", "is", "fixing", "his", "bike"], say: "A boy is fixing his bike." },
      { id: "dw7", kind: "fill", vi: "Điền từ: Bầu trời nhiều mây nên trời có thể …", q: "The sky is cloudy, so it might ___.", options: ["rain", "sing", "sleep"], answer: "rain", say: "The sky is cloudy, so it might rain." },
    ],
  },
  {
    id: "missingbackpack", title: "Describe the Missing Backpack", vi: "Mô tả chiếc balô thất lạc", image: L2 + "level-2-unit-01-missing-backpack.webp",
    emojis: ["🎒", "🧭", "📓", "🥤", "🔎", "🚪", "🪑", "🏫"],
    intro: "Nhìn dãy hành lang. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "tmb1", kind: "choose", vi: "Chọn câu mô tả ĐÚNG bức tranh.", q: "Which sentence is TRUE about the picture?", options: ["The children are searching the floor.", "The children are swimming.", "The children are eating lunch."], answer: "The children are searching the floor.", say: "The children are searching the floor." },
      { id: "tmb2", kind: "spot", vi: "Câu nào KHÔNG đúng với tranh?", q: "Which sentence is FALSE about the picture?", options: ["A boy is holding a map.", "A water bottle is on the floor.", "A dog is reading a map."], answer: "A dog is reading a map.", say: "A boy is holding the map, not a dog." },
      { id: "tmb3", kind: "fill", vi: "Điền từ: Cậu bé cầm một … của trường.", q: "The boy is holding a ___ of the school.", options: ["map", "cake", "shoe"], answer: "map", say: "The boy is holding a map of the school." },
      { id: "tmb4", kind: "position", vi: "Quyển vở xanh ở đâu?", q: "Where is the green notebook?", options: ["on the floor", "in the sky", "under the sea"], answer: "on the floor", say: "The green notebook is on the floor." },
      { id: "tmb5", kind: "arrange", vi: "Xếp câu: Các bạn đang tìm một chiếc balô.", solution: ["The", "children", "are", "looking", "for", "a", "backpack"], say: "The children are looking for a backpack." },
      { id: "tmb6", kind: "arrange", vi: "Xếp câu: Một chai nước nằm trên sàn.", solution: ["A", "water", "bottle", "is", "on", "the", "floor"], say: "A water bottle is on the floor." },
      { id: "tmb7", kind: "fill", vi: "Điền từ: Nắng chiếu qua khung … lớn.", q: "Sunlight comes through the big ___.", options: ["window", "oven", "river"], answer: "window", say: "Sunlight comes through the big window." },
    ],
  },
  {
    id: "classtrip", title: "Describe the Class Trip Plan", vi: "Mô tả buổi lên kế hoạch chuyến đi", image: L3 + "collection-01-making-choices/level-3-c01-unit-01-planning-class-trip.webp",
    emojis: ["🗺️", "🔭", "🌲", "🦌", "⛴️", "🗳️", "🌍", "🧑‍🏫"],
    intro: "Nhìn buổi bàn kế hoạch. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "tct1", kind: "choose", vi: "Chọn câu mô tả ĐÚNG bức tranh.", q: "Which sentence is TRUE about the picture?", options: ["The students are choosing a class trip.", "The students are sleeping.", "The students are on a boat."], answer: "The students are choosing a class trip.", say: "The students are choosing a class trip." },
      { id: "tct2", kind: "spot", vi: "Câu nào KHÔNG đúng với tranh?", q: "Which sentence is FALSE about the picture?", options: ["There are three cards on the table.", "The teacher holds a voting board.", "A deer is sitting at the table."], answer: "A deer is sitting at the table.", say: "The deer is only a picture on a card." },
      { id: "tct3", kind: "fill", vi: "Điền từ: Trên bàn có … tấm thẻ chuyến đi.", q: "There are ___ trip cards on the table.", options: ["three", "ten", "two"], answer: "three", say: "There are three trip cards on the table." },
      { id: "tct4", kind: "position", vi: "Quả địa cầu ở đâu?", q: "Where is the globe?", options: ["on the shelf", "under the table", "up in the sky"], answer: "on the shelf", say: "The globe is on the shelf." },
      { id: "tct5", kind: "arrange", vi: "Xếp câu: Cả lớp đang lên kế hoạch một chuyến đi.", solution: ["The", "class", "is", "planning", "a", "school", "trip"], say: "The class is planning a school trip." },
      { id: "tct6", kind: "arrange", vi: "Xếp câu: Chúng mình bỏ phiếu cho lựa chọn tốt nhất.", solution: ["We", "vote", "for", "the", "best", "choice"], say: "We vote for the best choice." },
      { id: "tct7", kind: "fill", vi: "Điền từ: Tấm thẻ cuối có hình một … với chiếc mỏ neo.", q: "The last card shows a ___ with an anchor.", options: ["boat", "plane", "train"], answer: "boat", say: "The last card shows a boat with an anchor." },
    ],
  },
  {
    id: "talentshow", title: "Describe the Talent Show", vi: "Mô tả hội diễn tài năng", image: L2 + "level-2-unit-03-school-talent-show.webp",
    emojis: ["🎸", "🎩", "🃏", "🎤", "✨", "🎭"],
    intro: "Nhìn hậu trường hội diễn. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "tsh1", kind: "choose", vi: "Chọn câu mô tả ĐÚNG bức tranh.", q: "Which sentence is TRUE about the picture?", options: ["A girl is playing the guitar.", "A girl is riding a bike.", "A girl is cooking soup."], answer: "A girl is playing the guitar.", say: "A girl is playing the guitar." },
      { id: "tsh2", kind: "spot", vi: "Câu nào KHÔNG đúng với tranh?", q: "Which sentence is FALSE about the picture?", options: ["A boy is holding playing cards.", "A top hat is on the floor.", "A lion is singing on stage."], answer: "A lion is singing on stage.", say: "There is no lion; the children are performing." },
      { id: "tsh3", kind: "fill", vi: "Điền từ: Nhà ảo thuật đang mặc một chiếc …", q: "The magician is wearing a ___.", options: ["cape", "raincoat", "apron"], answer: "cape", say: "The magician is wearing a cape." },
      { id: "tsh4", kind: "position", vi: "Chiếc micro ở đâu?", q: "Where is the microphone?", options: ["on the stage", "under the table", "inside the trunk"], answer: "on the stage", say: "The microphone is on the stage." },
      { id: "tsh5", kind: "arrange", vi: "Xếp câu: Một bạn nữ đang chơi ghi ta.", solution: ["A", "girl", "is", "playing", "the", "guitar"], say: "A girl is playing the guitar." },
      { id: "tsh6", kind: "arrange", vi: "Xếp câu: Nhà ảo thuật cầm vài lá bài.", solution: ["The", "magician", "holds", "some", "cards"], say: "The magician holds some cards." },
      { id: "tsh7", kind: "fill", vi: "Điền từ: Một cậu bé nhìn trộm qua tấm …", q: "A boy is peeking through the ___.", options: ["curtain", "window", "door"], answer: "curtain", say: "A boy is peeking through the curtain." },
    ],
  },
  {
    id: "gardenmystery", title: "Describe the Garden Mystery", vi: "Mô tả bí ẩn khu vườn", image: L2 + "level-2-unit-06-community-garden-mystery.webp",
    emojis: ["🥕", "🍅", "🔍", "🦝", "🚿", "👣"],
    intro: "Nhìn khu vườn chung. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "tgm1", kind: "choose", vi: "Chọn câu mô tả ĐÚNG bức tranh.", q: "Which sentence is TRUE about the picture?", options: ["The children are looking for clues in the garden.", "The children are swimming.", "The children are sleeping."], answer: "The children are looking for clues in the garden.", say: "The children are looking for clues in the garden." },
      { id: "tgm2", kind: "spot", vi: "Câu nào KHÔNG đúng với tranh?", q: "Which sentence is FALSE about the picture?", options: ["A watering can is on the ground.", "A girl is holding a magnifying glass.", "A tiger is eating the plants."], answer: "A tiger is eating the plants.", say: "There is no tiger; a raccoon made the mess." },
      { id: "tgm3", kind: "fill", vi: "Điền từ: Bạn nữ dùng một chiếc … để soi dấu vết.", q: "The girl is using a ___ to look at the prints.", options: ["magnifying glass", "spoon", "hat"], answer: "magnifying glass", say: "The girl is using a magnifying glass to look at the prints." },
      { id: "tgm4", kind: "position", vi: "Con gấu mèo ở đâu?", q: "Where is the raccoon?", options: ["in the compost bin", "in the sky", "under the water"], answer: "in the compost bin", say: "The raccoon is in the compost bin." },
      { id: "tgm5", kind: "arrange", vi: "Xếp câu: Một con gấu mèo đang trốn trong thùng.", solution: ["A", "raccoon", "is", "hiding", "in", "the", "bin"], say: "A raccoon is hiding in the bin." },
      { id: "tgm6", kind: "arrange", vi: "Xếp câu: Bình tưới nằm trên mặt đất.", solution: ["The", "watering", "can", "is", "on", "the", "ground"], say: "The watering can is on the ground." },
      { id: "tgm7", kind: "fill", vi: "Điền từ: Các bạn tìm thấy dấu … thú trong bùn.", q: "The children found ___ prints in the mud.", options: ["paw", "hand", "bird"], answer: "paw", say: "The children found paw prints in the mud." },
    ],
  },
  {
    id: "aquarium", title: "Describe the Aquarium", vi: "Mô tả thủy cung về đêm", image: L2 + "level-2-unit-09-aquarium-night-mystery.webp",
    emojis: ["🐙", "🔦", "⚠️", "🪣", "🌊", "🕐"],
    intro: "Nhìn thủy cung ban đêm. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "taq1", kind: "choose", vi: "Chọn câu mô tả ĐÚNG bức tranh.", q: "Which sentence is TRUE about the picture?", options: ["The children are at an aquarium at night.", "The children are at a beach party.", "The children are in a classroom."], answer: "The children are at an aquarium at night.", say: "The children are at an aquarium at night." },
      { id: "taq2", kind: "spot", vi: "Câu nào KHÔNG đúng với tranh?", q: "Which sentence is FALSE about the picture?", options: ["An octopus is in the tank.", "The floor is wet.", "A dog is swimming in the tank."], answer: "A dog is swimming in the tank.", say: "There is no dog; an octopus is in the tank." },
      { id: "taq3", kind: "fill", vi: "Điền từ: Cậu bé cầm một chiếc … để nhìn trong bóng tối.", q: "The boy is holding a ___ to see in the dark.", options: ["flashlight", "spoon", "kite"], answer: "flashlight", say: "The boy is holding a flashlight to see in the dark." },
      { id: "taq4", kind: "position", vi: "Biển cảnh báo ở đâu?", q: "Where is the caution sign?", options: ["on the wet floor", "in the sky", "inside the tank"], answer: "on the wet floor", say: "The caution sign is on the wet floor." },
      { id: "taq5", kind: "arrange", vi: "Xếp câu: Sàn nhà bị ướt và trơn.", solution: ["The", "floor", "is", "wet", "and", "slippery"], say: "The floor is wet and slippery." },
      { id: "taq6", kind: "arrange", vi: "Xếp câu: Một con bạch tuộc sống trong bể.", solution: ["An", "octopus", "lives", "in", "the", "tank"], say: "An octopus lives in the tank." },
      { id: "taq7", kind: "fill", vi: "Điền từ: Các bạn thấy một con … trong bể lớn.", q: "They see an ___ in the big tank.", options: ["octopus", "airplane", "apple"], answer: "octopus", say: "They see an octopus in the big tank." },
    ],
  },
  {
    id: "cookingworkshop", title: "Describe the Cooking Workshop", vi: "Mô tả lớp nấu ăn", image: L2 + "level-2-unit-16-cooking-workshop-mix-up.webp",
    emojis: ["🧁", "🫐", "🍫", "⏲️", "🥣", "📋"],
    intro: "Nhìn lớp làm bánh. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "tcw1", kind: "choose", vi: "Chọn câu mô tả ĐÚNG bức tranh.", q: "Which sentence is TRUE about the picture?", options: ["The children are baking muffins.", "The children are washing a car.", "The children are flying a kite."], answer: "The children are baking muffins.", say: "The children are baking muffins." },
      { id: "tcw2", kind: "spot", vi: "Câu nào KHÔNG đúng với tranh?", q: "Which sentence is FALSE about the picture?", options: ["One bowl has blueberry batter.", "One bowl has chocolate batter.", "A cat is stirring the bowl."], answer: "A cat is stirring the bowl.", say: "No cat is stirring; the children are baking." },
      { id: "tcw3", kind: "fill", vi: "Điền từ: Tô bột nhạt màu có … bên trong.", q: "The pale batter has ___ in it.", options: ["blueberries", "rocks", "socks"], answer: "blueberries", say: "The pale batter has blueberries in it." },
      { id: "tcw4", kind: "position", vi: "Chiếc đồng hồ hẹn giờ ở đâu?", q: "Where is the timer?", options: ["on the table", "in the sky", "under the floor"], answer: "on the table", say: "The timer is on the table." },
      { id: "tcw5", kind: "arrange", vi: "Xếp câu: Chúng mình đang làm bánh muffin việt quất.", solution: ["We", "are", "making", "blueberry", "muffins"], say: "We are making blueberry muffins." },
      { id: "tcw6", kind: "arrange", vi: "Xếp câu: Tô bột sô-cô-la có màu nâu.", solution: ["The", "chocolate", "batter", "is", "brown"], say: "The chocolate batter is brown." },
      { id: "tcw7", kind: "fill", vi: "Điền từ: Các thẻ chỉ các … theo thứ tự.", q: "The step cards show the ___ in order.", options: ["steps", "stars", "shoes"], answer: "steps", say: "The step cards show the steps in order." },
    ],
  },
  {
    id: "wildlifecam", title: "Describe the Wildlife Camera", vi: "Mô tả máy ảnh rừng", image: L2 + "level-2-unit-14-wildlife-camera-surprise.webp",
    emojis: ["📷", "🦝", "👣", "🏕️", "🔦", "🔭"],
    intro: "Nhìn khu cắm trại bên hồ. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "twc1", kind: "choose", vi: "Chọn câu mô tả ĐÚNG bức tranh.", q: "Which sentence is TRUE about the picture?", options: ["The children are looking at wildlife photos.", "The children are swimming in the lake.", "The children are baking a cake."], answer: "The children are looking at wildlife photos.", say: "The children are looking at wildlife photos." },
      { id: "twc2", kind: "spot", vi: "Câu nào KHÔNG đúng với tranh?", q: "Which sentence is FALSE about the picture?", options: ["A camera is on the tree.", "There are paw prints in a photo.", "A dinosaur is sitting at the table."], answer: "A dinosaur is sitting at the table.", say: "There is no dinosaur; they study animal photos." },
      { id: "twc3", kind: "fill", vi: "Điền từ: Một chiếc máy ảnh … đã chụp ảnh vào ban đêm.", q: "A ___ camera took pictures at night.", options: ["trail", "toy", "paper"], answer: "trail", say: "A trail camera took pictures at night." },
      { id: "twc4", kind: "position", vi: "Chiếc đèn lồng ở đâu?", q: "Where is the lantern?", options: ["on the table", "in the lake", "on the roof"], answer: "on the table", say: "The lantern is on the table." },
      { id: "twc5", kind: "arrange", vi: "Xếp câu: Một con gấu mèo đã đến vào ban đêm.", solution: ["A", "raccoon", "came", "at", "night"], say: "A raccoon came at night." },
      { id: "twc6", kind: "arrange", vi: "Xếp câu: Máy ảnh đã chụp rất nhiều tấm hình.", solution: ["The", "camera", "took", "many", "photos"], say: "The camera took many photos." },
      { id: "twc7", kind: "fill", vi: "Điền từ: Những tấm ảnh cho thấy dấu … thú trong bùn.", q: "The photos show animal ___ in the mud.", options: ["prints", "books", "hats"], answer: "prints", say: "The photos show animal prints in the mud." },
    ],
  },
  {
    id: "teamproject", title: "Describe the Team Project", vi: "Mô tả buổi chọn dự án", image: L3 + "collection-01-making-choices/level-3-c01-unit-03-choosing-team-project.webp",
    emojis: ["🌱", "🔆", "🗺️", "🐝", "💡", "🧩"],
    intro: "Nhìn buổi chọn dự án nhóm. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "ttj1", kind: "choose", vi: "Chọn câu mô tả ĐÚNG bức tranh.", q: "Which sentence is TRUE about the picture?", options: ["The children are choosing a team project.", "The children are riding bikes.", "The children are sailing a boat."], answer: "The children are choosing a team project.", say: "The children are choosing a team project." },
      { id: "ttj2", kind: "spot", vi: "Câu nào KHÔNG đúng với tranh?", q: "Which sentence is FALSE about the picture?", options: ["One model has a solar panel.", "One model is a city map.", "A robot is teaching the class."], answer: "A robot is teaching the class.", say: "No robot is teaching; a teacher helps them." },
      { id: "ttj3", kind: "fill", vi: "Điền từ: Tấm pin … sạc điện thoại bằng ánh nắng.", q: "The ___ panel charges the phone with sunlight.", options: ["solar", "paper", "ice"], answer: "solar", say: "The solar panel charges the phone with sunlight." },
      { id: "ttj4", kind: "position", vi: "Những con ong ở đâu?", q: "Where are the bees?", options: ["in the flower model", "in the sky", "under the sea"], answer: "in the flower model", say: "The bees are in the flower model." },
      { id: "ttj5", kind: "arrange", vi: "Xếp câu: Chúng mình đang chọn một dự án nhóm.", solution: ["We", "are", "choosing", "a", "team", "project"], say: "We are choosing a team project." },
      { id: "ttj6", kind: "arrange", vi: "Xếp câu: Tấm pin mặt trời tạo ra điện.", solution: ["The", "solar", "panel", "makes", "power"], say: "The solar panel makes power." },
      { id: "ttj7", kind: "fill", vi: "Điền từ: Cô giáo cho xem một … nhóm trên máy tính bảng.", q: "The teacher shows a team ___ on her tablet.", options: ["diagram", "dinner", "dog"], answer: "diagram", say: "The teacher shows a team diagram on her tablet." },
    ],
  },
  {
    id: "greenfestival", title: "Describe the Green Festival", vi: "Mô tả lễ hội trường xanh", image: L3 + "collection-03-solving-problems/level-3-c03-unit-03-greener-school-festival.webp",
    emojis: ["♻️", "🌱", "💧", "🔆", "🏮", "🎪"],
    intro: "Nhìn lễ hội trường xanh. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "tgf1", kind: "choose", vi: "Chọn câu mô tả ĐÚNG bức tranh.", q: "Which sentence is TRUE about the picture?", options: ["The children are sorting waste into bins.", "The children are swimming.", "The children are sleeping."], answer: "The children are sorting waste into bins.", say: "The children are sorting waste into bins." },
      { id: "tgf2", kind: "spot", vi: "Câu nào KHÔNG đúng với tranh?", q: "Which sentence is FALSE about the picture?", options: ["There are three sorting bins.", "A girl is hanging flags.", "A whale is filling a bottle."], answer: "A whale is filling a bottle.", say: "There is no whale; children fill reusable bottles." },
      { id: "tgf3", kind: "fill", vi: "Điền từ: Các bạn đổ đầy bình … tại trạm nước.", q: "They fill their ___ bottles at the water station.", options: ["reusable", "paper", "glass"], answer: "reusable", say: "They fill their reusable bottles at the water station." },
      { id: "tgf4", kind: "position", vi: "Tấm pin mặt trời ở đâu?", q: "Where is the solar panel?", options: ["on the table", "in the sky", "in the bin"], answer: "on the table", say: "The solar panel is on the table." },
      { id: "tgf5", kind: "arrange", vi: "Xếp câu: Các bạn phân loại rác.", solution: ["The", "children", "sort", "the", "waste"], say: "The children sort the waste." },
      { id: "tgf6", kind: "arrange", vi: "Xếp câu: Chúng mình đổ nước đầy bình.", solution: ["We", "fill", "our", "bottles", "with", "water"], say: "We fill our bottles with water." },
      { id: "tgf7", kind: "fill", vi: "Điền từ: Thùng xanh dương dùng để …", q: "The blue bin is for ___.", options: ["recycling", "food", "clothes"], answer: "recycling", say: "The blue bin is for recycling." },
    ],
  },
  {
    id: "recessspace", title: "Describe the Recess Space", vi: "Mô tả sân chơi", image: L3 + "collection-03-solving-problems/level-3-c03-unit-10-designing-a-better-recess-space.webp",
    emojis: ["🏀", "🛝", "🧩", "♿", "🌳", "📋"],
    intro: "Nhìn buổi thiết kế sân chơi. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "trs1", kind: "choose", vi: "Chọn câu mô tả ĐÚNG bức tranh.", q: "Which sentence is TRUE about the picture?", options: ["The children are planning a new play space.", "The children are cooking dinner.", "The children are sailing a boat."], answer: "The children are planning a new play space.", say: "The children are planning a new play space." },
      { id: "trs2", kind: "spot", vi: "Câu nào KHÔNG đúng với tranh?", q: "Which sentence is FALSE about the picture?", options: ["A boy is using crutches.", "Children are playing basketball.", "A dragon is flying over the court."], answer: "A dragon is flying over the court.", say: "There is no dragon; children play in the yard." },
      { id: "trs3", kind: "fill", vi: "Điền từ: Bạn nữ cầm một tấm thẻ … để chấm điểm các ý tưởng.", q: "The girl is holding a ___ card to score the ideas.", options: ["rating", "birthday", "bus"], answer: "rating", say: "The girl is holding a rating card to score the ideas." },
      { id: "trs4", kind: "position", vi: "Vòng rổ bóng ở đâu?", q: "Where is the basketball hoop?", options: ["on the court", "in the classroom", "under the sea"], answer: "on the court", say: "The basketball hoop is on the court." },
      { id: "trs5", kind: "arrange", vi: "Xếp câu: Các bạn lên kế hoạch một sân chơi mới.", solution: ["The", "children", "plan", "a", "new", "play", "space"], say: "The children plan a new play space." },
      { id: "trs6", kind: "arrange", vi: "Xếp câu: Một cậu bé đang xếp các viên gạch.", solution: ["A", "boy", "is", "placing", "the", "tiles"], say: "A boy is placing the tiles." },
      { id: "trs7", kind: "fill", vi: "Điền từ: Họ muốn sân chơi vui cho …", q: "They want the playground to be fun for ___.", options: ["everyone", "no one", "only adults"], answer: "everyone", say: "They want the playground to be fun for everyone." },
    ],
  },
  {
    id: "screentime", title: "Describe the Choice", vi: "Mô tả lựa chọn trong nhà", image: L3 + "collection-01-making-choices/level-3-c01-unit-02-screen-or-outdoor-time.webp",
    emojis: ["📱", "🎧", "🚲", "🧩", "☀️", "🌳"],
    intro: "Nhìn cảnh phòng khách. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "tsn1", kind: "choose", vi: "Chọn câu mô tả ĐÚNG bức tranh.", q: "Which sentence is TRUE about the picture?", options: ["The children are deciding between screens and going outside.", "The children are cooking dinner.", "The children are driving a car."], answer: "The children are deciding between screens and going outside.", say: "The children are deciding between screens and going outside." },
      { id: "tsn2", kind: "spot", vi: "Câu nào KHÔNG đúng với tranh?", q: "Which sentence is FALSE about the picture?", options: ["A boy is holding a tablet.", "There are bicycles outside.", "A whale is sitting at the table."], answer: "A whale is sitting at the table.", say: "There is no whale; the friends are deciding what to do." },
      { id: "tsn3", kind: "fill", vi: "Điền từ: Ngoài trời nắng nên các bạn có thể đạp …", q: "Outside it is sunny, so they could ride ___.", options: ["bikes", "boats", "rockets"], answer: "bikes", say: "Outside it is sunny, so they could ride bikes." },
      { id: "tsn4", kind: "position", vi: "Chiếc tai nghe ở đâu?", q: "Where are the headphones?", options: ["on the table", "in the garden", "in the sky"], answer: "on the table", say: "The headphones are on the table." },
      { id: "tsn5", kind: "arrange", vi: "Xếp câu: Hôm nay chúng mình có thể chơi ngoài trời.", solution: ["We", "can", "play", "outside", "today"], say: "We can play outside today." },
      { id: "tsn6", kind: "arrange", vi: "Xếp câu: Máy tính bảng đang chiếu một trò chơi điện tử.", solution: ["The", "tablet", "shows", "a", "video", "game"], say: "The tablet shows a video game." },
      { id: "tsn7", kind: "fill", vi: "Điền từ: Bảng có các biểu tượng … như nắng và mưa.", q: "The board shows ___ icons like sun and rain.", options: ["weather", "money", "music"], answer: "weather", say: "The board shows weather icons like sun and rain." },
    ],
  },
  {
    id: "timecapsule", title: "Describe the Time Capsule", vi: "Mô tả hộp thời gian", image: "/assets/images/learn/level-2/level-2-unit-07-time-capsule-discovery.webp",
    emojis: ["📦", "🕰️", "🗺️", "🪙", "📷", "🚗", "🌳", "🖌️"],
    intro: "Nhìn cảnh khám phá hộp thời gian. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "capt1", kind: "choose", vi: "Chọn câu mô tả ĐÚNG bức tranh.", q: "Which sentence is TRUE about the picture?", options: ["The children dug up an old metal box.", "The children found a new computer.", "The children are cooking dinner."], answer: "The children dug up an old metal box.", say: "The children dug up an old metal box." },
      { id: "capt2", kind: "spot", vi: "Câu nào KHÔNG đúng với tranh?", q: "Which sentence is FALSE about the picture?", options: ["A girl is cleaning the box with a paintbrush.", "A boy is holding an old photo.", "The children are swimming in a pool."], answer: "The children are swimming in a pool.", say: "The children are not swimming. They are digging in a school garden." },
      { id: "capt3", kind: "fill", vi: "Điền từ: Một bạn cầm tấm ảnh … cũ.", q: "A boy is holding an old ___ photo.", options: ["black-and-white", "colourful", "brand-new"], answer: "black-and-white", say: "A boy is holding an old black-and-white photo." },
      { id: "capt4", kind: "position", vi: "Hình cái cây khắc ở đâu?", q: "Where is the carved tree symbol?", options: ["on the wooden plank", "on the school roof", "inside the coin"], answer: "on the wooden plank", say: "The carved tree symbol is on the wooden plank." },
      { id: "capt5", kind: "arrange", vi: "Xếp câu: Các bạn tìm thấy một chiếc hộp cũ.", solution: ["The", "children", "found", "an", "old", "box"], say: "The children found an old box." },
      { id: "capt6", kind: "arrange", vi: "Xếp câu: Một bạn trai đang cầm một đồng xu.", solution: ["A", "boy", "is", "holding", "a", "coin"], say: "A boy is holding a coin." },
      { id: "capt7", kind: "fill", vi: "Điền từ: Chiếc hộp cũ đến từ … .", q: "The old box is from the ___.", options: ["past", "future", "moon"], answer: "past", say: "The old box is from the past." },
    ],
  },
  {
    id: "rainycourtyard", title: "Describe the Rainy Courtyard", vi: "Mô tả sân trường ngày mưa", image: "/assets/images/learn/level-3/collection-03-solving-problems/level-3-c03-unit-11-rainy-school-courtyard.webp",
    emojis: ["🌧️", "☔", "📋", "📏", "💧", "🪑", "🧥", "🌱"],
    intro: "Nhìn sân trường ngày mưa. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "ryct1", kind: "choose", vi: "Chọn câu mô tả ĐÚNG bức tranh.", q: "Which sentence is TRUE about the picture?", options: ["It is raining in the courtyard.", "It is a hot sunny day.", "The children are at the beach."], answer: "It is raining in the courtyard.", say: "It is raining in the courtyard." },
      { id: "ryct2", kind: "spot", vi: "Câu nào KHÔNG đúng với tranh?", q: "Which sentence is FALSE about the picture?", options: ["A girl is holding a measuring stick.", "A boy is holding a clipboard.", "The children are sleeping on the bench."], answer: "The children are sleeping on the bench.", say: "The children are not sleeping. They are solving a rain problem." },
      { id: "ryct3", kind: "fill", vi: "Điền từ: Nước chảy ra từ … .", q: "Water is pouring from the ___.", options: ["drainpipe", "chimney", "window"], answer: "drainpipe", say: "Water is pouring from the drainpipe." },
      { id: "ryct4", kind: "position", vi: "Bạn nữ áo tím đang ở đâu?", q: "Where is the girl in the purple coat?", options: ["by the drain", "on the roof", "up in a tree"], answer: "by the drain", say: "The girl in the purple coat is by the drain." },
      { id: "ryct5", kind: "arrange", vi: "Xếp câu: Trời đang mưa trong sân trường.", solution: ["It", "is", "raining", "in", "the", "courtyard"], say: "It is raining in the courtyard." },
      { id: "ryct6", kind: "arrange", vi: "Xếp câu: Hai bạn nhỏ che tấm bạt lên ghế.", solution: ["Two", "children", "cover", "the", "bench"], say: "Two children cover the bench." },
      { id: "ryct7", kind: "fill", vi: "Điền màu: Bạn nữ cạnh ghế mặc áo mưa màu … .", q: "The girl by the bench is wearing a ___ raincoat.", options: ["yellow", "red", "green"], answer: "yellow", say: "The girl by the bench is wearing a yellow raincoat." },
    ],
  },
  {
    id: "camping", title: "Describe the camping trip", vi: "Mô tả chuyến cắm trại trong mưa", image: "/assets/images/learn/level-2/level-2-unit-02-stormy-camping-trip.webp",
    emojis: ["⛺", "🌧️", "🔦", "🗺️", "🎒", "🌲", "⛰️", "☕"],
    intro: "Nhìn cảnh cắm trại trong mưa. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "cmpt1", kind: "choose", vi: "Câu nào ĐÚNG về bức tranh?", q: "Which sentence is TRUE about the picture?", options: ["The children are setting up a tent.", "The children are building a house.", "The children are washing a car."], answer: "The children are setting up a tent.", say: "The children are setting up a tent." },
      { id: "cmpt2", kind: "spot", vi: "Câu nào SAI về bức tranh?", q: "Which sentence is FALSE about the picture?", options: ["It is raining.", "There are four children.", "It is sunny and hot."], answer: "It is sunny and hot.", say: "It is raining." },
      { id: "cmpt3", kind: "fill", vi: "Cậu bé đeo kính đang cầm một cái ___.", q: "The boy with glasses is holding a ___.", options: ["flashlight", "ball", "phone"], answer: "flashlight", say: "The boy with glasses is holding a flashlight." },
      { id: "cmpt4", kind: "position", vi: "Nhà chòi gỗ ở đâu?", q: "Where is the wooden shelter?", options: ["On the right side", "On the left side", "In the tent"], answer: "On the right side", say: "The wooden shelter is on the right side." },
      { id: "cmpt5", kind: "arrange", vi: "Xếp từ thành câu đúng.", solution: ["The", "children", "set", "up", "a", "tent"], say: "The children set up a tent." },
      { id: "cmpt6", kind: "arrange", vi: "Xếp từ thành câu đúng.", solution: ["The", "boy", "holds", "a", "map"], say: "The boy holds a map." },
      { id: "cmpt7", kind: "fill", vi: "Cái cốc màu xanh nằm trên ___.", q: "The blue mug is on the ___.", options: ["ground", "table", "tent"], answer: "ground", say: "The blue mug is on the ground." },
    ],
  },
  {
    id: "lunchroom", title: "Describe the quieter lunchroom", vi: "Mô tả phòng ăn yên tĩnh hơn", image: "/assets/images/learn/level-3/collection-03-solving-problems/level-3-c03-unit-01-quieter-lunchroom.webp",
    emojis: ["🍽️", "🪑", "🔇", "🟩", "🟨", "🧑‍🏫", "🎒", "🪟"],
    intro: "Nhìn cảnh phòng ăn. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "lnrt1", kind: "choose", vi: "Câu nào ĐÚNG về bức tranh?", q: "Which sentence is TRUE about the picture?", options: ["The children are making the lunchroom quieter.", "The children are swimming in a pool.", "The children are sleeping on the floor."], answer: "The children are making the lunchroom quieter.", say: "The children are making the lunchroom quieter." },
      { id: "lnrt2", kind: "spot", vi: "Câu nào SAI về bức tranh?", q: "Which sentence is FALSE about the picture?", options: ["The soft panels are hexagons.", "The girl holds a sound meter.", "The soft panels are stars."], answer: "The soft panels are stars.", say: "The soft panels are hexagons." },
      { id: "lnrt3", kind: "fill", vi: "Cậu bé áo hoodie xanh lá gắn ___ nỉ mềm vào chân ghế.", q: "The boy in the green hoodie puts soft felt ___ on the chair legs.", options: ["pads", "cups", "wheels"], answer: "pads", say: "The boy in the green hoodie puts soft felt pads on the chair legs." },
      { id: "lnrt4", kind: "position", vi: "Những miếng đệm nỉ dự phòng ở đâu?", q: "Where are the extra felt pads?", options: ["In a blue bin on the floor", "In a backpack", "On the ceiling"], answer: "In a blue bin on the floor", say: "The extra felt pads are in a blue bin on the floor." },
      { id: "lnrt5", kind: "arrange", vi: "Xếp từ thành câu đúng.", solution: ["The", "girl", "holds", "a", "sound", "meter"], say: "The girl holds a sound meter." },
      { id: "lnrt6", kind: "arrange", vi: "Xếp từ thành câu đúng.", solution: ["The", "panels", "are", "soft", "hexagons"], say: "The panels are soft hexagons." },
      { id: "lnrt7", kind: "fill", vi: "Người phụ nữ gần chậu cây đang xử lý hai chiếc ___ màu xanh.", q: "The woman near the plants is working with two blue ___.", options: ["chairs", "tables", "doors"], answer: "chairs", say: "The woman near the plants is working with two blue chairs." },
    ],
  },
  {
    id: "ferrytrip", title: "Describe the ferry trip", vi: "Mô tả chuyến đi phà", image: "/assets/images/learn/level-2/level-2-unit-04-ferry-trip-mix-up.webp",
    emojis: ["⛴️", "🚢", "🎫", "🕓", "🧳", "🗺️", "🌊", "👦"],
    intro: "Nhìn bến phà. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "fryt1", kind: "choose", vi: "Câu nào ĐÚNG về bức tranh?", q: "Which sentence is TRUE about the picture?", options: ["The children are waiting for a ferry.", "The children are waiting for a train.", "The children are waiting for a bus."], answer: "The children are waiting for a ferry.", say: "The children are waiting for a ferry." },
      { id: "fryt2", kind: "spot", vi: "Câu nào SAI về bức tranh?", q: "Which sentence is FALSE about the picture?", options: ["There are two ferries on the water.", "A big suitcase is on the cart.", "A dog is on the cart."], answer: "A dog is on the cart.", say: "A big suitcase is on the cart." },
      { id: "fryt3", kind: "fill", vi: "Cậu bé bên trái đang cầm ___ tấm vé.", q: "The boy on the left is holding ___ tickets.", options: ["two", "four", "no"], answer: "two", say: "The boy on the left is holding two tickets." },
      { id: "fryt4", kind: "position", vi: "Chiếc đồng hồ tròn ở đâu?", q: "Where is the round clock?", options: ["Near the sign board", "On the ground", "On the suitcase"], answer: "Near the sign board", say: "The clock is near the sign board." },
      { id: "fryt5", kind: "arrange", vi: "Xếp các từ thành câu.", solution: ["The", "woman", "is", "taller"], say: "The woman is taller." },
      { id: "fryt6", kind: "arrange", vi: "Xếp các từ thành câu.", solution: ["The", "boy", "holds", "a", "map"], say: "The boy holds a map." },
      { id: "fryt7", kind: "fill", vi: "Một vé màu xanh và một vé màu ___.", q: "One ticket is blue and one is ___.", options: ["red", "green", "yellow"], answer: "red", say: "One ticket is blue and one is red." },
    ],
  },
  {
    id: "bikeroute", title: "Describe the bike route", vi: "Mô tả tuyến đường xe đạp", image: "/assets/images/learn/level-3/collection-03-solving-problems/level-3-c03-unit-02-safer-bike-route.webp",
    emojis: ["🚲", "⛑️", "📏", "🚦", "🗺️", "📱", "🚧", "🌳"],
    intro: "Nhìn con đường. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "bkrt1", kind: "choose", vi: "Câu nào ĐÚNG về bức tranh?", q: "Which sentence is TRUE about the picture?", options: ["The children are wearing helmets.", "The children are wearing caps.", "The children are wearing crowns."], answer: "The children are wearing helmets.", say: "The children are wearing helmets." },
      { id: "bkrt2", kind: "spot", vi: "Câu nào SAI về bức tranh?", q: "Which sentence is FALSE about the picture?", options: ["The bikes are near the woman.", "The traffic cones are orange and white.", "The bikes are up in a tree."], answer: "The bikes are up in a tree.", say: "The bikes are near the woman." },
      { id: "bkrt3", kind: "fill", vi: "Hai bạn nhỏ đang kéo căng một ___ trên mặt đất.", q: "The two children are stretching a ___ on the ground.", options: ["measuring tape", "jump rope", "garden hose"], answer: "measuring tape", say: "The two children are stretching a measuring tape on the ground." },
      { id: "bkrt4", kind: "position", vi: "Những chiếc xe đạp ở đâu?", q: "Where are the bicycles?", options: ["On the right, near the woman", "Floating on a river", "Up in a tree"], answer: "On the right, near the woman", say: "The bikes are on the right, near the woman." },
      { id: "bkrt5", kind: "arrange", vi: "Xếp các từ thành câu.", solution: ["The", "children", "wear", "helmets"], say: "The children wear helmets." },
      { id: "bkrt6", kind: "arrange", vi: "Xếp các từ thành câu.", solution: ["The", "boy", "holds", "a", "tablet"], say: "The boy holds a tablet." },
      { id: "bkrt7", kind: "fill", vi: "Đèn tín hiệu hiện hình một ___ đang đi bộ màu trắng.", q: "The traffic light shows a white walking ___.", options: ["person", "car", "hand"], answer: "person", say: "The traffic light shows a white walking person." },
    ],
  },
  {
    id: "robotstart", title: "Describe the robot workshop", vi: "Mô tả xưởng sửa robot", image: "/assets/images/learn/level-2/level-2-unit-05-robot-wouldnt-start.webp",
    emojis: ["🤖", "🔧", "🔩", "🔋", "⚙️", "⏱️", "🛞", "👓"],
    intro: "Nhìn xưởng sửa robot. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "robt1", kind: "choose", vi: "Câu nào ĐÚNG với bức tranh?", q: "Which sentence is TRUE about the picture?", options: ["The robot has glowing blue eyes.", "The robot has glowing red eyes.", "The robot has glowing green eyes."], answer: "The robot has glowing blue eyes.", say: "The robot has glowing blue eyes." },
      { id: "robt2", kind: "spot", vi: "Câu nào SAI với bức tranh?", q: "Which sentence is FALSE about the picture?", options: ["Tools like wrenches hang on the wall.", "A small robot is on the floor.", "A sleeping cat is on the floor."], answer: "A sleeping cat is on the floor.", say: "A small robot is on the floor." },
      { id: "robt3", kind: "fill", vi: "Người phụ nữ phía sau đang ___ hai tay.", q: "The woman at the back is ___ her arms.", options: ["crossing", "waving", "clapping"], answer: "crossing", say: "The woman at the back is crossing her arms." },
      { id: "robt4", kind: "position", vi: "Vòng tròn phóng to bánh xe ở đâu?", q: "Where is the close-up circle of a wheel?", options: ["in the bottom right corner", "in the top left corner", "in the top right corner"], answer: "in the bottom right corner", say: "The close-up circle of a wheel is in the bottom right corner." },
      { id: "robt5", kind: "arrange", vi: "Xếp các từ theo đúng thứ tự.", solution: ["A", "boy", "works", "on", "the", "left"], say: "A boy works on the left." },
      { id: "robt6", kind: "arrange", vi: "Xếp các từ theo đúng thứ tự.", solution: ["The", "robot", "is", "on", "the", "floor"], say: "The robot is on the floor." },
      { id: "robt7", kind: "fill", vi: "Cậu bé mặc áo khoác xanh lá đeo kính ___.", q: "The boy in the green jacket wears ___ glasses.", options: ["round", "square", "red"], answer: "round", say: "The boy in the green jacket wears round glasses." },
    ],
  },
  {
    id: "foodwaste", title: "Describe the recycling cafeteria", vi: "Mô tả căng tin phân loại rác", image: "/assets/images/learn/level-3/collection-03-solving-problems/level-3-c03-unit-05-reducing-food-waste.webp",
    emojis: ["🍎", "🍌", "🗑️", "♻️", "🥛", "🧤", "🌱", "🍽️"],
    intro: "Nhìn căng tin trường. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "fwzt1", kind: "choose", vi: "Câu nào ĐÚNG với bức tranh?", q: "Which sentence is TRUE about the picture?", options: ["There are three sorting bins in the front.", "There are five sorting bins in the front.", "There are two sorting bins in the front."], answer: "There are three sorting bins in the front.", say: "There are three sorting bins in the front." },
      { id: "fwzt2", kind: "spot", vi: "Câu nào SAI với bức tranh?", q: "Which sentence is FALSE about the picture?", options: ["The scene takes place in a school cafeteria.", "Several children are wearing blue gloves.", "Several children are wearing pink gloves."], answer: "Several children are wearing pink gloves.", say: "Several children are wearing blue gloves." },
      { id: "fwzt3", kind: "fill", vi: "Cậu bé mặc áo hoodie xanh lá đang thả một ___ vào thùng.", q: "The boy in the green hoodie is dropping a ___ into a bin.", options: ["banana peel", "soccer ball", "pencil"], answer: "banana peel", say: "The boy in the green hoodie is dropping a banana peel into a bin." },
      { id: "fwzt4", kind: "position", vi: "Cậu bé ngồi xe lăn ở đâu?", q: "Where is the boy in the wheelchair?", options: ["on the left side", "on the right side", "in the middle"], answer: "on the left side", say: "The boy in the wheelchair is on the left side." },
      { id: "fwzt5", kind: "arrange", vi: "Xếp các từ theo đúng thứ tự.", solution: ["The", "adult", "stands", "on", "the", "right"], say: "The adult stands on the right." },
      { id: "fwzt6", kind: "arrange", vi: "Xếp các từ theo đúng thứ tự.", solution: ["The", "boy", "holds", "a", "clipboard"], say: "The boy holds a clipboard." },
      { id: "fwzt7", kind: "fill", vi: "Thùng màu ___ có hình lõi táo và vỏ chuối.", q: "The ___ bin has an apple core and banana peel picture.", options: ["green", "blue", "black"], answer: "green", say: "The green bin has an apple core and banana peel picture." },
    ],
  },
  {
    id: "beachcleanup", title: "Describe the beach cleanup", vi: "Mô tả buổi dọn dẹp bãi biển", image: "/assets/images/learn/level-2/level-2-unit-08-beach-cleanup-change-of-plan.webp",
    emojis: ["🏖️", "🌊", "🪣", "🧤", "🚩", "🐦", "🗺️", "♻️"],
    intro: "Nhìn cảnh dọn bãi biển. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "bcut1", kind: "choose", vi: "Câu nào ĐÚNG với bức tranh?", q: "Which sentence is TRUE about the picture?", options: ["The children are standing on a rocky beach.", "The children are swimming in a pool.", "The children are sitting in a classroom."], answer: "The children are standing on a rocky beach.", say: "The children are standing on a rocky beach." },
      { id: "bcut2", kind: "spot", vi: "Câu nào SAI với bức tranh?", q: "Which sentence is FALSE about the picture?", options: ["The seagulls are flying in the sky.", "The stacked buckets are blue.", "The seagulls are swimming in the water."], answer: "The seagulls are swimming in the water.", say: "The seagulls are flying in the sky." },
      { id: "bcut3", kind: "fill", vi: "Bạn trai bên trái đang cầm một chiếc ___ màu xanh.", q: "The boy on the left is holding a blue ___.", options: ["clipboard", "umbrella", "kite"], answer: "clipboard", say: "The boy on the left is holding a blue clipboard." },
      { id: "bcut4", kind: "position", vi: "Tấm bản đồ ở đâu?", q: "Where is the map?", options: ["On a wooden sign at the bottom left", "On the picnic table", "In the girl's hand"], answer: "On a wooden sign at the bottom left", say: "The map is on a wooden sign at the bottom left." },
      { id: "bcut5", kind: "arrange", vi: "Xếp thành câu về bạn gái.", solution: ["The", "girl", "holds", "a", "bag"], say: "The girl holds a bag." },
      { id: "bcut6", kind: "arrange", vi: "Xếp thành câu về những lá cờ.", solution: ["Two", "flags", "are", "on", "the", "pole"], say: "Two flags are on the pole." },
      { id: "bcut7", kind: "fill", vi: "Những thùng màu xanh đựng găng tay và ___ gắp rác.", q: "The blue bins have gloves and litter ___.", options: ["pickers", "books", "fruit"], answer: "pickers", say: "The blue bins have gloves and litter pickers." },
    ],
  },
  {
    id: "hotclassroom", title: "Describe the hot classroom", vi: "Mô tả lớp học nóng bức", image: "/assets/images/learn/level-3/collection-03-solving-problems/level-3-c03-unit-06-cooling-a-hot-classroom.webp",
    emojis: ["🏫", "🌀", "☀️", "🌳", "📋", "✏️", "📄", "🪴"],
    intro: "Nhìn cảnh lớp học nóng bức. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "hclt1", kind: "choose", vi: "Câu nào ĐÚNG với bức tranh?", q: "Which sentence is TRUE about the picture?", options: ["There is an electric fan on the table.", "There is a television on the table.", "There is a birthday cake on the table."], answer: "There is an electric fan on the table.", say: "There is an electric fan on the table." },
      { id: "hclt2", kind: "spot", vi: "Câu nào SAI với bức tranh?", q: "Which sentence is FALSE about the picture?", options: ["Four children are around the table.", "The potted plants are on the windowsill.", "The girl in the purple hoodie is sleeping on the desk."], answer: "The girl in the purple hoodie is sleeping on the desk.", say: "The girl in the purple hoodie is writing with a pencil." },
      { id: "hclt3", kind: "fill", vi: "Các bạn nhỏ đứng quanh một ___ bằng bìa cứng hình một tòa nhà.", q: "The children are around a cardboard ___ of a building.", options: ["model", "cake", "television"], answer: "model", say: "The children are around a cardboard model of a building." },
      { id: "hclt4", kind: "position", vi: "Chiếc quạt ở đâu?", q: "Where is the fan?", options: ["On the table near the model", "On the ceiling", "Outside the window"], answer: "On the table near the model", say: "The fan is on the table near the model." },
      { id: "hclt5", kind: "arrange", vi: "Xếp thành câu về chiếc quạt.", solution: ["The", "fan", "blows", "the", "air"], say: "The fan blows the air." },
      { id: "hclt6", kind: "arrange", vi: "Xếp thành câu về những chậu cây.", solution: ["The", "plants", "are", "on", "the", "windowsill"], say: "The plants are on the windowsill." },
      { id: "hclt7", kind: "fill", vi: "Cô giáo đang giơ lên một tấm ___ có các ký hiệu.", q: "The teacher is holding a ___ with symbols.", options: ["board", "pizza", "trophy"], answer: "board", say: "The teacher is holding a board with symbols." },
    ],
  },
  {
    id: "sciencefairmix", title: "Describe the science fair", vi: "Mô tả hội chợ khoa học", image: "/assets/images/learn/level-2/level-2-unit-11-science-fair-mix-up.webp",
    emojis: ["🔬", "🌬️", "💧", "🔋", "🌱", "💡", "📷", "📊"],
    intro: "Nhìn hội chợ khoa học. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "sfmt1", kind: "choose", vi: "Câu nào ĐÚNG với bức tranh?", q: "Which sentence is TRUE about the picture?", options: ["Four children are around the table.", "Six children are around the table.", "The wind turbine model is green."], answer: "Four children are around the table.", say: "Four children are around the table." },
      { id: "sfmt2", kind: "spot", vi: "Câu nào SAI với bức tranh?", q: "Which sentence is FALSE about the picture?", options: ["The wind turbine model is white.", "The woman wears an ID badge around her neck.", "The girl in the middle is holding a big book."], answer: "The girl in the middle is holding a big book.", say: "The girl in the middle is holding a photo of a waterfall." },
      { id: "sfmt3", kind: "fill", vi: "Mô hình tua-bin gió có màu ___.", q: "The wind turbine model is ___.", options: ["white", "red", "green"], answer: "white", say: "The wind turbine model is white." },
      { id: "sfmt4", kind: "position", vi: "Máy lọc nước cao ở đâu?", q: "Where is the tall water filter?", options: ["On the right side of the table", "On the floor", "Outside the window"], answer: "On the right side of the table", say: "The tall water filter is on the right side of the table." },
      { id: "sfmt5", kind: "arrange", vi: "Xếp các từ theo đúng thứ tự.", solution: ["The", "woman", "is", "the", "tallest"], say: "The woman is the tallest." },
      { id: "sfmt6", kind: "arrange", vi: "Xếp các từ theo đúng thứ tự.", solution: ["This", "is", "a", "science", "fair"], say: "This is a science fair." },
      { id: "sfmt7", kind: "fill", vi: "Người phụ nữ đeo ___ quanh cổ.", q: "The woman wears an ___ around her neck.", options: ["ID badge", "scarf", "headphones"], answer: "ID badge", say: "The woman wears an ID badge around her neck." },
    ],
  },
  {
    id: "pickupzone", title: "Describe the pickup zone", vi: "Mô tả khu vực đón học sinh", image: "/assets/images/learn/level-3/collection-03-solving-problems/level-3-c03-unit-07-improving-school-pickup-zone.webp",
    emojis: ["🚸", "🚌", "🚗", "🚦", "🚲", "🗺️", "📋", "🦺"],
    intro: "Nhìn khu vực đón học sinh. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "pznt1", kind: "choose", vi: "Câu nào ĐÚNG với bức tranh?", q: "Which sentence is TRUE about the picture?", options: ["The children are gathered around a wooden table.", "The children are gathered around a swimming pool.", "The adult is wearing a red vest."], answer: "The children are gathered around a wooden table.", say: "The children are gathered around a wooden table." },
      { id: "pznt2", kind: "spot", vi: "Câu nào SAI với bức tranh?", q: "Which sentence is FALSE about the picture?", options: ["The adult wears a yellow vest.", "The adult has a green cap on his head.", "The girl with the purple headband is holding a ball."], answer: "The girl with the purple headband is holding a ball.", say: "The girl with the purple headband is holding a clipboard." },
      { id: "pznt3", kind: "fill", vi: "Người lớn đang mặc áo phản quang màu ___.", q: "The adult is wearing a ___ vest.", options: ["yellow", "red", "blue"], answer: "yellow", say: "The adult is wearing a yellow vest." },
      { id: "pznt4", kind: "position", vi: "Chiếc xe buýt trường học màu vàng thật ở đâu?", q: "Where is the real yellow school bus?", options: ["In the background on the street", "On the table", "Inside the building"], answer: "In the background on the street", say: "The real yellow school bus is in the background on the street." },
      { id: "pznt5", kind: "arrange", vi: "Xếp các từ theo đúng thứ tự.", solution: ["The", "adult", "wears", "a", "green", "cap"], say: "The adult wears a green cap." },
      { id: "pznt6", kind: "arrange", vi: "Xếp các từ theo đúng thứ tự.", solution: ["The", "adult", "is", "a", "crossing", "guard"], say: "The adult is a crossing guard." },
      { id: "pznt7", kind: "fill", vi: "Bạn gái đội băng đô tím đang cầm một cái ___.", q: "The girl with the purple headband is holding a ___.", options: ["clipboard", "camera", "ball"], answer: "clipboard", say: "The girl with the purple headband is holding a clipboard." },
    ],
  },
  {
    id: "fieldtripdetour", title: "Describe the bus stop", vi: "Mô tả trạm chờ xe buýt", image: "/assets/images/learn/level-2/level-2-unit-12-field-trip-detour.webp",
    emojis: ["🚌", "🗺️", "🕓", "🌧️", "🎒", "🚢", "🧭", "🚧"],
    intro: "Nhìn trạm chờ xe buýt. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "ftdt1", kind: "choose", vi: "Câu nào ĐÚNG với bức tranh?", q: "Which sentence is TRUE about the picture?", options: ["The children are looking at a map.", "The children are looking at a laptop.", "The children are looking at a book."], answer: "The children are looking at a map.", say: "The children are looking at a map." },
      { id: "ftdt2", kind: "spot", vi: "Câu nào SAI với bức tranh?", q: "Which sentence is FALSE about the picture?", options: ["It is rainy in the picture.", "There is a bus behind the glass wall.", "The weather is sunny."], answer: "The weather is sunny.", say: "It is rainy in the picture." },
      { id: "ftdt3", kind: "fill", vi: "Điền từ đúng vào câu.", q: "The ___ is behind the glass wall.", options: ["bus", "truck", "train"], answer: "bus", say: "The bus is behind the glass wall." },
      { id: "ftdt4", kind: "position", vi: "Chiếc đồng hồ tròn ở đâu?", q: "Where is the round clock?", options: ["On the wall on the left", "On the table", "On the bus"], answer: "On the wall on the left", say: "The round clock is on the wall on the left." },
      { id: "ftdt5", kind: "arrange", vi: "Xếp các từ thành câu đúng.", solution: ["A", "boat", "is", "floating"], say: "A boat is floating." },
      { id: "ftdt6", kind: "arrange", vi: "Xếp các từ thành câu đúng.", solution: ["The", "woman", "is", "taller"], say: "The woman is taller." },
      { id: "ftdt7", kind: "fill", vi: "Điền từ đúng vào câu.", q: "The blue water bottle is on the ___ end of the table.", options: ["right", "left", "middle"], answer: "right", say: "The blue water bottle is on the right end of the table." },
    ],
  },
  {
    id: "watergarden", title: "Describe the school garden", vi: "Mô tả vườn trường", image: "/assets/images/learn/level-3/collection-03-solving-problems/level-3-c03-unit-08-saving-water-in-school-garden.webp",
    emojis: ["💧", "🌱", "🪴", "🚿", "🧤", "🪣", "🌸", "🏫"],
    intro: "Nhìn vườn trường. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "wtgt1", kind: "choose", vi: "Câu nào ĐÚNG với bức tranh?", q: "Which sentence is TRUE about the picture?", options: ["The boy in the green hoodie is wearing green gloves.", "The boy in the green hoodie is wearing red mittens.", "The boy in the green hoodie is wearing nothing on his hands."], answer: "The boy in the green hoodie is wearing green gloves.", say: "The boy in the green hoodie is wearing green gloves." },
      { id: "wtgt2", kind: "spot", vi: "Câu nào SAI với bức tranh?", q: "Which sentence is FALSE about the picture?", options: ["The boy in the blue hoodie is filling cups with water.", "Watering cans are lying on the ground.", "The girl in the purple jacket is holding a phone."], answer: "The girl in the purple jacket is holding a phone.", say: "The girl in the purple jacket is holding a tool that checks the soil." },
      { id: "wtgt3", kind: "fill", vi: "Điền từ đúng vào câu.", q: "The watering cans are lying on the ___ near the flowers.", options: ["ground", "roof", "table"], answer: "ground", say: "The watering cans are lying on the ground near the flowers." },
      { id: "wtgt4", kind: "position", vi: "Thùng nước lớn có vòi ở đâu?", q: "Where is the big water barrel with a tap?", options: ["On the left side", "On the right side", "In the sky"], answer: "On the left side", say: "The big water barrel is on the left side." },
      { id: "wtgt5", kind: "arrange", vi: "Xếp các từ thành câu đúng.", solution: ["The", "water", "barrel", "is", "bigger"], say: "The water barrel is bigger." },
      { id: "wtgt6", kind: "arrange", vi: "Xếp các từ thành câu đúng.", solution: ["The", "boy", "fills", "the", "cups"], say: "The boy fills the cups." },
      { id: "wtgt7", kind: "fill", vi: "Điền từ đúng vào câu.", q: "The picture sign board is on the ___ side.", options: ["right", "left", "top"], answer: "right", say: "The picture sign board is on the right side." },
    ],
  },
  {
    id: "communitynews", title: "Describe the news studio", vi: "Mô tả phòng làm bản tin", image: "/assets/images/learn/level-2/level-2-unit-13-community-news-mix-up.webp",
    emojis: ["🎙️", "📷", "🌧️", "☀️", "☁️", "🖥️", "🪴", "👩‍🏫"],
    intro: "Nhìn phòng làm bản tin. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "cnmt1", kind: "choose", vi: "Câu nào ĐÚNG với bức tranh?", q: "Which sentence is TRUE about the picture?", options: ["The boy in the green hoodie is using a sound recorder.", "The boy in the green hoodie is using a big camera.", "The tablet screen shows a football game."], answer: "The boy in the green hoodie is using a sound recorder.", say: "The boy in the green hoodie is using a sound recorder." },
      { id: "cnmt2", kind: "spot", vi: "Câu nào SAI với bức tranh?", q: "Which sentence is FALSE about the picture?", options: ["The tablet screen shows weather icons.", "A woman is standing behind the children.", "The weather outside the window is sunny."], answer: "The weather outside the window is sunny.", say: "The weather outside the window is rainy." },
      { id: "cnmt3", kind: "fill", vi: "Cô bé mặc áo khoác jean đang giơ lên một ___ về công viên.", q: "The girl in the denim jacket is holding up a ___ of a park.", options: ["photo", "newspaper", "drawing"], answer: "photo", say: "The girl in the denim jacket is holding up a photo of a park." },
      { id: "cnmt4", kind: "position", vi: "Chiếc cốc xanh đầy bút nằm ở đâu?", q: "Where is the blue mug full of pens?", options: ["On the table near the tablet", "On the bookshelf", "On the window sill"], answer: "On the table near the tablet", say: "The blue mug is on the table near the tablet." },
      { id: "cnmt5", kind: "arrange", vi: "Xếp các từ để mô tả thời tiết bên ngoài.", solution: ["The", "weather", "outside", "is", "rainy"], say: "The weather outside is rainy." },
      { id: "cnmt6", kind: "arrange", vi: "Xếp các từ để mô tả ai ở quanh bàn.", solution: ["More", "children", "are", "around", "the", "table"], say: "More children are around the table." },
      { id: "cnmt7", kind: "fill", vi: "Tấm ảnh cho thấy một ngày ___, nhưng bên ngoài trời đang mưa.", q: "The photo shows a ___ day, but outside it is rainy.", options: ["sunny", "snowy", "cloudy"], answer: "sunny", say: "The photo shows a sunny day, but outside it is rainy." },
    ],
  },
  {
    id: "lostandfound", title: "Describe the lost and found", vi: "Mô tả góc đồ thất lạc", image: "/assets/images/learn/level-3/collection-03-solving-problems/level-3-c03-unit-09-improving-lost-and-found.webp",
    emojis: ["🎒", "🧢", "🥤", "🧤", "🧣", "📋", "✅", "📷"],
    intro: "Nhìn góc đồ thất lạc. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "laft1", kind: "choose", vi: "Câu nào ĐÚNG với bức tranh?", q: "Which sentence is TRUE about the picture?", options: ["The girl in the purple hoodie is holding up a green water bottle.", "The girl in the purple hoodie is holding up a soccer ball.", "The boy with glasses is holding a scarf."], answer: "The girl in the purple hoodie is holding up a green water bottle.", say: "The girl in the purple hoodie is holding up a green water bottle." },
      { id: "laft2", kind: "spot", vi: "Câu nào SAI với bức tranh?", q: "Which sentence is FALSE about the picture?", options: ["The girl in the yellow hoodie is holding a purple beanie hat.", "The boy with glasses in the blue hoodie is holding a tablet.", "The colorful sorting bins are hanging on the wall."], answer: "The colorful sorting bins are hanging on the wall.", say: "The colorful sorting bins are on the table in the middle." },
      { id: "laft3", kind: "fill", vi: "Các kệ bên trái để những món đồ ___ như ba lô và túi.", q: "The shelves on the left store ___ items like backpacks and bags.", options: ["lost", "new", "clean"], answer: "lost", say: "The shelves on the left store lost items like backpacks and bags." },
      { id: "laft4", kind: "position", vi: "Tấm bảng trắng có danh sách các hình nằm ở đâu?", q: "Where is the whiteboard with the checklist of shapes?", options: ["On an easel on the right side", "On the floor", "Inside a bin"], answer: "On an easel on the right side", say: "The whiteboard is on an easel on the right side." },
      { id: "laft5", kind: "arrange", vi: "Xếp các từ để mô tả ai làm việc ở đây.", solution: ["More", "children", "work", "at", "the", "station"], say: "More children work at the station." },
      { id: "laft6", kind: "arrange", vi: "Xếp các từ để so sánh các kệ với các thùng.", solution: ["The", "shelves", "hold", "many", "more", "items"], say: "The shelves hold many more items." },
      { id: "laft7", kind: "fill", vi: "Các bạn nhỏ phân loại đồ để chúng dễ ___.", q: "The children sort the items to keep them easy to ___.", options: ["find", "lose", "hide"], answer: "find", say: "The children sort the items to keep them easy to find." },
    ],
  },
  {
    id: "festivalschedule", title: "Describe the festival setup", vi: "Mô tả cảnh chuẩn bị lễ hội", image: "/assets/images/learn/level-2/level-2-unit-15-festival-schedule-problem.webp",
    emojis: ["🎪", "🎸", "🕐", "🍂", "💡", "🎨", "🎉", "🏮"],
    intro: "Nhìn cảnh lễ hội mùa thu. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "fspt1", kind: "choose", vi: "Câu nào ĐÚNG với bức tranh?", q: "Which sentence is TRUE about the picture?", options: ["Five kids are sitting around the wooden table.", "Eight kids are sitting around the wooden table.", "The people on the stage are cooking food."], answer: "Five kids are sitting around the wooden table.", say: "Five kids are sitting around the wooden table." },
      { id: "fspt2", kind: "spot", vi: "Câu nào SAI với bức tranh?", q: "Which sentence is FALSE about the picture?", options: ["The band is playing music on the stage.", "It is autumn in the picture.", "The lit lantern is up in a tree."], answer: "The lit lantern is up in a tree.", say: "The lit lantern is on the right end of the table." },
      { id: "fspt3", kind: "fill", vi: "___ đang nằm trên bàn cạnh các tấm thẻ màu.", q: "The ___ are lying flat on the table next to the colored cards.", options: ["clocks", "books", "plates"], answer: "clocks", say: "The clocks are lying flat on the table next to the colored cards." },
      { id: "fspt4", kind: "position", vi: "Chiếc đèn lồng đang sáng nằm ở đâu?", q: "Where is the lit lantern?", options: ["On the right end of the table", "On the stage", "Up in a tree"], answer: "On the right end of the table", say: "The lit lantern is on the right end of the table." },
      { id: "fspt5", kind: "arrange", vi: "Xếp các từ thành câu đúng.", solution: ["The", "band", "is", "playing", "music"], say: "The band is playing music." },
      { id: "fspt6", kind: "arrange", vi: "Xếp các từ thành câu đúng.", solution: ["The", "woman", "holds", "a", "card"], say: "The woman holds a card." },
      { id: "fspt7", kind: "fill", vi: "Một người đang leo ___ phía sau các bạn nhỏ.", q: "A person is climbing a ___ behind the kids.", options: ["ladder", "stage", "tree"], answer: "ladder", say: "A person is climbing a ladder behind the kids." },
    ],
  },
  {
    id: "beachpath", title: "Describe the beach path", vi: "Mô tả lối đi ra biển", image: "/assets/images/learn/level-3/collection-03-solving-problems/level-3-c03-unit-12-accessible-beach-path.webp",
    emojis: ["🏖️", "♿", "📏", "📝", "🌊", "🪵", "⛰️", "🌲"],
    intro: "Nhìn cảnh lối đi ra biển. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "bptt1", kind: "choose", vi: "Câu nào ĐÚNG với bức tranh?", q: "Which sentence is TRUE about the picture?", options: ["The girl on the right is sitting in a wheelchair.", "The girl on the right is sitting on a wooden bench.", "The path across the sand is made of glass."], answer: "The girl on the right is sitting in a wheelchair.", say: "The girl on the right is sitting in a wheelchair." },
      { id: "bptt2", kind: "spot", vi: "Câu nào SAI với bức tranh?", q: "Which sentence is FALSE about the picture?", options: ["The path across the sand is made of wooden boards.", "You can see mountains and the sea in the background.", "The open notebook is in the sea."], answer: "The open notebook is in the sea.", say: "The open notebook is on the ground in the front left." },
      { id: "bptt3", kind: "fill", vi: "Bạn nam quỳ mặc áo hoodie xanh lá đang dùng một ___.", q: "The kneeling boy in the green hoodie is using a ___.", options: ["measuring tape", "paintbrush", "camera"], answer: "measuring tape", say: "The kneeling boy in the green hoodie is using a measuring tape." },
      { id: "bptt4", kind: "position", vi: "Bạn nam mặc áo hoodie xanh dương đẩy xe kéo ở đâu?", q: "Where is the boy in the blue hoodie pushing the cart?", options: ["On the wooden path", "In the ocean", "Up a tree"], answer: "On the wooden path", say: "The boy pushing the cart is on the wooden path." },
      { id: "bptt5", kind: "arrange", vi: "Xếp các từ thành câu đúng.", solution: ["The", "girl", "sits", "in", "a", "wheelchair"], say: "The girl sits in a wheelchair." },
      { id: "bptt6", kind: "arrange", vi: "Xếp các từ thành câu đúng.", solution: ["You", "can", "see", "the", "sea"], say: "You can see the sea." },
      { id: "bptt7", kind: "fill", vi: "Bạn gái mặc áo khoác tím đang cầm một ___.", q: "The girl in the purple jacket is holding a ___.", options: ["clipboard", "camera", "ball"], answer: "clipboard", say: "The girl in the purple jacket is holding a clipboard." },
    ],
  },
  {
    id: "museumaudio", title: "Describe the museum audio guides", vi: "Mô tả buổi tham quan bảo tàng với máy thuyết minh", image: "/assets/images/learn/level-2/level-2-unit-18-museum-audio-guide-mix-up.webp",
    emojis: ["🦖", "🦴", "🏛️", "📱", "🔋", "🐢", "🍃", "📝"],
    intro: "Nhìn cảnh bảo tàng. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "magt1", kind: "choose", vi: "Câu nào ĐÚNG với bức tranh?", q: "Which sentence is TRUE about the picture?", options: ["A big dinosaur skeleton stands on the left.", "An elephant skeleton stands on the left.", "A whale skeleton stands on the left."], answer: "A big dinosaur skeleton stands on the left.", say: "A big dinosaur skeleton stands on the left." },
      { id: "magt2", kind: "spot", vi: "Câu nào SAI với bức tranh?", q: "Which sentence is FALSE about the picture?", options: ["The boy on the left holds a device to his ear.", "The boy with glasses writes on the paper.", "The boy with glasses is sleeping on the table."], answer: "The boy with glasses is sleeping on the table.", say: "The boy with glasses is writing on the paper with a pen." },
      { id: "magt3", kind: "fill", vi: "Có ___ chiếc máy trong khay sạc.", q: "There are ___ devices in the charging dock.", options: ["four", "two", "seven"], answer: "four", say: "There are four devices in the charging dock." },
      { id: "magt4", kind: "position", vi: "Khay sạc với bốn chiếc máy ở đâu?", q: "Where is the charging dock with the four devices?", options: ["On the table at the left", "On the ceiling", "Outside the window"], answer: "On the table at the left", say: "The charging dock is on the table at the left." },
      { id: "magt5", kind: "arrange", vi: "Xếp các từ thành câu.", solution: ["The", "girl", "holds", "two", "devices"], say: "The girl holds two devices." },
      { id: "magt6", kind: "arrange", vi: "Xếp các từ thành câu.", solution: ["The", "green", "device", "shows", "a", "leaf"], say: "The green device shows a leaf." },
      { id: "magt7", kind: "fill", vi: "Bạn có thể thấy một con ___ biển trong ô kính màu xanh.", q: "You can see a sea ___ in the blue display.", options: ["turtle", "lion", "horse"], answer: "turtle", say: "You can see a sea turtle in the blue display." },
    ],
  },
  {
    id: "pollinator", title: "Describe the pollinator garden", vi: "Mô tả khu vườn thụ phấn ở trường", image: "/assets/images/learn/level-3/collection-03-solving-problems/level-3-c03-unit-14-pollinator-school-garden.webp",
    emojis: ["🦋", "🐝", "🌻", "🌸", "🌿", "📓", "✏️", "💧"],
    intro: "Nhìn cảnh khu vườn. Chọn, xếp câu mô tả đúng — rồi nói theo Maple nhé!",
    tasks: [
      { id: "polt1", kind: "choose", vi: "Câu nào ĐÚNG với bức tranh?", q: "Which sentence is TRUE about the picture?", options: ["A butterfly flies near the flowers on the left.", "A bird flies near the flowers on the left.", "A bat flies near the flowers on the left."], answer: "A butterfly flies near the flowers on the left.", say: "A butterfly flies near the flowers on the left." },
      { id: "polt2", kind: "spot", vi: "Câu nào SAI với bức tranh?", q: "Which sentence is FALSE about the picture?", options: ["The girl on the left is writing in a notebook.", "Bees are resting on the flowers.", "The girl on the left is cooking dinner."], answer: "The girl on the left is cooking dinner.", say: "The girl on the left is writing in a notebook." },
      { id: "polt3", kind: "fill", vi: "Bạn gái bên trái mặc áo hoodie màu ___.", q: "The girl on the left wears a ___ hoodie.", options: ["purple", "orange", "black"], answer: "purple", say: "The girl on the left wears a purple hoodie." },
      { id: "polt4", kind: "position", vi: "Những con ong đang đậu ở đâu?", q: "Where are the bees resting?", options: ["On the flowers", "Under the ground", "In the sky"], answer: "On the flowers", say: "The bees are resting on the flowers." },
      { id: "polt5", kind: "arrange", vi: "Xếp các từ thành câu.", solution: ["The", "girl", "holds", "bamboo", "tubes"], say: "The girl holds bamboo tubes." },
      { id: "polt6", kind: "arrange", vi: "Xếp các từ thành câu.", solution: ["The", "boy", "points", "at", "the", "water"], say: "The boy points at the water." },
      { id: "polt7", kind: "fill", vi: "Một con ___ đang bay gần những bông hoa bên trái.", q: "A ___ is flying near the flowers on the left.", options: ["butterfly", "bird", "bat"], answer: "butterfly", say: "A butterfly is flying near the flowers on the left." },
    ],
  },
];
export const talkSceneById = (id: string) => TALK_SCENES.find((s) => s.id === id);
export function randomTalkScene(): TalkScene {
  return TALK_SCENES[Math.floor(Math.random() * TALK_SCENES.length)];
}
