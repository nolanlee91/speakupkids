  ...
  return <div className="clubhouse-overlay clubhouse-v2" role="dialog" aria-modal="true" aria-label="Maple Clubhouse">
    ...
    <main className="clubhouse-main">
      ...
      {panel === "shop" && <section className="clubhouse-drawer clubhouse-shop">
        <header><div><span className="rl-kicker">MAPLE MARKET · {CLUBHOUSE_SHOP.length} ITEMS</span><h3>Chọn phong cách của con</h3><p>Hai bộ sưu tập · mua một lần, sở hữu mãi · cuộn để xem tất cả.</p></div><button className="drawer-close" onClick={() => setPanel("none")}>×</button></header>
        {shopMessage && <div className="shop-message" role="status">◆ {shopMessage}</div>}
        <div className="shop-grid">{CLUBHOUSE_SHOP.map((item) => { const owned = purchased.has(item.id); const canBuy = state.clubhouse.coins >= item.price; return <article key={item.id} role="button" tabIndex={owned ? -1 : 0} aria-disabled={owned} aria-label={`${owned ? "Đã sở hữu" : "Mua"} ${item.en}, ${item.price} Coins`} className={`${owned ? "owned" : ""} ${!owned && !canBuy ? "needs-coins" : ""}`} onClick={() => buy(item)} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); buy(item); } }}><span className={`shop-card-badge ${owned ? "is-owned" : ""}`}>{owned ? "OWNED ✓" : <>◆ {item.price}</>}</span><ShopArt item={item} className="large" /><div><small>{item.collection === "cosmic" ? "COSMIC CLUB" : "TRAIL CLUB"}</small><h4>{item.en}</h4><p>{item.vi}</p></div><button type="button" disabled={owned} tabIndex={-1} onClick={(e) => { e.stopPropagation(); buy(item); }}>{owned ? "Đã có trong kho" : canBuy ? "Mua vật phẩm" : `Thiếu ${item.price - state.clubhouse.coins} Coins`}</button></article>; })}</div>
      </section>}
